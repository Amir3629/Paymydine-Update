        ...
        return (
          ...
          >
            <div className="flex items-center gap-2 mb-4">
              ...
              <div className="flex items-center gap-2">
                <img
                  src={iconForPayment("worldline")}
                  alt="Worldline"
                  width={32}
                  height={20}
                  className="object-contain"
                />
                <span className="font-semibold text-paydine-elegant-gray">
                  {selectedMethod?.name || "Worldline"}
                </span>
              </div>
            </div>
...
case "cod":
  ...
  const renderPaymentButton = () => {
    ...
    if (["stripe", "authorizenetaim", "visa", "mastercard", "paypal", "paypalexpress", "worldline"].includes(selectedMethod.code)) {
      return null
    }
...
{/* Totals */}
          ...
          {selectedPaymentMethod && ["stripe","authorizenetaim","visa","mastercard","apple_pay","google_pay","paypal","paypalexpress","cod","worldline"].includes(selectedPaymentMethod) && (
            <div className="pt-3">
              {renderPaymentForm()}
            </div>
          )}
                ...
                <div className="flex justify-center items-center gap-3 flex-wrap">
                  {loadingPayments ? (
                    ...
                    (() => {
                      ...
                      const withWorldline = base.some((m) => m.code === "worldline")
                        ? base
                        : [...base, { code: "worldline", name: "Worldline" } as PaymentMethod]

                      const has = new Set(withWorldline.map((m) => m.code))
                      const extra: PaymentMethod[] = []

                      const apEnabled = stripeConfig ? !!stripeConfig.applePayEnabled : true
                      const gpEnabled = stripeConfig ? !!stripeConfig.googlePayEnabled : true

                      if (apEnabled && !has.has("apple_pay")) {
                        extra.push({ code: "apple_pay", name: "Apple Pay" } as PaymentMethod)
                      }
                      if (gpEnabled && !has.has("google_pay")) {
                        extra.push({ code: "google_pay", name: "Google Pay" } as PaymentMethod)
                      }

                      return [...withWorldline, ...extra]
                    })().map((method) => (
