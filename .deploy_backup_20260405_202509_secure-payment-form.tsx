...
function pmdWorldlineDebug(label: string, data: any) {
  try {
    console.info("[PMD_WL_DEBUG]", label, data)
  } catch {}
}
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      onPaymentComplete({
        ...
        try {
          pmdWorldlineDebug("encrypt_failed", {
            message: e?.message || String(e),
            values: typeof (paymentRequest as any)?.getValues === "function"
              ? (paymentRequest as any).getValues()
              : null,
            rawExpiryInput: formData.expiryDate,
            normalizedExpiryInput: normalizeExpiry(formData.expiryDate),
            rawCvvInput: formData.cvv,
          })
        } catch {}
