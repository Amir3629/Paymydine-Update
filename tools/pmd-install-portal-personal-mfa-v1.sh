...
EXISTING=("$LOGIN" "$TRUSTED" "$LIFECYCLE")
NEWFILES=("$MIGRATION" "$SERVICE" "$TRAIT" "$VIEW")
ALLFILES=("${EXISTING[@]}" "${NEWFILES[@]}")

TMPROOT="$(mktemp -d /tmp/pmd-portal-mfa.XXXXXX)"
PATCHER="$TMPROOT/patch_integration.py"
BACKUP="/tmp/pmd-portal-mfa-backup-$(date +%Y%m%d-%H%M%S)"
SOURCE_WORKTREE=""
APPLY_STARTED=0
...
test -f "app/Services/PmdOwnerTotpService.php"
test -f "app/admin/traits/ValidatesForm.php"
test -f "app/admin/views/auth/login_workplace_v4.blade.php"
test -f "app/system/database/migrations/2026_08_30_123000_create_pmd_owner_mfa_table.php"
...
for f in "${NEWFILES[@]}"; do
    git cat-file -e "${REF}:${f}" || { echo "STOP: feature branch missing $f"; exit 21; }
done
...
checks = {
    'app/admin/controllers/Login.php': [
        ('PMD_PORTAL_PERSONAL_MFA_LOGIN_INTEGRATION_V1', 'use Admin\\Traits\\ValidatesForm;'),
        ('PMD_PORTAL_PERSONAL_MFA_LOGIN_INTEGRATION_V1', '// PMD_TRUSTED_PASSWORD_POST_RESUME_V3'),
    ],
    'app/Services/PmdTrustedLoginDeviceService.php': [
        ('PMD_PORTAL_MFA_NO_PERSISTENT_TRUST_V1', 'public function rememberVerifiedResponse(Request $request, $response)'),
        ('PMD_PORTAL_MFA_NO_TRUSTED_RESUME_V1', 'public function resumeIfPossible(Request $request)'),
    ],
    'app/Services/SuperAdminTenantLifecycleService.php': [
        ('PMD_NEW_TENANT_PORTAL_MFA_SCHEMA_V1', '2026_08_30_123000_create_pmd_owner_mfa_table.php'),
    ],
}
for path, pairs in checks.items():
    s = Path(path).read_text()
    for marker, anchor in pairs:
        if marker not in s and anchor not in s:
            raise SystemExit(f'STOP: integration anchor missing in {path}: {anchor}')
print('Production integration anchors OK')
PY
...
def patch_lifecycle():
    ...
    s = replace_once(
        ...
        "        'pmd_owner_mfa',",
        "        'pmd_owner_mfa',\n        'pmd_portal_mfa',",
        'Lifecycle empty security table'
    )
    ...
    verify_anchor = "            'pmd_owner_mfa',\n        ] as $table) {"
    verify_new = "            'pmd_owner_mfa',\n            'pmd_portal_mfa',\n        ] as $table) {"
...
for file in [LOGIN, TRUSTED, LIFECYCLE]:
    if not file.is_file():
...
grep -Fq "PMD_PORTAL_PERSONAL_MFA_LOGIN_V1" "$TMPROOT/$TRAIT"
...
grep -nF "PMD_PORTAL_PERSONAL_MFA_LOGIN_V1" "$TRAIT"
...
SOURCE_WORKTREE="/tmp/pmd-portal-source-$(date +%Y%m%d-%H%M%S)-$$"
...
if git worktree add --detach "$SOURCE_WORKTREE" "$REF" >/dev/null 2>&1; then
    if python3 "$PATCHER" "$SOURCE_WORKTREE" \
       ...
       && git -C "$SOURCE_WORKTREE" diff --check; then
        ...
        git -C "$SOURCE_WORKTREE" add -- "$LOGIN" "$TRUSTED" "$LIFECYCLE"
        ...
        if git -C "$SOURCE_WORKTREE" diff --cached --quiet; then
            ...
            SOURCE_SHA="$(git -C "$SOURCE_WORKTREE" rev-parse HEAD)"
              ...
              commit -m "Integrate personal Portal MFA login" >/dev/null \
...
echo ""
echo "========================================"
echo "SUCCESS - LIVE PORTAL MFA INSTALLED"
echo "========================================"
echo "Behavior:"
