...
REMOTE_BRANCH="${REMOTE_BRANCH:-feat/quick-pos-canonical-floor-v35}"
REF="origin/${REMOTE_BRANCH}"
STAMP="$(date +%Y%m%d_%H%M%S)"
STAGE="$(mktemp -d /tmp/pmd-qpos-floor-v35.XXXXXX)"
BACKUP="$ROOT/storage/pmd-patch-backups/qpos-canonical-floor-v35-$STAMP"
DEPLOY_COMPLETE=0
...
echo "=============================================================="
echo " PayMyDine QUICK POS CANONICAL FLOOR V35"
echo " Branch: $REF"
echo " Backup: $BACKUP"
echo "=============================================================="
...
echo
echo "===== STAGE EXACT FLOOR + QUICK POS FILES ====="
for file in "${FILES[@]}"; do
...
if grep -Fq "data-qpos-floor-map-stage" "$STAGE/app/admin/views/pmd_quick_pos_v1.blade.php"; then
  echo "ERROR: old POS-specific floor renderer still exists in the view" >&2
  exit 1
fi
...
if grep -Fq "data-qpos-map-table" "$STAGE/app/admin/assets/js/pmd-quick-pos-v1.js"; then
  echo "ERROR: old POS-specific floor-table renderer still exists in JS" >&2
  exit 1
fi
...
grep -q "pmd-quick-pos-v1.css?v=20260920-35" "$STAGE/app/admin/views/pmd_quick_pos_v1.blade.php"
grep -q "pmd-quick-pos-v1.js?v=20260920-35" "$STAGE/app/admin/views/pmd_quick_pos_v1.blade.php"
grep -q "pmd-dashboard-lab-exact-floor-v1.js?v=20260920-floor-v35" "$STAGE/app/admin/views/pmd_quick_pos_v1.blade.php"
grep -q "pmd-shared-floor-multi-floor-v1.js?v=20260920-floor-v35" "$STAGE/app/admin/views/pmd_quick_pos_v1.blade.php"

echo
echo "===== LIVE ROUTE + CANONICAL ASSET GUARD ====="
grep -q "PmdQuickPosV1::class, 'floorAjax'" "$ROOT/routes/admin-quick-mode.php"
for file in   "app/admin/assets/css/pmd-floor-v1.css"   "app/admin/assets/css/pmd-floor-v1-stable-v11.css"   "app/admin/assets/css/pmd-floor-v1-native-smart-v20.css"   "app/admin/assets/css/pmd-reservations2-floor-canvas-v310.css"   "app/admin/assets/css/pmd-reservations2-floor-toolbar-v316.css"   "app/admin/assets/css/pmd-reservations2-floor-reservation-v312.css"   "app/admin/assets/css/pmd-dashboard-lab-exact-floor-v1.css"   "app/admin/assets/css/pmd-shared-floor-multi-floor-v1.css"   "app/admin/assets/js/pmd-dashboard-lab-exact-floor-v1.js"; do
  test -s "$ROOT/$file"
done
echo "OK canonical Dashboard Floor assets exist"
...
grep -q "admin::_partials.pmd_dashboard_lab_exact_floor_v1" "$ROOT/app/admin/views/pmd_quick_pos_v1.blade.php"
grep -q "PMD_QPOS_EXACT_DASHBOARD_FLOOR_UI_V26" "$ROOT/app/admin/assets/js/pmd-quick-pos-v1.js"
grep -q "PMD_DEFAULT_FLOOR_RENAME_ASSIGNMENT_V1" "$ROOT/app/admin/assets/js/pmd-shared-floor-multi-floor-v1.js"
grep -q "pmd-quick-pos-v1.css?v=20260920-35" "$ROOT/app/admin/views/pmd_quick_pos_v1.blade.php"

DEPLOY_COMPLETE=1

echo
echo "=============================================================="
echo " QUICK POS CANONICAL FLOOR V35 DEPLOY COMPLETE"
echo " Backup: $BACKUP"
echo " Quick POS Map now embeds the same Dashboard Floor partial/CSS/JS."
echo " Old POS-specific floor renderer is removed."
echo " V33/V34 food-card polish is preserved."
echo "=============================================================="
