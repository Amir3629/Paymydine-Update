...
REMOTE_BRANCH="${REMOTE_BRANCH:-fix/quick-pos-check-cleaning-v40}"
REF="origin/${REMOTE_BRANCH}"
STAMP="$(date +%Y%m%d_%H%M%S)"
STAGE="$(mktemp -d /tmp/pmd-qpos-v40.XXXXXX)"
BACKUP="$ROOT/storage/pmd-patch-backups/qpos-check-cleaning-v40-$STAMP"
DEPLOY_COMPLETE=0
...
echo "=============================================================="
echo " PayMyDine QUICK POS CHECK + CLEANING FIX V40"
echo " Branch: $REF"
echo " Backup: $BACKUP"
echo "=============================================================="
...
if grep -Fq "$('[data-qpos-check]', box).forEach" "$STAGE/app/admin/assets/js/pmd-quick-pos-v1.js"; then
...
echo
echo "=============================================================="
echo " QUICK POS CHECK + CLEANING FIX V40 DEPLOY COMPLETE"
echo " Backup: $BACKUP"
