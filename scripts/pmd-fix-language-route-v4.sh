...
sha256sum "$ROUTES" | tee "$BACKUP/routes-before.sha256"
...
print('ROUTE_V4_PATCHED=1')
PY

if ! php -l "$ROUTES"; then
  cp -a "$BACKUP/routes.php.before" "$ROUTES"
  echo "ERROR=PHP syntax failed; original routes.php restored." >&2
  exit 2
fi

# Confirm only the intended marked block changed.
python3 - "$BACKUP/routes.php.before" "$ROUTES" <<'PY'
from pathlib import Path
import sys
...
sha256sum "$ROUTES" | tee "$BACKUP/routes-after.sha256"

echo "BACKUP=$BACKUP"
echo "ROUTE_FIX_OK=1"
echo "NEXT=Hard refresh Mimoza admin and click the language toggle once."
echo "ROLLBACK=cp -a '$BACKUP/routes.php.before' '$ROUTES'"
