...
BRANCH="${PMD_R19_BRANCH:-origin/fix/platform-performance-complete-r19}"
STAMP="$(date '+%Y%m%d_%H%M%S')"
BACKUP="$ROOT/storage/pmd-patch-backups/platform-performance-complete-r19-$STAMP"
STAGE="$(mktemp -d /tmp/pmd-r19-stage.XXXXXX)"
DEPLOY_STARTED=0
DEPLOY_COMPLETE=0
...
restore_files() {
  for file in "${FILES[@]}"; do
    ...
    restore_tmp="$ROOT/$file.pmd-r19-rollback-$STAMP.tmp"
...
rollback_if_needed() {
  ...
  if [ "$DEPLOY_STARTED" -eq 1 ] && [ "$DEPLOY_COMPLETE" -ne 1 ]; then
    echo
    echo "===== R19 DEPLOY FAILED: RESTORING PRE-RUN FILES =====" >&2
    restore_files
...
echo "=============================================================="
echo " PayMyDine PERFORMANCE R19"
echo " Branch: $BRANCH"
echo " Backup: $BACKUP"
echo "=============================================================="

git fetch origin fix/platform-performance-complete-r19

echo
echo "===== STAGE + VALIDATE R19 FILES ====="
for file in "${FILES[@]}"; do
...
echo
echo "===== R19 CONTENT PRECHECK ====="
grep -q 'PMD_PERF_R19_SKIP_UNUSED_GLOBAL_MEDIAMANAGER'   "$STAGE/app/admin/classes/AdminController.php"
grep -q 'PMD_PERF_R19_FLOOR_BOOTSTRAP_MICROCACHE'   "$STAGE/app/admin/classes/PmdCleanWorkspaceControllerV1.php"
grep -Fq 'now()->addSeconds(3)'   "$STAGE/app/admin/classes/PmdCleanWorkspaceControllerV1.php"
grep -Fq 'now()->addSeconds(2)'   "$STAGE/app/admin/classes/PmdCleanWorkspaceControllerV1.php"
grep -Fq "request()->is('admin/orders')"   "$STAGE/app/main/widgets/MediaManager.php"

# Preserve the working R18 request-local guards.
...
# Preserve the successful R17 schema strategy.
...
echo "OK R19 performance markers present"
echo "OK R18 request-local guards remain present"
...
echo
echo "===== DEPLOY VALIDATED R19 FILES ====="
DEPLOY_STARTED=1
...
for file in "${FILES[@]}"; do
  ...
  live_tmp="$ROOT/$file.pmd-r19-$STAMP.tmp"
