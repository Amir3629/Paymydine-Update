...
: "${SHA:?Set SHA to the exact Square R5 branch commit before running this script.}"
...
BACKUP="$HOME/pmd-square-r5-$STAMP"
mkdir -p "$BACKUP"
...
rollback() {
  ...
  if [ "$ROLLED_BACK" -eq 0 ]; then
    ...
    echo
    echo "=========================================="
    echo "SQUARE R5 FAILED - RESTORING SOURCE"
    echo "=========================================="
    for rel in "${PATCHED_FILES[@]}" "${DOWNLOAD_FILES[@]}"; do
      dst="$ROOT/$rel"
      saved="$BACKUP/$rel"
...
echo "=========================================="
echo "APPLY SQUARE R5 PATCH CHAIN"
echo "=========================================="
sudo python3 "$ROOT/scripts/pmd-square-ui-runtime-r5.py"

echo "=========================================="
echo "VERIFY R5 REQUIRED MARKERS"
echo "=========================================="
grep -q "SquareDirectMethodButton" "$V2/src/runtime/components/RuntimeOverlays.tsx"
grep -q "isSquareSingleAction ? null" "$V2/src/runtime/components/RuntimeOverlays.tsx"
grep -q "selectedProvider === 'square' && selectedCode === 'card'" "$V2/src/runtime/components/RuntimeOverlays.tsx"
grep -q "request->query('currency', '')" "$ROOT/routes/square-runtime.php"
grep -q "configuredCurrency" "$ROOT/app/Services/Payments/SquareRuntimeService.php"
grep -q "squareCurrency !== '' &&.*restaurantCurrency !== ''" "$ROOT/app/admin/controllers/Payments.php"
grep -q "payments.card({ style: PMD_SQUARE_CARD_STYLE })" "$V2/src/runtime/components/SquareInlinePayment.tsx"
grep -q "const tokenResultPromise = wallet.tokenize()" "$V2/src/runtime/components/SquareDirectMethodButton.tsx"
...
echo "=========================================="
echo "ALL SQUARE R5 VALIDATION PASSED"
echo "=========================================="
