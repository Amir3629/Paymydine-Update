...
class DetectTenant
{
    ...
    public function handle(Request $request, Closure $next)
    {
        ...
        if ($subdomain && $subdomain !== 'www') {
            ...
            // No subdomain detected - check if we're in local development mode
            if (env('APP_ENV') === 'local' || env('APP_DEBUG') === true) {
                // Use default tenant for local development (first active tenant)
                try {
                    $tenant = DB::connection('mysql')->table('tenants')
                        ->where('status', 'active')
                        ->orderBy('id', 'asc')
                    ...
                    if ($tenant && $tenant->database) {
                        ...
                        Log::info("LOCAL DEV MODE: Using default tenant database: {$tenant->database}");
                        
                        return $next($request);
                    }
                } catch (\Exception $e) {
                    Log::error("Error loading default tenant: " . $e->getMessage());
                }
            }
