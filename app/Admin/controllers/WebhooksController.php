    ...
    public function stripe(Request $request)
    {
        ...
        try {
            ...
            if ($eventType === 'payment_intent.succeeded') {
                ...
                if ($paymentIntentId) {
                    $order = Orders_model::where('stripe_payment_intent_id', $paymentIntentId)->first();
                    ...
                    if ($order) {
                        try {
                            (new \Admin\Services\Fiskaly\FiskalySignDeService())->finalizeOrder(
                                (int)($order->order_id ?? $order->id),
                                (int)($order->location_id ?? 1),
                                (string)($order->payment ?? 'card'),
                                (string)$paymentIntentId
                            );
                        } catch (\Throwable $fiskalyError) {
                            Log::error('[StripeWebhook] Fiskaly finalize failed', [
                                'request_id' => $requestId,
                                'payment_intent_id' => $paymentIntentId,
                                'order_id' => $order->order_id ?? $order->id,
                                'message' => $fiskalyError->getMessage(),
                            ]);
                        }
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    Log::error('[PayPalWebhook] Fiskaly finalize failed', [
                        'request_id' => $requestId,
                        'order_id' => $order->order_id ?? ($order->id ?? null),
                        'message' => $fiskalyError->getMessage(),
                    ]);
        ...
        try {
            ...
            try {
                ...
                try {
                    ...
                    Log::error('[SquareWebhook] Fiskaly finalize failed', [
                        'request_id' => $requestId,
                        'order_id' => $order->order_id ?? ($order->id ?? null),
                        'message' => $fiskalyError->getMessage(),
                    ]);
