@php
    ...
    $copy = $locale === 'de'
        ? [
            ...
            'connect' => 'Verbinden',
            'verify' => 'Bestätigen',
            'match_title' => 'Restaurant-Freigabe',
            'match_text' => 'Zeige diese Nummer dem Kassierer oder Owner. Auf deren PMD-Seite erscheint dieselbe Nummer.',
            'match_label' => 'DEIN ANMELDECODE',
            'waiting' => 'Warte auf Freigabe durch Kassierer oder Owner…',
        ...
        : [
            ...
            'connect' => 'Connect',
            'verify' => 'Verify',
            'match_title' => 'Restaurant approval',
            'match_text' => 'Show this number to the Cashier or Owner. Their PMD screen shows the same number.',
            'match_label' => 'YOUR SIGN-IN CODE',
            'waiting' => 'Waiting for Cashier or Owner approval…',
            'expired' => 'This request expired. Sign in again.',
        ];

    $requestCodeDisplay = null;
    if ($securityMode === 'workplace') {
        try {
            $site = app(\App\Services\PmdSiteAccessService::class);
            $challenge = $site->challengeForSession();
            if ($challenge) {
                $rawCode = preg_replace('/\D+/', '', $site->challengeCodeForHub($challenge));
                if (strlen($rawCode) === 6) {
                    $requestCodeDisplay = substr($rawCode, 0, 3).' '.substr($rawCode, 3);
                }
            }
...
<head>
    ...
    <link rel="shortcut icon" href="/app/admin/assets/images/pmd-brand-mark.svg?v=pmd-login-v7">
    <style>
        ...
        *{box-sizing:border-box}html,body{margin:0;min-height:100%}
        ...
        .submit{height:49px;display:flex;align-items:center;justify-content:center;border:1px solid var(--jade);border-radius:13px;background:var(--jade);color:#fff;font:inherit;font-size:14px;font-weight:900;cursor:pointer;text-decoration:none}.submit:hover{background:var(--jade-dark)}
...
<main class="card" @if($securityMode === 'workplace' && !empty($security['expires_at'])) data-pmd-workplace-login data-expires-at="{{ $security['expires_at'] }}" @endif>
    ...
    <div class="brand"><img src="{{ asset('app/admin/assets/images/pmd-login-logo.svg') }}?v=pmd-login-v7" alt="PayMyDine"></div>
    ...
    @if(!$securityActive)
        {!! form_open(['id'=>'edit-form','class'=>'form','role'=>'form','method'=>'POST','data-request'=>'onLogin']) !!}
            <label class="field"><span>{{ $copy['username'] }}</span><input type="text" name="username" id="input-username" autocomplete="username" placeholder="{{ $copy['username_placeholder'] }}" value="{{ old('username') }}" required autofocus>{!! form_error('username', '<small class="error">', '</small>') !!}</label>
            <label class="field"><span>{{ $copy['password'] }}</span><span class="input"><input type="password" name="password" id="input-password" autocomplete="current-password" minlength="6" placeholder="{{ $copy['password_placeholder'] }}" required><button type="button" class="toggle" data-password-toggle aria-label="Show password">◉</button></span>{!! form_error('password', '<small class="error">', '</small>') !!}</label>
    ...
    @elseif($securityMode === 'setup')
        ...
        <div class="form">
            <div class="qrbox" aria-label="Authenticator QR">@if(!empty($security['qr_svg'])){!! $security['qr_svg'] !!}@else<div class="qr-fallback">QR unavailable</div>@endif</div>
    ...
    @else
        <section class="security-head"><h1>{{ $copy['match_title'] }}</h1><p>{{ $copy['match_text'] }}</p></section>
        <div class="match-card">
            <div class="match-label">{{ $copy['match_label'] }}</div>
            <div class="match-code">{{ $requestCodeDisplay ?: '--- ---' }}</div>
        </div>
        <div class="wait" data-pmd-workplace-status>{{ $copy['waiting'] }}</div>
        <div class="privacy-note">This number only identifies this short sign-in request. It is not an Authenticator code and cannot approve itself.</div>
    @endif
</main>
...
(function () {
    ...
    document.querySelectorAll('[data-security-code]').forEach(function (input) {
            ...
            fetch('{{ admin_url('siteaccess/status') }}', {
                ...
                .then(function (payload) {
                    ...
                    if (payload.status === 'declined' || payload.status === 'expired') {
                        ...
                        window.setTimeout(function () { window.location.assign('{{ admin_url('login') }}'); }, 700);
                    }
                }).catch(function () {});
        }

        poll();
        window.setInterval(poll, 1400);
    }
})();
