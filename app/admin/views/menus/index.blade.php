...
<div class="pmd-menu-card-grid-wrap">
    ...
    <div id="pmd-category-buttons" class="category-buttons pmd-category-buttons">
        ...
        @endforeach
        <a href="{{ admin_url('categories/create') }}" class="category-btn pmd-category-add" title="Add category"><i class="fa fa-plus"></i> Category</a>
    </div>
...
.pmd-menu-grid{display:grid;grid-template-columns:repeat(auto-fill,minmax(230px,280px));justify-content:start;gap:14px}@media(max-width:760px){.pmd-menu-grid{grid-template-columns:1fr}}
...
.pmd-menu-img-wrap{width:140px;height:110px;margin:0 auto 8px;display:flex;align-items:center;justify-content:center;background:transparent;border-radius:14px;padding:0}.pmd-menu-img-wrap img{max-width:100%;max-height:100%;object-fit:contain}
...
.pmd-card-menu{position:absolute;right:8px;top:8px;z-index:3}.pmd-card-menu .dropdown-toggle{width:32px;height:32px;border-radius:999px;padding:0;display:inline-flex;align-items:center;justify-content:center}.pmd-select-dot{display:none;position:absolute;left:10px;top:10px;z-index:4}.pmd-select-dot input{display:none}.pmd-select-dot span{width:16px;height:16px;border:2px solid #9ca3af;border-radius:50%;display:block;background:#fff}.pmd-select-dot input:checked+span{background:#3b82f6;border-color:#3b82f6}
...
function syncMode(){const filtered=activeCategory()!=='all'; warning.style.display=filtered?'block':'none'; cards().forEach(c=>{c.draggable=editMode&&!filtered;c.classList.toggle('is-draggable',editMode&&!filtered);c.classList.toggle('select-mode',selectMode); c.querySelector('.pmd-card-menu').style.display=editMode?'none':'';}); if(toggleBtn) toggleBtn.disabled=filtered; saveBtn.style.display=(editMode&&dirty)?'':'none'; saveBtn.disabled=!dirty||!editMode||filtered; if(!dirty) stateEl.textContent=''; countEl.style.display=selectMode?'':'none'; }
