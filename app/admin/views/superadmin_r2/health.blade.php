...
@section('content')
<div class="hero"><div><h2>Tenant health</h2><p>Health and recovery controls for every registered restaurant.</p></div></div>
...
<tr>
    ...
    <td><span class="health-dot {{ $row->db_ok?'ok':'bad' }}"></span> {{ $row->db_ok?'Ready':'Missing' }}<span class="sub">{{ $row->tenant->database }}</span></td>
    ...
    <td><span class="health-dot {{ $row->tls_ok?'ok':'bad' }}"></span> {{ $row->tls_ok?'Valid for tenant':'Missing / wrong certificate' }}@if($row->tls_name)<span class="sub">Served: {{ $row->tls_name }}</span>@endif @if($row->tls_expires)<span class="sub">Expires {{ $row->tls_expires }}</span>@endif</td>
...
@empty<tr><td colspan="7" class="empty">No tenants registered.</td></tr>@endforelse
</tbody></table></div></div>
@endsection