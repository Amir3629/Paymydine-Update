@php
    ...
    $selectedMonth = $selectedMonth ?? now()->startOfMonth();
    $monthKey = $monthKey ?? $selectedMonth->format('Y-m');
    $previousMonthKey = $previousMonthKey ?? $selectedMonth->copy()->subMonth()->format('Y-m');
    $nextMonthKey = $nextMonthKey ?? $selectedMonth->copy()->addMonth()->format('Y-m');
    $currentMonthKey = $currentMonthKey ?? now()->format('Y-m');
    $monthlyReport = $monthlyReport ?? [
        'planned_hours' => 0,
        'worked_hours' => null,
        'scheduled_shifts' => $shifts->count(),
        'scheduled_days' => $shifts->pluck('shift_date')->unique()->count(),
        'recorded_days' => 0,
        'open_sessions' => 0,
    ];
    $attendanceReady = !empty($attendanceReady);
    $today = now()->startOfDay();
    $next = $nextShift ?? $shifts->first(function ($shift) use ($today) {
        return \Carbon\Carbon::parse($shift->shift_date)->endOfDay()->gte($today);
    });
    ...
    $personInitial = strtoupper(mb_substr((string)$person->display_name, 0, 1));
@endphp
<!doctype html>
<html lang="en">
<head>
    ...
    <link rel="stylesheet" href="/app/admin/assets/css/pmd-staff-portal-v3.css?v=1">
</head>
<body class="pmd-staff-portal-page">
<div class="pmd-staff-app" data-pmd-staff-portal data-pmd-mobile-active="chat">
    <header class="pmd-staff-topbar">
        ...
        <div class="pmd-staff-topbar__actions">
            @if(!empty($workspaceRoute) && $roleCode !== \Admin\Services\PmdDefaultStaffRoleService::TEAM_MEMBER)
                <a class="pmd-staff-workspace-link" href="{{ admin_url($workspaceRoute) }}" aria-label="Open workspace">
            ...
            <button type="button" class="pmd-staff-person pmd-staff-person-button" data-pmd-profile-open aria-label="Edit my profile">
                <span class="pmd-staff-person__avatar pmd-staff-avatar">
                    @if(!empty($person->avatar_url))
                        <img src="{{ $person->avatar_url }}" alt="{{ $person->display_name }}" data-pmd-avatar-image>
                        <span data-pmd-avatar-fallback hidden>{{ $personInitial }}</span>
                    @else
    ...
    <div class="pmd-staff-layout">
            ...
            <section class="pmd-staff-groups">
                <header class="pmd-staff-panel-head">
                    <div class="pmd-staff-section-title">
                        <span class="pmd-staff-section-icon"><svg viewBox="0 0 24 24"><path d="M21 15a4 4 0 0 1-4 4H8l-5 3v-15a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4z"></path></svg></span>
                ...
                @if(!empty($chatReady))
                    <div class="pmd-staff-group-list">
                        @foreach($groups as $group)
                            <a href="{{ admin_url('mywork') }}?group={{ (int)$group->id }}&month={{ $monthKey }}#chat" class="{{ $activeGroup && (int)$activeGroup->id === (int)$group->id ? 'is-active' : '' }}">
                    ...
                    <form method="post" action="{{ admin_url('mywork/creategroup') }}" class="pmd-staff-group-create" data-pmd-group-form hidden>
                        ...
                        <div class="pmd-staff-inline-form-head">
                            ...
                            <button type="button" data-pmd-group-close aria-label="Close group form"><svg viewBox="0 0 24 24"><path d="M6 6l12 12M18 6 6 18"></path></svg></button>
                        </div>
                        ...
                        <div class="pmd-staff-group-members">
                            @foreach($teamMembers as $member)
                                @php
                                    $memberInitial = strtoupper(mb_substr((string)$member->display_name, 0, 1));
                                @endphp
                                <label>
                                    ...
                                    <span class="pmd-staff-member-option">
                                        <span class="pmd-staff-mini-avatar pmd-staff-avatar">
                                            @if(!empty($member->avatar_url))
                                                <img src="{{ $member->avatar_url }}" alt="" data-pmd-avatar-image>
                                                <span data-pmd-avatar-fallback hidden>{{ $memberInitial }}</span>
                                            @else
        ...
        <main id="chat" class="pmd-staff-chat" data-pmd-mobile-screen="chat">
            ...
            @if(!empty($chatReady))
                <nav class="pmd-staff-mobile-groups" aria-label="Conversations">
                    @foreach($groups as $group)
                        <a href="{{ admin_url('mywork') }}?group={{ (int)$group->id }}&month={{ $monthKey }}#chat" class="{{ $activeGroup && (int)$activeGroup->id === (int)$group->id ? 'is-active' : '' }}">
                            <b>{{ strtoupper(mb_substr((string)$group->name, 0, 1)) }}</b>
            ...
            <div class="pmd-staff-chat__messages">
                @forelse($messages as $message)
                    @php
                        ...
                        $senderInitial = strtoupper(mb_substr((string)$senderName, 0, 1));
                    @endphp
                    <article class="{{ $isMe ? 'is-me' : '' }}">
                        <span class="pmd-staff-message-avatar pmd-staff-avatar">
                            @if($senderAvatar)
                                <img src="{{ $senderAvatar }}" alt="" data-pmd-avatar-image>
                                <span data-pmd-avatar-fallback hidden>{{ $senderInitial }}</span>
                            @else
        ...
        <aside class="pmd-staff-tools">
            <section id="schedule" class="pmd-staff-tool-card" data-pmd-mobile-screen="schedule">
                <header class="pmd-staff-panel-head">
                    <div class="pmd-staff-section-title">
                        <span class="pmd-staff-section-icon is-blue"><svg viewBox="0 0 24 24"><path d="M4 5h16v15H4zM8 3v4M16 3v4M4 10h16"></path></svg></span>
                        <div><small>Personal rota</small><h2>My schedule</h2></div>
                    </div>
                    <b class="pmd-staff-count">{{ $shifts->count() }}</b>
                </header>

                <div class="pmd-staff-month-nav">
                    <a href="{{ admin_url('mywork') }}?month={{ $previousMonthKey }}#schedule" aria-label="Previous month"><svg viewBox="0 0 24 24"><path d="m15 18-6-6 6-6"></path></svg></a>
                    <div class="pmd-staff-month-title">
                        <strong>{{ $selectedMonth->format('F Y') }}</strong>
                        <small>Only your assigned shifts</small>
                    </div>
                    <a href="{{ admin_url('mywork') }}?month={{ $nextMonthKey }}#schedule" aria-label="Next month"><svg viewBox="0 0 24 24"><path d="m9 18 6-6-6-6"></path></svg></a>
                </div>
                @if($monthKey !== $currentMonthKey)
                    <div class="pmd-staff-month-today"><a href="{{ admin_url('mywork') }}?month={{ $currentMonthKey }}#schedule">Back to this month</a></div>
                @endif

                <div class="pmd-staff-month-report">
                    <div class="pmd-staff-month-stat"><small>Planned</small><strong>{{ number_format((float)$monthlyReport['planned_hours'], 1) }}h</strong></div>
                    <div class="pmd-staff-month-stat is-worked"><small>Worked</small><strong>{{ $attendanceReady ? number_format((float)$monthlyReport['worked_hours'], 1).'h' : '—' }}</strong></div>
                    <div class="pmd-staff-month-stat"><small>Shifts</small><strong>{{ (int)$monthlyReport['scheduled_shifts'] }}</strong></div>
                    <div class="pmd-staff-month-stat"><small>Work days</small><strong>{{ $attendanceReady ? (int)$monthlyReport['recorded_days'] : (int)$monthlyReport['scheduled_days'] }}</strong></div>
                </div>

                @if($attendanceReady)
                    <p class="pmd-staff-month-report-note is-live"><svg viewBox="0 0 24 24"><path d="M5 12l4 4L19 6"></path></svg><span>Worked hours come from your recorded check-in/check-out data. {{ (int)$monthlyReport['open_sessions'] > 0 ? (int)$monthlyReport['open_sessions'].' open attendance session(s) are not counted yet.' : '' }}</span></p>
                @else
                    <p class="pmd-staff-month-report-note"><svg viewBox="0 0 24 24"><circle cx="12" cy="12" r="9"></circle><path d="M12 8v5M12 17h.01"></path></svg><span>Time-clock data is not available yet, so only planned schedule hours are shown.</span></p>
                @endif

                <div class="pmd-staff-month-shifts">
                    @forelse($shifts as $shift)
                        @php
                            $rule = $workRuleWarnings[(int)$shift->id] ?? null;
                            $shiftDate = \Carbon\Carbon::parse($shift->shift_date);
                            $isFutureShift = $shiftDate->endOfDay()->gte($today);
                            ...
                            $attendanceStatus = strtolower(trim((string)($shift->attendance_status ?? 'planned'))) ?: 'planned';
                        @endphp
                        <article>
                            <time><strong>{{ $shiftDate->format('d') }}</strong><small>{{ $shiftDate->format('D') }}</small></time>
                            <div>
                                <strong>{{ $shiftLabel }}</strong>
                                <span>{{ $shiftTime }}@if(isset($shift->break_minutes) && (int)$shift->break_minutes > 0) · {{ (int)$shift->break_minutes }}m break @endif @if(isset($shift->planned_hours) && (float)$shift->planned_hours > 0) · {{ number_format((float)$shift->planned_hours, 1) }}h @endif</span>
                                <small class="pmd-staff-shift-state is-{{ $attendanceStatus }}">{{ $attendanceStatus }}</small>
                                @if($rule && !empty($rule['warnings']))
                                    <small class="pmd-staff-rule-warning">{{ $rule['warnings'][0]['message'] }}</small>
                                @endif
                            </div>
                            @if(!empty($requestsReady) && $isFutureShift)
                                <button type="button" data-pmd-request-shift="{{ (int)$shift->id }}" data-pmd-request-shift-label="{{ $shiftLabel }}" data-pmd-request-shift-date="{{ $shiftDate->format('D, d M') }}" data-pmd-request-shift-time="{{ $shiftTime }}">Change</button>
                            @endif
                        </article>
                    @empty
                        <div class="pmd-staff-empty is-month"><strong>No shifts assigned for {{ $selectedMonth->format('F Y') }}</strong><span>Use the arrows to check previous or future months.</span></div>
                    @endforelse
                </div>
            </section>

            @if($openShifts->isNotEmpty())
                <section id="open-shifts" class="pmd-staff-tool-card" data-pmd-mobile-screen="schedule">
                    <header class="pmd-staff-panel-head">
                        <div class="pmd-staff-section-title">
                        ...
                        @foreach($openShifts as $shift)
                            <article>
                                <div>
                                    ...
                                    <small>{{ $shift->starts_at ? substr((string)$shift->starts_at, 0, 5) : 'All day' }}@if($shift->ends_at) – {{ substr((string)$shift->ends_at, 0, 5) }} @endif · {{ $shift->label ?: 'Shift' }}</small>
                                </div>
            ...
            <section id="requests" class="pmd-staff-tool-card" data-pmd-mobile-screen="requests">
                <header class="pmd-staff-panel-head">
                    <div class="pmd-staff-section-title">
                    ...
                    <form method="post" action="{{ admin_url('mywork/saverequest') }}" class="pmd-staff-request-form" data-pmd-staff-request-form>
                        ...
                        <div class="pmd-staff-selected-shift" data-pmd-selected-shift {{ $selectedShift ? '' : 'hidden' }}>
                            ...
                            <div>
                                ...
                                <strong data-pmd-selected-shift-title>{{ $selectedShift ? ($selectedShift->label ?: 'Shift') : 'Choose a shift' }}</strong>
                                <span data-pmd-selected-shift-meta>@if($selectedShift){{ \Carbon\Carbon::parse($selectedShift->shift_date)->format('D, d M') }} · {{ $selectedShift->starts_at ? substr((string)$selectedShift->starts_at, 0, 5) : 'All day' }}@if($selectedShift->ends_at) – {{ substr((string)$selectedShift->ends_at, 0, 5) }}@endif @endif</span>
                            </div>
                        ...
                        <label class="pmd-staff-request-note"><span>Message</span><textarea name="message" rows="3" maxlength="2000" required placeholder="Add a short note for management…" data-pmd-request-message>{{ old('message') }}</textarea></label>
                    ...
                    <div class="pmd-staff-request-history">
                        @forelse($requests->whereIn('request_type', ['shift_change','time_off','sick','cover_shift'])->take(10) as $item)
                            <article>
                                ...
                                @endif
                                <p>{{ $item->message }}</p>
                                @if(!empty($item->manager_reply))<div class="pmd-staff-manager-reply"><b>Management</b><span>{{ $item->manager_reply }}</span></div>@endif
                            </article>
                        @empty
            ...
            @if($canManage)
                <section id="management" class="pmd-staff-tool-card is-management" data-pmd-mobile-screen="manage">
                    <header class="pmd-staff-panel-head">
                        <div class="pmd-staff-section-title">
                        ...
                        @forelse($managementRequests as $item)
                            <article>
                                <div>
                                    ...
                                    @if(!empty($item->request_shift_date))<span class="pmd-staff-management-shift">{{ \Carbon\Carbon::parse($item->request_shift_date)->format('D, d M') }} · {{ $item->request_shift_label ?: 'Shift' }}</span>@endif
                                    <p>{{ $item->message }}</p>
                                </div>
                                <form method="post" action="{{ admin_url('mywork/handlerequest') }}">
                                    ...
                                    <div><button name="decision" value="approved" type="submit">Approve</button><button name="decision" value="declined" type="submit" class="is-decline">Decline</button></div>
                                </form>
                            </article>
                        @empty
    ...
    <nav class="pmd-staff-mobile-nav" aria-label="Staff Portal">
        <button type="button" data-pmd-mobile-target="chat" class="is-active"><svg viewBox="0 0 24 24"><path d="M21 15a4 4 0 0 1-4 4H8l-5 3v-15a4 4 0 0 1 4-4h10a4 4 0 0 1 4 4z"></path></svg><span>Chat</span></button>
        <button type="button" data-pmd-mobile-target="schedule"><svg viewBox="0 0 24 24"><path d="M4 5h16v15H4zM8 3v4M16 3v4M4 10h16"></path></svg><span>Schedule</span></button>
        <button type="button" data-pmd-mobile-target="requests"><svg viewBox="0 0 24 24"><path d="M5 4h14v16H5zM8 8h8M8 12h8M8 16h5"></path></svg><span>Requests</span>@if($pending)<b>{{ $pending }}</b>@endif</button>
        <button type="button" data-pmd-profile-open><svg viewBox="0 0 24 24"><circle cx="12" cy="8" r="4"></circle><path d="M4 21a8 8 0 0 1 16 0"></path></svg><span>Profile</span></button>
        @if($canManage)
            <button type="button" data-pmd-mobile-target="manage"><svg viewBox="0 0 24 24"><circle cx="9" cy="8" r="3"></circle><path d="M3 20a6 6 0 0 1 12 0M16 5a3 3 0 0 1 0 6M17 14a5 5 0 0 1 4 5"></path></svg><span>Manage</span>@if($managementRequests->count())<b>{{ $managementRequests->count() }}</b>@endif</button>
        @endif
    </nav>
    ...
    <div class="pmd-staff-modal" data-pmd-profile-modal hidden>
        ...
        <section id="profile" class="pmd-staff-profile-card" role="dialog" aria-modal="true" aria-labelledby="pmd-profile-title">
            ...
            <form method="post" action="{{ admin_url('mywork/updateprofile') }}" enctype="multipart/form-data" class="pmd-staff-profile-form">
                ...
                <div class="pmd-staff-profile-photo-row">
                    ...
                    <div>
                        ...
                        @if(!empty($avatarReady))
                            <label class="pmd-staff-upload-button"><input type="file" name="avatar" accept="image/jpeg,image/png,image/webp" data-pmd-avatar-input><svg viewBox="0 0 24 24"><path d="M12 16V4M7 9l5-5 5 5M5 20h14"></path></svg><span>Choose photo</span></label>
                        @else
                ...
                <label class="pmd-staff-profile-field"><span>Display name</span><input type="text" name="display_name" maxlength="128" value="{{ old('display_name', $person->display_name) }}" required><small>This name is used in Team, chat and your shift profile.</small></label>
                <div class="pmd-staff-profile-readonly"><div><small>Job role</small><strong>{{ $person->job_role ?: 'Team member' }}</strong></div><div><small>Department</small><strong>{{ ucfirst((string)($person->department ?: 'other')) }}</strong></div></div>
                @if(!empty($person->avatar_url))<label class="pmd-staff-remove-photo"><input type="checkbox" name="remove_avatar" value="1"><span>Remove my current photo</span></label>@endif
                ...
                <footer><button type="button" class="pmd-staff-secondary" data-pmd-profile-close>Cancel</button><button type="submit" class="pmd-staff-primary">Save profile</button></footer>
            </form>
        </section>
    </div>
</div>
...
(function () {
    ...
    function setMobileScreen(name) {
        if (!name) name = 'chat';
        root.setAttribute('data-pmd-mobile-active', name);
        root.querySelectorAll('[data-pmd-mobile-target]').forEach(function (button) {
            button.classList.toggle('is-active', button.getAttribute('data-pmd-mobile-target') === name);
        });
    }

    function screenFromHash() {
        var hash = window.location.hash || '';
        if (hash === '#schedule' || hash === '#open-shifts') return 'schedule';
        if (hash === '#requests') return 'requests';
        if (hash === '#management') return 'manage';
        return 'chat';
    }

    root.querySelectorAll('[data-pmd-mobile-target]').forEach(function (button) {
        button.addEventListener('click', function () {
            var target = button.getAttribute('data-pmd-mobile-target') || 'chat';
            setMobileScreen(target);
            var targetId = target === 'manage' ? 'management' : target;
            if (target === 'chat') targetId = 'chat';
            var section = document.getElementById(targetId);
            if (section) section.scrollIntoView({behavior: 'smooth', block: 'start'});
        });
    });

    setMobileScreen(screenFromHash());
    ...
    if (requestForm) {
        ...
        function setType(value) {
            ...
            if (value === 'time_off') {
                ...
                help.textContent = shiftInput.value ? 'Tell management what should change about this shift.' : 'Open Schedule, choose one of your future shifts, then describe the change you need.';
        ...
        function chooseShift(button) {
            ...
            setMobileScreen('requests');
            requestForm.scrollIntoView({behavior: 'smooth', block: 'start'});
            window.setTimeout(function () { message.focus(); }, 300);
        }

        root.querySelectorAll('[data-pmd-request-type-button]').forEach(function (button) {
            button.addEventListener('click', function () { setType(button.getAttribute('data-pmd-request-type-button')); });
        });
        ...
        requestForm.addEventListener('submit', function (event) {
            if (typeInput.value === 'shift_change' && !shiftInput.value) {
                ...
                selected.hidden = false;
                selectedTitle.textContent = 'Choose a future shift';
                selectedMeta.textContent = 'Open Schedule and tap Change on the shift you want to adjust.';
                selected.classList.add('is-warning');
                setMobileScreen('schedule');
