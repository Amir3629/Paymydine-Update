@php
    // PMD_RESERVATIONS_EXACT_RESERVATIONS_V2_5
    ...
    // the data/save authority; only the proven Reservations visual contract
    // is reused.
    $schedule = $pmdReservationsSchedule ?? [];
    ...
    $pmdReservationsStrings = $locale === 'de'
        ? [
    ...
    if ($locale === 'tr') {
        $pmdReservationsStrings = \Admin\Classes\PmdPlatformI18n::translateStructure(
            $pmdReservationsStrings,
            '',
            'tr'
        );
    }
    ...
    $serverStrings = is_array($schedule['strings'] ?? null)
        ? $schedule['strings']
        : [];
    $strings = array_replace($serverStrings, $pmdReservationsStrings);
    ...
    /* PMD_RESERVATIONS_BERLIN_BOOKING_CLOCK_V1
       Keep past-slot decisions independent of the browser's local timezone. */
    $pmdReservationsBerlinNow = \Carbon\Carbon::now('Europe/Berlin');
    $schedule['today'] = (string)($schedule['today'] ?? $pmdReservationsBerlinNow->format('Y-m-d'));
    $schedule['server_now_berlin'] = $pmdReservationsBerlinNow->format('Y-m-d\TH:i:sP');

    /* PMD_RESERVATIONS_SHARED_OPENING_HOURS_AUTHORITY_V1
    ...
    $pmdReservationsOpeningHours = [];
    $pmdReservationsLocationId = (int)($schedule['location_id'] ?? 0);

    if ($pmdReservationsLocationId < 1) {
        try {
            $pmdCurrentLocation = \Admin\Facades\AdminLocation::current();
            if ($pmdCurrentLocation && (int)$pmdCurrentLocation->location_id > 0) {
                $pmdReservationsLocationId = (int)$pmdCurrentLocation->location_id;
            }
    ...
    if ($pmdReservationsLocationId < 1) {
        try {
            $pmdReservationsLocationId = (int)\Admin\Facades\AdminLocation::getSession('id');
        } catch (\Throwable $pmdOpeningHoursSessionError) {
        }
    }

    if ($pmdReservationsLocationId > 0) {
        try {
            $pmdOpeningHourRows = \Illuminate\Support\Facades\DB::table('working_hours')
                ->where('location_id', $pmdReservationsLocationId)
                ->where('type', 'opening')
                ->orderBy('weekday')
                ->get();
            ...
            foreach ($pmdOpeningHourRows as $pmdOpeningHourRow) {
                ...
                $pmdReservationsOpeningHours[$pmdWeekday] = [
                    'weekday' => $pmdWeekday,
    ...
    ksort($pmdReservationsOpeningHours);
    $schedule['opening_hours'] = array_values($pmdReservationsOpeningHours);
...
{{-- PMD_RESERVATIONS_EXACT_R2_COMPOSER_RUNTIME_V2
...
<script
    ...
    id="pmd-reservations-schedule-bootstrap-v1"
