...
(function() {
    ...
    function removeMediaItem(mediaFinder) {
        ...
        if (findValue) {
            ...
            findValue.value = '';
            // Keep the name attribute so empty value will be submitted and clear the field
            // Don't remove name - we want to submit empty value to clear it
        }
        ...
        if (fieldName && fieldName.indexOf('dashboard_logo') !== -1) {
        ...
        var grid = mediaFinder.querySelector('.grid');
        if (grid) {
            // Create new blank button HTML
            var blankButton = '<a class="find-button blank-cover"><i class="fa fa-plus"></i></a>';
            
            // Preserve the hidden inputs (they should exist but ensure they do)
            var hiddenInputs = '';
            if (findValue) {
                // Keep the hidden input with empty value but preserve the name
                hiddenInputs += '<input type="hidden" data-find-value name="' + fieldName + '" value="">';
            }
            var hiddenIdInput = '';
            if (findId) {
                hiddenIdInput = '<input type="hidden" data-find-identifier value="">';
            }
            
            // Replace grid content but preserve hidden inputs
            grid.innerHTML = blankButton + hiddenInputs + hiddenIdInput;
            
            // CRITICAL: Re-initialize the plus button handler after creating new button
            setTimeout(function() {
                var newButton = grid.querySelector('.find-button, .blank-cover');
                if (newButton) {
                    // Remove any existing handler flag to allow re-initialization
                    delete newButton.dataset.plusHandler;
                    delete newButton.dataset.plusHandlerInitialized;
                    // Re-initialize plus button handlers
                    initPlusButtons();
                }
                
                // Also ensure MediaFinder plugin is re-initialized if using jQuery version
                if (typeof jQuery !== 'undefined' && jQuery.fn.mediaFinder) {
                    var $mediaFinder = jQuery(mediaFinder);
                    if ($mediaFinder.data('ti.mediaFinder')) {
                        // Dispose old instance
                        $mediaFinder.mediaFinder('dispose');
                    }
                    // Re-initialize
