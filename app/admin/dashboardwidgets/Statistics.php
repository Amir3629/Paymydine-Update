...
class Statistics extends BaseDashboardWidget
{
    ...
    public function initialize()
    {
        ...
        // Auto-set default card type for individual stat circle widgets if not already set
        $widgetCode = array_get($this->config, 'widget', '');
        
        // Check if card property exists in config, if not set defaults
        $cardValue = array_get($this->config, 'card', null);
        
        // Set default card only if not already configured by user
        if (!$cardValue) {
            if ($widgetCode === 'stat_circle_1') {
                $this->setProperty('card', 'sale');
            } elseif ($widgetCode === 'stat_circle_2') {
                $this->setProperty('card', 'cash_payment');
            } elseif ($widgetCode === 'stat_circle_3') {
                $this->setProperty('card', 'order');
            }
        }
    }
    ...
    protected function getDefaultCards(): array
    {
        return [
            'sale' => [
                ...
                'icon' => 'stat-icon-brown-light text-white fa fa-bar-chart',
            ],
            'lost_sale' => [
            ...
            'customer' => [
                ...
                'icon' => 'stat-icon-brown-light text-white fa fa-users',
            ],
            'order' => [
            ...
            'collection_order' => [
                ...
                'icon' => 'stat-icon-brown-light text-white fa fa-shopping-bag',
            ],
            'completed_order' => [
            ...
            'reserved_guest' => [
                ...
                'icon' => 'stat-icon-brown-light text-white fa fa-user-friends',
            ],
            'reservation' => [
