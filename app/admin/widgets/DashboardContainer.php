...
class DashboardContainer extends BaseWidget
{
    ...
    public function onLoadAddPopup()
    {
        ...
        $allWidgets = collect(Widgets::instance()->listDashboardWidgets())->pluck('label', 'code');
        
        // Get currently added widgets
        $addedWidgets = collect($this->getWidgetsFromUserPreferences());
        
        // Extract widget codes from aliases (format: widgetCode_randomString)
        $addedWidgetCodes = $addedWidgets->map(function ($widget, $alias) {
            // Extract widget code from alias (before the underscore and random string)
            // Example: "statistics_abc123" -> "statistics"
            return $widget['widget'] ?? preg_replace('/_[^_]+$/', '', $alias);
        })->unique()->values();
        
        // Filter out already added widgets - only show widgets that haven't been added
        $availableWidgets = $allWidgets->filter(function ($label, $code) use ($addedWidgetCodes) {
            return !$addedWidgetCodes->contains($code);
        });
        
        $this->vars['widgets'] = $availableWidgets;
    ...
    public function onSetAsDefault()
    {
        ...
        // Get the current widget order from the request (sent from frontend)
        $aliases = post('aliases', []);
        
        if (empty($aliases)) {
            // Fallback: get from user preferences if no aliases sent
            $widgets = $this->getWidgetsFromUserPreferences();
        } else {
            // Get existing widget configurations and reorder them based on current DOM order
            $existingWidgets = $this->getWidgetsFromUserPreferences();
            $widgets = [];
            
            foreach ($aliases as $priority => $alias) {
                if (isset($existingWidgets[$alias])) {
                    $widgets[$alias] = $existingWidgets[$alias];
                    $widgets[$alias]['priority'] = $priority;
                }
            }
        }
        
        // If empty, don't save empty default
        if (empty($widgets)) {
            flash()->warning('No widgets to save as default. Add widgets first.');
            return;
        }

        // Save to system parameters AND persist to database
        params()->set($this->getSystemParametersKey(), $widgets);
        params()->save(); // 🔥 Must save to database!
        
        // Also save to user preferences so it persists for current user
    ...
    public function onDuplicateWidget()
    {
        if (!$this->canManage) {
            throw new ApplicationException(lang('admin::lang.alert_access_denied'));
        }
        
        $sourceAlias = post('alias');
        
        // Get the source widget configuration
        $widgets = $this->getWidgetsFromUserPreferences();
        
        if (!isset($widgets[$sourceAlias])) {
            throw new ApplicationException('Source widget not found');
        }
        
        $sourceWidget = $widgets[$sourceAlias];
        $widgetCode = $sourceWidget['widget'] ?? $sourceWidget['class'] ?? null;
        
        if (!$widgetCode) {
            throw new ApplicationException('Invalid widget configuration');
        }
        ...
        $widget = $this->makeWidget($widgetClass, array_merge(['widget' => $widgetCode], $sourceWidget));
        ...
        // Generate new alias
        $newAlias = $widgetCode.'_'.str_random(5);
        
        // Get next priority
        $nextPriority = collect($widgets)->max('priority') + 1;
        
        // Copy all properties from source widget
        $newWidgetConfig = $sourceWidget;
        $newWidgetConfig['priority'] = $nextPriority;
        
        // Add to user preferences
        $widgets[$newAlias] = $newWidgetConfig;
        $this->setWidgetsToUserPreferences($widgets);
        
        // Set properties on widget for rendering
        foreach ($newWidgetConfig as $key => $value) {
            if (!in_array($key, ['widget', 'class', 'priority'])) {
                $widget->setProperty($key, $value);
            }
        }
        $widget->setProperty('priority', $nextPriority);
        
        // Return the new widget HTML
        return [
            '@#'.$this->getId('container-list') => $this->makePartial('widget_item', [
                'widget' => $widget,
                'widgetAlias' => $newAlias,
            ]),
        ];
    }
    ...
    protected function makeDashboardWidget($alias, $widgetConfig)
    {
        ...
        if (!$widgetClass) {
            logger()->error("Failed to resolve widget: $widgetCode for alias: $alias");
            return null;
        }

        return rescue(function () use ($widgetClass, $widgetConfig, $alias, $widgetCode) {
            try {
                $widget = $this->makeWidget($widgetClass, $widgetConfig);
                $widget->bindToController();
                return $widget;
            } catch (\Exception $e) {
                logger()->error("Failed to create widget $widgetCode ($alias): " . $e->getMessage());
                throw $e;
            }
        });
    }
