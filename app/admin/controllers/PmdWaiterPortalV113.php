...
class PmdWaiterPortalV113 extends \Admin\Classes\AdminController
{
    ...
    {
        foreach ($names as $name) if ($this->hasTable($name)) return $name;
        return null;
    }
    ...
    private function primaryKey($table, $fallbacks = array())
    {
        ...
        foreach ($cols as $name => $info) {
            if (isset($info->Extra) && stripos($info->Extra, 'auto_increment') !== false) return $name;
        }

        foreach ($fallbacks as $f) if (isset($cols[$f])) return $f;
        foreach (array('id', $table.'_id', rtrim($table, 's').'_id') as $f) if (isset($cols[$f])) return $f;
        return null;
    }
    ...
    private function tableRows($d)
    {
        ...
        $pk = $this->primaryKey($t, array('table_id','id'));
    ...
    private function menuRows($d)
    {
        ...
        $pk = $this->primaryKey($t, array('menu_id','id','item_id','product_id'));
    ...
    private function activeOrderRows($d)
    {
        $t = $d['orders'];
        $cols = $d['orders_cols'];
        if (!$t) return array();

        $pk = $this->primaryKey($t, array('order_id','id'));
        $created = $this->col($cols, array('created_at','date_added','order_date','created_on'));
        $q = DB::table($t);
        if ($created) $q->orderBy($created, 'desc');
        elseif ($pk) $q->orderBy($pk, 'desc');
        try { return $q->limit(500)->get(); } catch (Exception $e) { return array(); }
    }
    ...
    private function decorateTables($d, $tables)
    {
        ...
        $ordersPk = $ordersT ? $this->primaryKey($ordersT, array('order_id','id')) : null;
        ...
        $byId = array();
        foreach ($tables as $i => $t) $byId[(string)$t['table_id']] = $i;
        ...
        $activeOrders = array();
        foreach ($this->activeOrderRows($d) as $o) {
            $tid = null;
            if ($oTableId && isset($o->$oTableId)) $tid = (string)$o->$oTableId;
            elseif ($oTableNo && isset($o->$oTableNo)) {
                foreach ($tables as $i => $t) if ((string)$t['number'] === (string)$o->$oTableNo || (string)$t['label'] === (string)$o->$oTableNo) { $tid = (string)$t['table_id']; break; }
            }
            ...
            $closed = false;
            foreach (array('complete','completed','served','closed','cancel','cancelled','canceled','paid') as $word) {
                if ($status !== '' && strpos($status, $word) !== false) $closed = true;
            }
            if ($paid === '1' || $paid === 'paid' || $paid === 'yes' || $paid === 'true') {
                if ($status === '' || strpos($status, 'open') === false) $closed = true;
            }
            if ($closed) continue;
        ...
        if ($itemT && count($activeOrders)) {
            $itemOrder = $this->col($itemCols, array('order_id','orderID','orders_id','order_ref'));
            $itemStatus = $this->col($itemCols, array('status','order_status','item_status','kitchen_status','food_status'));
            if ($itemOrder) {
                try {
                    foreach (DB::table($itemT)->whereIn($itemOrder, array_keys($activeOrders))->limit(1000)->get() as $it) {
            ...
            try {
                ...
                foreach ($q->limit(100)->get() as $m) {
                    foreach (array($pCol, $sCol) as $c) {
                        if ($c && isset($m->$c) && isset($byId[(string)$m->$c])) {
                            $tables[$byId[(string)$m->$c]]['floor_status'] = 'merged';
                        }
                    }
                }
            } catch (Exception $e) {}
        ...
        foreach ($tables as $i => $t) {
            ...
            elseif ($t['due'] > 0 && $t['open_order_count'] > 0) $status = 'open';
        ...
        try {
            ...
            return $this->json(array(
                ...
                'detected'=>array(
                    ...
                    'orders_pk'=>$d['orders'] ? $this->primaryKey($d['orders'], array('order_id','id')) : null,
                    'items_pk'=>$d['items'] ? $this->primaryKey($d['items'], array('order_menu_id','id')) : null
                ),
                'tables'=>$tables,
                'menu_items'=>$menus
            ));
        } catch (Exception $e) {
            return $this->json(array('ok'=>false,'version'=>'v113','error'=>$e->getMessage(),'file'=>basename($e->getFile()),'line'=>$e->getLine()), 500);
        }
    }

    private function columnMeta($cols, $name)
    {
    ...
    private function latestOrderId($orders, $pk, $tableCol, $tableValue)
    {
        if (!$pk) return null;
        try {
            $r = DB::table($orders)->where($tableCol, $tableValue)->orderBy($pk, 'desc')->first();
            if ($r && isset($r->$pk)) return $r->$pk;
        } catch (Exception $e) {}
        return null;
    }
    ...
    {
        try {
            if ((string)$this->req('apply','0') !== '1') return $this->json(array('ok'=>false,'version'=>'v113','error'=>'Missing apply=1'), 400);
            ...
            if (!$tablesT || !$menusT || !$ordersT || !$itemsT) return $this->json(array('ok'=>false,'version'=>'v113','error'=>'Missing real table/menu/order/order_items table','detected'=>$d), 500);

            $tCols = $d['tables_cols']; $mCols = $d['menus_cols']; $oCols = $d['orders_cols']; $iCols = $d['items_cols'];
            $tPk = $this->primaryKey($tablesT, array('table_id','id'));
            $mPk = $this->primaryKey($menusT, array('menu_id','id','item_id','product_id'));
            $oPk = $this->primaryKey($ordersT, array('order_id','id'));
            $iPk = $this->primaryKey($itemsT, array('order_menu_id','order_item_id','id'));

            if (!$tPk || !$mPk) return $this->json(array('ok'=>false,'version'=>'v113','error'=>'Cannot detect table/menu primary key','detected'=>array('table_pk'=>$tPk,'menu_pk'=>$mPk)), 500);
            if (!$oPk) return $this->json(array('ok'=>false,'version'=>'v113','error'=>'Cannot detect orders primary key safely','orders_table'=>$ordersT,'order_columns'=>array_keys($oCols)), 500);

            $table = DB::table($tablesT)->where($tPk, $tableId)->first();
            $menu = DB::table($menusT)->where($mPk, $menuId)->first();
            if (!$table || !$menu) return $this->json(array('ok'=>false,'version'=>'v113','error'=>'Table or menu item not found'), 404);
            ...
            if (!$linkCol) return $this->json(array('ok'=>false,'version'=>'v113','error'=>'Cannot detect orders table link column','order_columns'=>array_keys($oCols)), 500);
            ...
            $orderId = $order && isset($order->$oPk) ? $order->$oPk : null;
            ...
            if (!$orderId) {
                ...
                $orderId = $this->latestOrderId($ordersT, $oPk, $linkCol, $linkVal);
            }

            if (!$orderId) return $this->json(array('ok'=>false,'version'=>'v113','error'=>'Order was created but order id could not be read','orders_table'=>$ordersT,'orders_pk'=>$oPk), 500);

            $iOrder = $this->col($iCols, array('order_id','orders_id','order_ref','orderID'));
            $iMenu = $this->col($iCols, array('menu_id','menu_item_id','item_id','product_id','food_id'));
            ...
            if (!$iOrder) return $this->json(array('ok'=>false,'version'=>'v113','error'=>'Cannot detect order item link column','items_table'=>$itemsT,'item_columns'=>array_keys($iCols)), 500);
            ...
            if ($oTotal) {
                try {
                    DB::table($ordersT)->where($oPk, $orderId)->increment($oTotal, $price * $qty);
                } catch (Exception $e) {}
            }

            $fresh = json_decode(json_encode($this->data()->getData()), true);
            return $this->json(array(
                'ok'=>true,
                'version'=>'v113',
                'message'=>'Item added',
                ...
                'table_id'=>$tableId,
                'item_name'=>$name,
                'data'=>isset($fresh['tables']) ? $fresh : null
            ));
        } catch (Exception $e) {
            return $this->json(array('ok'=>false,'version'=>'v113','error'=>$e->getMessage(),'file'=>basename($e->getFile()),'line'=>$e->getLine()), 500);
        }
    ...
    public function merge()
    {
        try {
            ...
            return $this->json(array('ok'=>true,'version'=>'v113','message'=>'Tables merged'));
        } catch (Exception $e) {
            return $this->json(array('ok'=>false,'version'=>'v113','error'=>$e->getMessage()), 500);
        }
    }
    ...
    public function clearMerges()
    {
        try {
            ...
            return $this->json(array('ok'=>true,'version'=>'v113','disabled_merges'=>$n));
    ...
    public function audit()
    {
        ...
        return $this->json(array(
            ...
            'detected'=>array(
                ...
                'reservations'=>$d['reservations'], 'assign'=>$d['assign'], 'merges'=>$d['merges'],
                'orders_pk'=>$d['orders'] ? $this->primaryKey($d['orders'], array('order_id','id')) : null,
                'items_pk'=>$d['items'] ? $this->primaryKey($d['items'], array('order_menu_id','order_item_id','id')) : null,
