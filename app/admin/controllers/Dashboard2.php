...
class Dashboard2 extends Reservations2
{
    ...
    protected function analyticsPayload(string $requestedPeriod): array
    {
        ...
        $payload['diagnostics'] = $this->analyticsDiagnostics($start, $now, $payload);
        return $payload;
    }
        ...
        if ($this->hasColumns('statuses', ['status_id','status_name']) && Schema::hasColumn('orders', 'status_id')) {
            $query->whereNotExists(function ($excluded) {
                $excluded->selectRaw('1')->from('statuses as analytics_status')
                    ->whereColumn('analytics_status.status_id', 'orders.status_id')
                    ->whereRaw("LOWER(analytics_status.status_name) REGEXP 'cancel|refund|failed|void'");
            });
        }
        return $query;
    }
    ...
    protected function analyticsPaidQuery(Carbon $start, Carbon $end): Builder
    {
        ...
        $payment = $authority['payment'] ? 'orders.`'.$authority['payment'].'`' : "''";
        return $query->selectRaw('orders.*, orders.`'.$authority['date'].'` AS effective_at, orders.`'.$authority['amount'].'` AS effective_amount, '.$payment.' AS effective_payment, GREATEST(COALESCE(orders.`'.$authority['amount'].'`,0) - '.($tips ? 'COALESCE(analytics_tips.tip_amount,0)' : '0').',0) AS net_revenue');
    }
    ...
    protected function analyticsSalesSeries(Carbon $start, Carbon $end, string $period): array
    {
        ...
        return ['available' => true, 'buckets' => $buckets, 'sample_count' => array_sum(array_column($buckets, 'orders')), 'source_mode' => $authority['mode'], 'source' => 'orders.'.$authority['amount'].' grouped by orders.'.$authority['date'].' minus tips'];
    }
    ...
    protected function analyticsSalesByHour(Carbon $start, Carbon $end): array
    {
        ...
        return ['available' => true, 'hours' => $hours, 'sample_count' => array_sum(array_column($hours, 'orders')), 'source_mode' => $authority['mode'], 'source' => 'eligible orders grouped by hour of '.$authority['date']];
    }
    ...
    protected function analyticsTopItems(Carbon $start, Carbon $end): array
    {
        ...
        return ['available' => true, 'items' => $rows->map(fn ($row) => ['name' => (string)$row->name, 'quantity' => (int)$row->quantity, 'revenue' => (float)$row->revenue])->all(), 'sample_count' => $rows->count(), 'source_mode' => $authority['mode'], 'source' => 'eligible orders joined to order_menus'];
    }
    ...
    protected function analyticsCategorySales(Carbon $start, Carbon $end): array
    {
        ...
        return ['available' => true, 'categories' => $rows->map(fn ($r) => ['category' => (string)$r->category, 'revenue' => (float)$r->revenue])->all(), 'sample_count' => $rows->count(), 'source_mode' => $authority['mode'], 'source' => 'eligible order items joined to menu categories'];
    }
    ...
    protected function analyticsPaymentMethods(Carbon $start, Carbon $end): array
    {
        ...
        $query = DB::query()->fromSub($this->analyticsPaidQuery($start, $end), 'paid')
            ->leftJoin('payments as p', 'p.code', '=', 'paid.effective_payment')->groupBy('paid.effective_payment', 'p.name')->orderByDesc('total')
            ->get([DB::raw("COALESCE(p.name, paid.effective_payment, 'Other') AS method"), DB::raw('SUM(paid.effective_amount) AS total'), DB::raw('COUNT(*) AS transactions')]);
        return ['available' => true, 'methods' => $query->map(fn ($r) => ['method' => (string)$r->method, 'total' => (float)$r->total, 'transactions' => (int)$r->transactions])->all(), 'sample_count' => $query->sum('transactions'), 'source_mode' => $authority['mode'], 'source' => 'orders.'.$authority['payment'].' joined to configured payments'];
    }
    ...
    protected function analyticsChannels(Carbon $start, Carbon $end): array
    {
        ...
        $authority = $this->analyticsAuthority($start, $end);
        return ['available' => true, 'channels' => $rows->map(fn ($r) => ['channel' => $r->channel, 'orders' => (int)$r->orders, 'revenue' => (float)$r->revenue])->all(), 'sample_count' => $rows->sum('orders'), 'source_mode' => $authority['mode'], 'source' => 'eligible orders by confirmed order_type mapping'];
    }
    ...
    protected function analyticsLiveOperations(Carbon $now): array
    {
        $query = $this->orders()->where('processed', 0)
            ...
            ->whereRaw("LOWER(COALESCE(s.status_name,'')) NOT REGEXP 'complete|closed|deliver(ed)?|cancel|refund|failed|void'");
    ...
    protected function analyticsTransactions(): array
    {
        ...
        $end = Carbon::now($this->restaurantTimezone()); $authority = $this->analyticsAuthority($start, $end);
        $rows = DB::query()->fromSub($this->analyticsPaidQuery($start, $end), 'paid')->leftJoin('payments as p', 'p.code', '=', 'paid.effective_payment')
            ->orderByDesc('paid.effective_at')->limit(10)->get(['paid.order_id', 'paid.effective_amount', 'paid.effective_at', DB::raw("COALESCE(p.name, paid.effective_payment, 'Other') AS payment_method")]);
        return ['available' => true, 'transactions' => $rows->map(fn ($r) => ['order_id' => (int)$r->order_id, 'method' => (string)$r->payment_method, 'amount' => (float)$r->effective_amount, 'status' => $authority['mode'] === 'settlement_fields' ? 'paid' : 'processed', 'timestamp' => (string)$r->effective_at])->all(), 'sample_count' => $rows->count(), 'source_mode' => $authority['mode'], 'source' => 'latest eligible orders by '.$authority['date']];
    }
    ...
    {
        $tips = $this->tipSubquery();
        if (!$tips) return $this->unavailable('confirmed order_totals tip code unavailable');
        $resolve = function (Carbon $from) use ($end, $tips) {
            $row = DB::query()->fromSub($this->analyticsEligibleOrders($from, $end)->select('orders.order_id'), 'paid')
                ...
                ->selectRaw('COUNT(*) AS samples, COALESCE(SUM(tips.tip_amount),0) AS total')->first();
    ...
    protected function analyticsDiagnostics(Carbon $start, Carbon $end, array $payload): array
    {
        $authority = $this->analyticsAuthority($start, $end);
        ...
        return [
            ...
            'eligible_order_count' => $eligible ? (clone $eligible)->count() : 0,
