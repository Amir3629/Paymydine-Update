...
class Dashboard2 extends Reservations2
{
    ...
    /*
     * PMD_DASHBOARD2_BERLIN_TIMEZONE_V1
     *
     * All Dashboard2 Today/Month periods use the restaurant's
     * required Berlin business timezone, including DST.
     */
    ...
    /*
     * PMD_DASHBOARD2_KPI_PREFIX_ALIAS_FIX_V2
     *
     * Laravel prefixes table aliases generated by Query Builder.
     * Raw SQL must therefore use the same real prefixed aliases.
     */
    protected function sqlAlias(string $alias): string
    {
    ...
    protected function eligiblePaidOrders(Carbon $start, Carbon $end): Builder
    {
        ...
        if ($this->hasColumns('statuses', ['status_id', 'status_name']) && Schema::hasColumn('orders', 'status_id')) {
            ...
            $query->whereNotExists(
                function ($excluded) use ($statusAlias) {
                    $excluded
                        ->selectRaw('1')
                        ->from('statuses as pmd_status')
                        ->whereColumn(
                            'pmd_status.status_id',
                            'orders.status_id'
                        )
                        ->whereRaw(
                            "LOWER({$statusAlias}.`status_name`) ".
                            "REGEXP 'cancel|refund|failed|void'"
                        );
                }
            );
        }
    ...
    protected function revenue(Carbon $start, Carbon $end): array
    {
        ...
        $ordersAlias =
            $this->sqlAlias('orders');

        $tipsAlias =
            $this->sqlAlias('pmd_tips');

        $tipExpression = $tips
            ? "COALESCE({$tipsAlias}.`tip_amount`, 0)"
            : '0';
    ...
    protected function tableTurnoverFromStateHistory(Carbon $start, Carbon $end): ?array
    {
        ...
        $openedAlias =
            $this->sqlAlias('opened');

        $closedAlias =
            $this->sqlAlias('closed');
        ...
        $closures = DB::table('pmd_table_status_history as opened')
            ...
            ->selectRaw(
                "{$openedAlias}.`id`, ".
                "{$openedAlias}.`created_at` AS opened_at, ".
    ...
    protected function kitchenTime(Carbon $start, Carbon $end): array
    {
        ...
        $timestamps = DB::table('status_history as sh')
            ...
            ->selectRaw(
                "{$statusHistoryAlias}.`object_id`, ".
                "MIN(CASE WHEN ".
                "LOWER({$statusAlias}.`status_name`) ".
                "REGEXP 'received|preparation' ".
                "THEN {$statusHistoryAlias}.`created_at` END) ".
                "AS kitchen_at, ".
                "MIN(CASE WHEN ".
                "LOWER({$statusAlias}.`status_name`) ".
                "REGEXP 'ready|served|delivery' ".
                "THEN {$statusHistoryAlias}.`created_at` END) ".
                "AS ready_at"
            );
    ...
    protected function tips(Carbon $start, Carbon $end): array
    {
        ...
        $row = $this
            ...
            ->selectRaw(
                "COUNT(*) AS samples, ".
                "COALESCE(SUM({$tipsAlias}.`tip_amount`), 0) ".
                "AS aggregate"
            )
            ->first();
...
protected function analyticsTransactions(
         ...
         *
         * The Recent Transactions card is a fixed Today card.
         * It must not inherit another Dashboard analytics period.
         */
        $now = Carbon::now(
