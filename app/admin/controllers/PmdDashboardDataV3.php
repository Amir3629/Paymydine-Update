...
class PmdDashboardDataV3 extends \Admin\Classes\AdminController
{
    ...
    protected function build()
    {
        ...
        $todayOrders = $this->orders($orders, $orderTotals, 'today');
        $totalOrders = $this->orders($orders, $orderTotals, 'all');

        $mode = 'today';
        $mainOrders = $todayOrders;

        if ((int)$todayOrders['count'] === 0 && (float)$todayOrders['revenue'] <= 0 && (int)$totalOrders['count'] > 0) {
            $mode = 'all';
            $mainOrders = $totalOrders;
        }

        $payments = $this->pendingPayments($orders, $payTx);
        $reservationsM = $this->reservations($reservations, $mode);
        ...
        $topItems = $this->topItems($orderMenus, $mode);

        $avg = $mainOrders['count'] > 0 ? ($mainOrders['revenue'] / max(1, $mainOrders['count'])) : 0;
        ...
        return [
            ...
            'period' => [
                'mode' => $mode,
                'label' => $mode === 'today' ? 'Today' : 'All time',
                'reason' => $mode === 'today' ? 'Today has live data' : 'Today is empty, showing all-time real totals',
            ],
            'detected_tables' => [
                'orders' => $orders,
            ...
            'metrics' => [
                'revenue_today' => [
                    'label' => $mode === 'today' ? 'REVENUE TODAY' : 'TOTAL SALES',
                    'value' => $this->money($mainOrders['revenue']),
                    'raw' => $mainOrders['revenue'],
                    'source' => $mainOrders['revenue_source'],
                ],
                'pending_payments' => [
                    'label' => 'PENDING PAYMENTS',
                    'value' => $this->money($payments['amount']),
                    'raw' => $payments['amount'],
                ...
                'orders' => [
                    'label' => $mode === 'today' ? 'ORDERS TODAY' : 'TOTAL ORDERS',
                    'value' => (string)$mainOrders['count'],
                    'raw' => $mainOrders['count'],
                    'dine_in' => $mainOrders['dine_in'],
                    'takeaway' => $mainOrders['takeaway'],
                    'delivery' => $mainOrders['delivery'],
                    'source' => $mainOrders['source'],
                ],
                'reservations' => [
                    'label' => $mode === 'today' ? 'RESERVATIONS TODAY' : 'TOTAL RESERVATIONS',
                    'value' => (string)$reservationsM['count'],
                    'raw' => $reservationsM['count'],
                    'upcoming' => $reservationsM['upcoming'],
                    'source' => $reservationsM['source'],
                ],
                'reports' => [
                    'label' => 'AVG TICKET',
    ...
    protected function orders($table, $totals, $mode)
    {
        $out = [
            'count' => 0,
            'revenue' => 0,
        ...
        if ($total) {
            ...
            if ($orderId && $val) {
                $dateWhere = ($mode === 'today' && $date) ? 'DATE(o.'.$this->q($date).') = CURDATE()' : '1=1';
                ...
                $out['revenue'] = (float)$this->scalar(
                    'SELECT COALESCE(SUM(CAST(ot.'.$this->q($val).' AS DECIMAL(15,2))),0) v
                     ...
                     WHERE '.$dateWhere.$codeWhere
                );
                $out['revenue_source'] = $totals.'.'.$val.' / '.$mode;
            }
        }
    ...
    protected function pendingPayments($orders, $tx)
    {
        ...
        if ($tx) {
            ...
            $where = $status
                ? 'LOWER(COALESCE('.$this->q($status).',"")) NOT IN ("paid","completed","complete","succeeded","success","captured","settled","cancelled","canceled","failed")'
                : '1=1';

            $out['count'] = (int)$this->scalar('SELECT COUNT(*) v FROM '.$this->q($tx).' WHERE '.$where);

            if ($amount) {
                $out['amount'] = (float)$this->scalar('SELECT COALESCE(SUM(CAST('.$this->q($amount).' AS DECIMAL(15,2))),0) v FROM '.$this->q($tx).' WHERE '.$where);
            }
        ...
        if ($orders) {
            ...
            $out['count'] = (int)$this->scalar('SELECT COUNT(*) v FROM '.$this->q($orders).' WHERE '.$where);
            if ($amount) {
                $out['amount'] = (float)$this->scalar('SELECT COALESCE(SUM(CAST('.$this->q($amount).' AS DECIMAL(15,2))),0) v FROM '.$this->q($orders).' WHERE '.$where);
            }
    ...
    protected function reservations($table, $mode)
    {
        $out = ['count' => 0, 'upcoming' => 0, 'source' => 'no reservations table'];
        if (!$table) return $out;
    ...
    protected function topItems($table, $mode)
    {
        if (!$table) return [];
        ...
        $where = ($mode === 'today' && $date) ? 'WHERE DATE('.$this->q($date).') = CURDATE()' : '';
