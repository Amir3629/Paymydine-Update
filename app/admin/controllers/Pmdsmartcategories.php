...
class Pmdsmartcategories extends AdminController
{
    ...
    /**
     * PMD_MENU_CATEGORY_REMOVE_PRESERVE_PRODUCTS_V1_6
     *
     * Category removal never deletes food or combo products.
     * Regular categories only lose their menu_categories memberships.
     * Chef/Bestseller additionally clear their existing flag membership.
     * Combination category removal leaves menu_combos untouched.
     */
    public function onDelete(): JsonResponse
    {
        ...
        try {
            DB::transaction(function () use ($categoryId, $kind) {
                ...
                $category = Categories_model::query()->find($categoryId);
                if ($category) {
                    $category->delete();
                }
            });
        ...
        return response()->json([
            ...
            'kind' => $kind,
            'message' => 'Category removed. Foods and combinations were kept.',
        ]);
    }
