...
class PmdDefaultStaffRoleService
{
    ...
    public function ensure(): array
    {
        ...
        foreach ($this->definitions() as $definition) {
            ...
            if (!$role) $role = Staff_roles_model::query()->whereRaw('LOWER(name) = ?', [strtolower($definition['name'])])->first();
    ...
    public function definitions(): array
    {
        ...
        $definitions = [
            ['code'=>self::OWNER,'name'=>'Owner','description'=>'Full PMD access including Owner dashboard, Team, Shifts and Settings.','permissions'=>$ownerPermissions],
            ['code'=>self::MANAGER,'name'=>'Manager','description'=>'Full PMD operational and settings access, except the Owner dashboard.','permissions'=>$managerPermissions],
            ['code'=>self::CASHIER,'name'=>'Cashier','description'=>'Cashier workspace only. No side menu.','permissions'=>[self::PMD_CASHIER_WORKSPACE=>1,'Admin.Dashboard'=>1,'Admin.Orders'=>1,'Admin.Payments'=>1]],
            ['code'=>self::WAITER,'name'=>'Waiter','description'=>'Cashier mobile Quick Mode only. No side menu.','permissions'=>[self::PMD_CASHIER_WORKSPACE=>1,'Admin.Dashboard'=>1,'Admin.Orders'=>1,'Admin.Payments'=>1]],
            ['code'=>self::ACCOUNTANT,'name'=>'Accountant','description'=>'Accountant workspace only. No side menu.','permissions'=>[self::PMD_ACCOUNTANT_WORKSPACE=>1,'Admin.Dashboard'=>1]],
            ['code'=>self::RESERVATIONS,'name'=>'Reservations','description'=>'Reservations workspace only. No side menu.','permissions'=>[self::PMD_RESERVATIONS_WORKSPACE=>1,'Admin.Reservations'=>1]],
            ['code'=>self::TEAM_MEMBER,'name'=>'Team Member','description'=>'Staff Portal only. No operational PMD workspace or side menu.','permissions'=>['Admin.Dashboard'=>1]],
        ];
        ...
        foreach ($this->kdsStations() as $station) {
            $definitions[] = [
                ...
                'permissions'=>[self::PMD_KDS_WORKSPACE=>1,'Admin.KitchenDisplay'=>1],
            ];
        }
        return $definitions;
    }
    ...
    public function isManagedCode(string $code): bool
    {
        ...
        return in_array($code, [self::OWNER,self::MANAGER,self::CASHIER,self::WAITER,self::ACCOUNTANT,self::RESERVATIONS,self::TEAM_MEMBER], true)
    ...
    public function mayOpenPath(string $code, string $path): bool
    {
        ...
        $cleanExact = [
            'admin/ownerdashboard'=>'admin/dashboardlab','admin/managerdashboard'=>'admin/managerlab','admin/accountantdashboard'=>'admin/accountantlab',
            'admin/orders'=>'admin/cashierlab','admin/reservations'=>'admin/reservationslab','admin/menu'=>'admin/pmdmenus','admin/settings'=>'admin/pmdsettings',
            'admin/settings/restaurant'=>'admin/pmdsettings/restaurant','admin/settings/customer-menu'=>'admin/pmdsettings/frontend','admin/settings/menu-checkout'=>'admin/pmdmenu',
            'admin/settings/customers'=>'admin/pmdcustomer','admin/settings/team'=>'admin/pmdteam','admin/settings/devices'=>'admin/pmddevices','admin/settings/finance'=>'admin/pmdfinance',
            'admin/settings/brand'=>'admin/pmdbrand','admin/settings/advanced'=>'admin/pmdadvanced','admin/smartcategories'=>'admin/pmdsmartcategories','admin/reports'=>'admin/pmdreports/sales',
            'admin/reports/tips'=>'admin/pmdreporttips','admin/reports/channels'=>'admin/pmdreportchannels',
        ];
        if (isset($cleanExact[$path])) $path = $cleanExact[$path];
        elseif (str_starts_with($path, 'admin/reports/')) $path = 'admin/pmdreports/'.substr($path, strlen('admin/reports/'));
        elseif (str_starts_with($path, 'admin/settings/devices/')) $path = 'admin/pmddevices/'.substr($path, strlen('admin/settings/devices/'));
        elseif (str_starts_with($path, 'admin/settings/team/')) $path = 'admin/pmdteam/'.substr($path, strlen('admin/settings/team/'));

        if (!$this->isManagedCode($code)) return true;
        if ($code === self::OWNER) return true;
        if (in_array($path, ['admin','admin/dashboard','admin/login','admin/logout'], true)) return true;

        $is = fn(string $route): bool => $path === 'admin/'.$route || str_starts_with($path, 'admin/'.$route.'/');
        if ($is('mywork')) return true;
        if ($code === self::TEAM_MEMBER) return $is('mywork');

        if ($code === self::MANAGER) return !$is('dashboardlab');
        if ($code === self::CASHIER || $code === self::WAITER) return $is('cashierlab') || $is('pmd-waiter-pos-v1') || $is('pmd-waiter-pos-v22');
