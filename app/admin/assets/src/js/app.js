+function ($) {
     ...
     *
     * Canonical source: app/admin/assets/src/js/app.js. The deployed
     * app/admin/assets/js/admin.js copy is kept in sync because the current
     * TastyIgniter/Flame production deploy serves that built file directly.
     *
     * The normalizer only classifies and moves top-toolbar actions. Button
     * colors, sizes, and spacing are CSS-owned by no-green-toolbar-buttons.css
     * so repeated AJAX renders never append duplicate style blocks.
     */
    ...
    function isToolbarBackAction(child) {
        ...
        if (getToolbarActionText(child) === 'back') return true;
    ...
    function hasPrimaryToolbarLabel(child) {
        ...
        return /^(new|save|create|add|update)(\b|\s|$)/.test(text) || /\b(save|create|add|update)\b/.test(text);
    }
    ...
    function shouldSkipToolbarContainer(container) {
        if (!container || container.closest('.modal, .modal-footer, .table, .list-table, .media-manager, .media-toolbar, [data-control="media-manager"]')) return true;
    ...
    function unwrapRightButtonsIfNoSplit(container, rightButtons) {
        if (!rightButtons || rightButtons.parentElement !== container) return;
        Array.prototype.slice.call(rightButtons.children).forEach(function (action) {
            container.insertBefore(action, rightButtons);
        });
    ...
    function shouldSplitToolbar(state, forceConfig) {
        ...
        if (forceConfig) return state.secondaryActions.length > 0 && leftActions > 0;
    ...
    function normalizeToolbar(container, forceConfig) {
        ...
        if (state.rightButtons && !shouldSplitToolbar(state, forceConfig)) {
            unwrapRightButtonsIfNoSplit(container, state.rightButtons);
            state = collectToolbarState(container);
        }
