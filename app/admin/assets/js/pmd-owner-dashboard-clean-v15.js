/* PayMyDine Owner/Admin Dashboard Clean v15
   Reference-style restaurant operations dashboard using the real v12 data model. */
(function () {
  'use strict';

  var VERSION = 'owner-clean-v15-complete-owner-ops-20260626';
  var root = null;
  var lastData = null;
  ...
  function floorPlan(fp) {
    ...
    var nodes = floorTables.map(function (t) {
      ...
      var nodeLabel = t.label || ('Table ' + (t.number || t.id));
      var statusLabel = t.status_label || t.status || 'free';
      var capacity = t.capacity_label || '';
      return '' +
        '<button type="button" data-pmd-table="' + esc(t.id) + '" class="pmd-v15-floor-node pmd-v15-floor-node--' + esc(t.status || 'free') + (isSelected ? ' is-selected' : '') + '" style="left:' + left + '%;top:' + top + '%;width:' + w + '%;height:' + h + '%">' +
          '<span class="pmd-v15-node-title">' + esc(nodeLabel) + '</span>' +
          '<span class="pmd-v15-node-pill">' + esc(statusLabel) + '</span>' +
          '<span class="pmd-v15-node-meta">' + esc(capacity) + '</span>' +
          (t.due_amount > 0 ? '<em>' + esc(t.due_label || fmtMoney(t.due_amount)) + '</em>' : '') +
        '</button>';
    }).join('');
    var side = selected ? tableSide(selected) : '<div class="pmd-v15-floor-empty"><strong>' + esc(floor.label || 'Main Floor') + '</strong><span>Click a table to inspect live status, capacity, reservations and quick links.</span></div>';
    return '' +
  ...
  function kitchenPerformance(data) {
    ...
    return '' +
      '<div class="pmd-v15-kitchen-metrics">' +
      ...
      '</div>' +
      '<div class="pmd-v15-rush"><span>Rush Status</span><b>' + (Number(q.raw || 0) > 8 ? 'High' : (Number(q.raw || 0) > 0 ? 'Medium' : 'Calm')) + '</b></div>';
  }
  ...
  function quickActions() {
    var items = [
      ...
      ['Open Checks', '/admin/orders?status=open', '💳'],
      ['Kitchen', '/admin/kitchendisplay/main-kitchen', '👨‍🍳'],
      ['Tables', '/admin/orders/create', '🍽️'],
      ['Reservations', '/admin/reservations', '📅']
    ];
    return '<div class="pmd-v15-actions">' + items.map(function (i) { return '<a href="' + esc(i[1]) + '"><span>' + esc(i[2]) + '</span><b>' + esc(i[0]) + '</b></a>'; }).join('') + '</div>';
  }
  ...
  function render(data) {
    ...
    root.innerHTML = '' +
      '<div class="pmd-v15-shell">' +
        '<header class="pmd-v15-topbar">' +
          '<div><h1>' + esc(greeting()) + ', ' + esc(findAdminName()) + '! 👋</h1><p>Here\'s what\'s happening at <b>Mimoza / PayMyDine</b></p></div>' +
          '<div class="pmd-v15-top-controls"><span>📅 ' + esc(nowLabel()) + '</span><span>🍽️ Live Restaurant Dashboard</span><a href="/admin/pmd-owner-dashboard-clean-v1-audit" target="_blank" rel="noopener">Data audit</a></div>' +
        '</header>' +

        '<section class="pmd-v15-kpi-grid">' +
        ...
        '<section class="pmd-v15-main-layout">' +
          section('Live Restaurant Floor', 'Large live floor view · tables, open checks and payment status', floorPlan(s.floor_plan), 'pmd-v15-card--floor', '<div class="pmd-v15-card-actions"><button type="button" data-pmd-owner-refresh>Refresh</button><a href="/admin/orders/create">New Order</a></div>') +
          section('Operations Alerts', 'Only owner-relevant issues from connected PMD sources', alerts(data), 'pmd-v15-card--alerts', '<a href="/admin/orders">Open checks</a>') +
          section('Recent Activity', 'Latest recorded orders and reservations from the database', timeline(data), 'pmd-v15-card--timeline', '<a href="/admin/orders">View all</a>') +
        '</section>' +

        '<section class="pmd-v15-analytics-grid">' +
          section('Revenue by Order Trend', 'Connected to real orders; today may be zero', revenueByHour(data), 'pmd-v15-card--chart') +
          section('Open Check Value', 'Current unpaid/open checks; payment transactions are empty right now', paymentBreakdown(data), 'pmd-v15-card--payment') +
          section('Average Check / Risk', 'Financial operations snapshot', smallFinance(data), 'pmd-v15-card--small') +
        ...
        '<section class="pmd-v15-bottom-grid">' +
          section('Team Overview', 'Staff role distribution; server sales will be added after assignments are reliable', staffOverview(staff), 'pmd-v15-card--servers') +
          section('Kitchen Performance', 'Kitchen queue + KDS station health', kitchenPerformance(data), 'pmd-v15-card--kitchen') +
        ...
        '<section class="pmd-v15-bottom-grid pmd-v15-bottom-grid--wide">' +
          section('Recent Live Orders', 'Draft/test rows are excluded', listRows(s.recent_orders, 'order'), 'pmd-v15-card--orders') +
          section('Current Service Mix', 'Dine-in / delivery / cashier distribution from open checks', serviceMix(data), 'pmd-v15-card--mix') +
        '</section>' +
  ...
  function staffOverview(staff) {
    ...
    return '<div class="pmd-v15-staff-total"><strong>' + esc(staff.total || 0) + '</strong><span>Total staff detected</span></div>' + barList(roles, 'label', 'count', 8);
  }
