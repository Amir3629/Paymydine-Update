...
(function () {
  ...
  function pauseAttentionCycleV57(ms) {
    state.attentionPauseUntil = Math.max(
      ...
      Date.now() + Math.max(1000, num(ms, 3500))
    );
  }
  ...
  function focusAttentionTableV57(tableId, immediate) {
    ...
    /* PMD_QPOS_ATTENTION_RAIL_TARGET_V61
     * Scroll the TABLE RAIL itself, not the document. This guarantees that an
     * attention table far down a 40/50+ table list is brought to the center. */
