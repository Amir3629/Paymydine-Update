...
function renderOpenChecks() {
  ...
  function renderCart(options) {
    ...
    if (pay) {
      ...
      /* PMD_QPOS_PAY_READY_V49
       * If an active check is already rendered, a stale/transient switch flag
       * must not keep Pay grey. During a real new-table load activeOrder() is
       * null, so Pay remains safely blocked until the check arrives. */
