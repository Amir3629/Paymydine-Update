...
(function () {
  ...
  function classify(file, decoded, stats) {
    ...
    // Only technical failures block an upload. Quality itself is advisory.
    ...
    if (!decoded.width || !decoded.height) {
      result.state = 'reject'; result.label = 'Not added'; result.reason = 'image could not be read'; return result;
    }
    ...
    var reasons = [];
    if (pixels < MIN_PIXELS) reasons.push('resolution could be better');
    if (stats.contrast < 6) reasons.push('image detail could be better');
    if (stats.mean < 12 || stats.mean > 244 || stats.darkRatio > 0.94 || stats.brightRatio > 0.94) reasons.push('lighting could be better');
    if (stats.edge < 1.05 && stats.contrast < 50) reasons.push('sharpness could be better');
    if (stats.saturation < 0.035 && stats.binaryRatio > 0.78 && stats.edge > 20) reasons.push('a real food photo would look better');

    if (!reasons.length && (pixels < 1500000 || stats.edge < 2.2 || stats.mean < 28 || stats.mean > 230 || stats.contrast < 18)) {
      reasons.push('a clearer photo could look better');
    }

    if (reasons.length) {
      result.state = 'usable';
      result.label = 'Could be better';
  ...
  function analyzeFile(file) {
    ...
    return decodeImage(file).then(function (decoded) {
      var result;
      try {
        result = classify(file, decoded, sampleStats(decoded));
      } catch (error) {
        result = {state:'usable', label:'Could be better', reason:'quality could not be fully checked', file:file, key:fileKey(file)};
      }
  ...
  function summarize(results) {
    ...
    var advisory = accepted.filter(function (row) { return row.state === 'usable'; });

    if (rejected.length) {
      var first = rejected[0];
      var prefix = accepted.length ? accepted.length + ' photo' + (accepted.length === 1 ? '' : 's') + ' added · ' : '';
      return {kind:'error', text:prefix + rejected.length + ' could not be added: ' + first.reason + (rejected.length > 1 ? ' (+ more)' : '')};
    }
    if (advisory.length) {
      return {kind:'warn', text:accepted.length === 1 ? 'Photo added · quality could be better.' : accepted.length + ' photos added · some could look better.'};
    }
    if (accepted.length) return {kind:'good', text:accepted.length === 1 ? 'Photo added ✓' : accepted.length + ' photos added ✓'};
    return {kind:'', text:''};
  }
  ...
  document.addEventListener('change', function (event) {
    ...
    Promise.all(files.map(analyzeFile)).then(function (results) {
      ...
      try {
        ...
        accepted = [];
        acceptedFiles = [];
        results = [{state:'reject', reason:'this browser could not safely prepare the selection'}];
      }
      ...
      input.value = '';
      checking = false;
      setStatus('error', 'Photo could not be read. Please choose another image.');
    });
  }, true);
