(function () {
  ...
  function svg(name) {
    ...
    return '<svg class="pmd-svg-v31" viewBox="0 0 24 24" aria-hidden="true">' + (icons[name] || icons.alert) + '</svg>';
  }
  ...
  function action(label, icon, href) {
    return '<a class="pmd-manager-action-v29" href="' + href + '"><i class="pmd-inline-icon-v31">' + svg(icon) + '</i><span>' + label + '</span></a>';
  }
  ...
  function alertItem(icon, title, detail, badge) {
    return '<div class="pmd-manager-alert-v29"><i class="pmd-inline-icon-v31">' + svg(icon) + '</i><div><strong>' + title + '</strong><span>' + detail + '</span></div><em class="pmd-manager-badge-v29">' + badge + '</em></div>';
  }

  function eventItem(icon, title, detail, time) {
    return '<div class="pmd-manager-event-v29"><i class="pmd-inline-icon-v31">' + svg(icon) + '</i><div><strong>' + title + '</strong><span>' + detail + '</span></div><time>' + time + '</time></div>';
  }
  ...
  function insight(icon, title, detail, badge) {
    return '<div class="pmd-manager-insight-v29"><i class="pmd-inline-icon-v31">' + svg(icon) + '</i><div><strong>' + title + '</strong><span>' + detail + '</span></div><em class="pmd-manager-badge-v29">' + badge + '</em></div>';
  }
  ...
  function managerHtml() {
    ...
    return '<div class="pmd-manager-ops-v29" data-pmd-manager-ops="v31">' +
      ...
      '<div class="pmd-manager-grid-v29">' +
        ...
        card('half', 'chart', 'gold', 'Performance Analytics', 'Revenue by hour, payment breakdown, average guest spend and lost revenue.',
          '<div class="pmd-manager-analytics-grid-v29">' +
            '<div><div class="pmd-manager-card-sub-v29">Revenue by Hour</div><div class="pmd-manager-chart-bars-v29">' +
  ...
  function applyManagerDashboard(force) {
    ...
    if (applying) return;
    if (!force && pn.getAttribute('data-pmd-manager-ops-v31') === '1') return;
    applying = true;
    pn.innerHTML = managerHtml();
    pn.setAttribute('data-pmd-manager-ops-v31', '1');
    pn.setAttribute('data-pmd-manager-ops-v29', '0');
    pn.setAttribute('data-pmd-manager-ops-v30', '0');
    applying = false;
  }

  function bindRoleButtons() {
    document.querySelectorAll('[data-pmd-role-btn="manager"]').forEach(function (btn) {
      if (btn.getAttribute('data-pmd-manager-v31-bound')) return;
      btn.setAttribute('data-pmd-manager-v31-bound', '1');
      btn.addEventListener('click', function () {
        setTimeout(function () { applyManagerDashboard(true); }, 20);
        setTimeout(function () { applyManagerDashboard(true); }, 180);
      }, true);
    });
  }

  var movingToggle = false;
  function menu() {
  ...
  function stableSidebarToggle() {
    ...
    try {
      ...
      var container = m.querySelector('.pmd-sidebar-toggle-stable-v31');
      if (!container) {
        container = document.createElement('li');
        container.className = 'pmd-sidebar-toggle-stable-v31';
      ...
      var keep = container.querySelector('.pmd-sidebar-icons-toggle') || buttons[0];

      buttons.forEach(function (btn) {
        if (btn !== keep) {
          btn.classList.add('pmd-sidebar-toggle-duplicate-v31');
          if (btn.parentNode) btn.parentNode.removeChild(btn);
        }
      });

      if (keep.parentNode !== container) container.appendChild(keep);

      keep.classList.remove('pmd-sidebar-toggle-duplicate-v31');
      keep.classList.add('pmd-sidebar-icons-toggle-stable-v31');
      keep.style.position = 'static';

      if (!keep.querySelector('.pmd-sidebar-toggle-label-v31')) {
        var label = document.createElement('span');
        label.className = 'pmd-sidebar-toggle-label-v31';
        label.textContent = 'Collapse';
        keep.appendChild(label);
      }

      // Keep the label in sync with state.
      var labelNode = keep.querySelector('.pmd-sidebar-toggle-label-v31');
      var expanded = keep.getAttribute('aria-expanded');
      if (labelNode) labelNode.textContent = expanded === 'false' ? 'Expand' : 'Collapse';

      // Move to the end every time, below System and before user profile block.
      if (m.lastElementChild !== container) m.appendChild(container);
    } finally {
      movingToggle = false;
    }
  }
  ...
  function init() {
    ...
    obs.observe(document.body, {
      ...
      subtree: true,
      attributes: true,
      attributeFilter: ['data-pmd-role', 'aria-expanded', 'class']
    });
    ...
    window.PMDManagerOpsDashboardV31 = {
      refresh: function () { applyManagerDashboard(true); },
      stabilizeSidebarToggle: stableSidebarToggle
    };
  }
