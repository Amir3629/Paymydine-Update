(function () {
  ...
  // PMD_MENU_HEADER_AI_HYDRATE_V5
  // Reuse the server-rendered first-paint AI node.
  // Do not delete it and do not append a second button.
  function mountTrigger() {
    ...
    var existing = actions.querySelector(
      '[data-pmd-menu-ai-import-trigger]'
    );
    ...
    if (existing) {
      ...
      if (slot) {
        ...
        trigger.classList.remove(
          'pmd-menu-firstpaint-action',
          'pmd-menu-firstpaint-action--ai'
        );
        ...
        var gap = actions.querySelector(
          '[data-pmd-main-header-notification-gap-r67]'
        );

        var notifSlot = actions.querySelector(
          '[data-pmd-menu-notif-slot]'
        );

        var notifRoot = actions.querySelector(
          '#notif-root'
        );

        var anchor =
          gap || notifSlot || notifRoot;

        if (anchor) {
          actions.insertBefore(trigger, anchor);
        } else {
