 ...
 * The explanation always follows the KPI currently occupying a card.
 */
(function () {
  'use strict';
  ...
  function currentKey(card) {
    return String(
      card.getAttribute('data-pmd-dashboard2-kpi') ||
      card.getAttribute('data-pmd-kpi-v2401-key') ||
      card.getAttribute('data-pmd-shifts-kpi-key') ||
      ''
    ).trim();
  }
  ...
  section.addEventListener('click', function (event) {
    ...
    // Always rebuild from the current KPI identity at the exact click time.
  ...
  var api = {
    version: '2.0.0-dynamic-selection-v157',
    mount: mount,
    syncCard: syncCard,
