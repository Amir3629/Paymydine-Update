/* PayMyDine Owner/Admin Dashboard Clean v21
   Waiter role cleanup: full-width floor + actionable service order cards. */
(function () {
  'use strict';

  var VERSION = 'owner-clean-v21-waiter-service-floor-orders-20260626';
  var root = null;
  var lastData = null;
    ...
    return '<div class="pmd-v21-waiter-order-grid">' + serviceRows.map(function (o) {
      ...
      return '' +
        '<article class="pmd-v21-waiter-order-card ' + (legacy ? 'is-legacy' : '') + '">' +
          ...
          '</div>' +
          '<footer>' +
            '<a href="' + esc(editUrl) + '">Edit</a>' +
            '<a href="' + esc(editUrl) + '#print">Print</a>' +
            '<a href="' + esc(editUrl) + '#status">Status</a>' +
            '<a href="/admin/kitchendisplay/main-kitchen">Kitchen</a>' +
          '</footer>' +
        '</article>';
    }).join('') + '</div>';
  }
    ...
    if (role === 'waiter') {
      return '<div class="' + shellClass + '">' + header + kpis +
        '<section class="pmd-v21-waiter-floor-row">' +
          section('Live Restaurant Floor', 'Full-width service floor · tables, open checks and payment status', floorPlan(s.floor_plan), 'pmd-v15-card--floor pmd-v19-area-floor pmd-v21-waiter-floor-card', '<div class="pmd-v15-card-actions"><button type="button" data-pmd-owner-refresh>Refresh</button><a href="/admin/orders/create">New Order</a></div>') +
        '</section>' +
