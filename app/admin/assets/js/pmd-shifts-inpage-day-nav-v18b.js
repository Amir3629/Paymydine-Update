...
(function () {
  ...
  function syncTeamAccessFields() {
    ...
    // PMD_SHIFTS_ROLE_DRIVEN_CREDENTIAL_UI_V18B
    // Operational roles are PIN-only in the visible UI. Full-login roles are
    // username/password-only. Inline display is intentional so legacy Shifts
    // CSS cannot accidentally re-expose hidden credential fields.
    setCredentialFieldVisible(teamUsernameField, !quickPin);
    setCredentialFieldVisible(teamPinField, quickPin);
    setCredentialFieldVisible(teamPasswordField, !quickPin);
