(function () {
  ...
  function enhancePosSearch(posRoot) {
    ...
    if (!search.classList.contains('pmd-v234-pos-header-search')) {
      ...
      if (rightSide) {
        /*
         * Core POS can replace the top action container while enhancement
         * scripts are running. prepend() does not depend on a stale reference
         * node and avoids Safari's NotFoundError from insertBefore().
         */
        if (search.parentNode !== rightSide || rightSide.firstElementChild !== search) {
          rightSide.prepend(search);
        }
      } else {
        var clearCart = topbar.querySelector('[data-pos-clear], .pmd-pos-clear');
        if (clearCart && clearCart.parentNode === topbar) topbar.insertBefore(search, clearCart);
        else if (search.parentNode !== topbar) topbar.appendChild(search);
      }
