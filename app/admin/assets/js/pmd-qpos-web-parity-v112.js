...
(function () {
  ...
  function classifyViewportV102(width, height) {
    ...
    var androidTabletV105 =
      ...
      document.documentElement.classList.contains('pmd-qpos-android-pos-v107') ||
      !!(
        document.body &&
        (
          document.body.classList.contains('pmd-qpos-android-pos-v105') ||
          document.body.classList.contains('pmd-qpos-android-pos-v107')
        )
      );
    ...
    if (snapshot.serviceMode === 'dine_in') {
      ...
      var pickupOrder = Object.assign(
        ...
        existingPickup || {},
        {
          ...
          payment_gate:
            json.payment_gate === true ||
            !!(existingPickup && existingPickup.payment_gate === true),
      ...
      force_new_check:
        snapshot.serviceMode === 'dine_in'
          ? (
              ...
              (snapshot.paymentGate && !snapshot.activeOrderId)
            )
          : false,
      quick_pos: true,
    ...
    if (pay) pay.onclick = function () {
      if (state.cart.length > 0) {
        ...
        var payBeforeKitchen =
          !state.activeOrderId ||
          !!(currentOrder && currentOrder.payment_gate === true);

        // PMD_QPOS_PAY_BEFORE_KITCHEN_V108
        // A fresh/direct-pay check must collect money before KDS can see it.
        // Existing kitchen orders keep the normal send+pay append behavior.
        submitOrder(payBeforeKitchen ? 'hold' : 'send', 'pay');
        return;
      }
      openPayment();
    };
