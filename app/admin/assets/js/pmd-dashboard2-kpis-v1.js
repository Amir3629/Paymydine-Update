...
(function () {
  ...
  function ensureRoot() {
    var page = document.getElementById('pmd-reservations2'); if (!page) return null;
    var roots = document.querySelectorAll('#' + ROOT_ID), root = roots[0] || document.createElement('section');
    Array.prototype.slice.call(roots, 1).forEach(function (node) { node.remove(); });
    root.id = ROOT_ID; root.className = 'pmd-dashboard2-analytics-v1'; root.setAttribute('aria-label', 'Dashboard analytics');
    if (root.parentElement !== page || page.lastElementChild !== root) page.appendChild(root);
    return root;
  }
