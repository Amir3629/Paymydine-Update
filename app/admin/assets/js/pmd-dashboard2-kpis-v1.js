...
(function () {
  ...
  var PERIOD_KEY = 'pmd.dashboard2.analyticsPeriod.v1', cache = {}, requestCount = 0, controller = null, data = null, salesChartMode = 'line';
  ...
  function svgLine(rows) { if(!rows||!rows.length)return empty(); var max=Math.max.apply(null,rows.map(function(r){return Number(r.sales||0);}).concat([1])),w=720,h=190,p=24; var points=rows.map(function(r,i){return {x:p+(w-p*2)*(rows.length===1?0:i/(rows.length-1)),y:h-p-(h-p*2)*Number(r.sales||0)/max,r:r};}); return '<svg class="pmd-dashboard2-chart" viewBox="0 0 '+w+' '+h+'" role="img" aria-label="Sales over time line chart"><path class="pmd-chart-grid" d="M24 24H696M24 95H696M24 166H696"/><polyline class="pmd-chart-line" points="'+points.map(function(q){return q.x+','+q.y;}).join(' ')+'"/>'+points.map(function(q){return '<circle cx="'+q.x+'" cy="'+q.y+'" r="4"><title>'+esc(q.r.bucket)+' · '+esc(money(q.r.sales))+' · '+esc(q.r.orders)+' orders</title></circle>';}).join('')+'</svg>'; }
  function svgBars(rows,label) { if(!rows||!rows.length)return empty(); var max=Math.max.apply(null,rows.map(function(r){return Number(r.sales||0);}).concat([1])),w=720,h=190,p=24,g=(w-p*2)/rows.length,bw=Math.max(2,g*.68),peak=Math.max.apply(null,rows.map(function(r){return Number(r.sales||0);})); return '<svg class="pmd-dashboard2-chart" viewBox="0 0 '+w+' '+h+'" role="img" aria-label="'+esc(label)+'"><path class="pmd-chart-grid" d="M24 24H696M24 95H696M24 166H696"/>'+rows.map(function(r,i){var value=Number(r.sales||0),height=(h-p*2)*value/max,x=p+i*g+(g-bw)/2,y=h-p-height;return '<rect class="'+(value===peak&&value>0?'is-peak':(value===0?'is-zero':''))+'" x="'+x+'" y="'+y+'" width="'+bw+'" height="'+Math.max(2,height)+'"><title>'+esc(r.bucket!=null?r.bucket:String(r.hour).padStart(2,'0')+':00')+' · '+esc(money(value))+' · '+esc(r.orders)+' orders</title></rect>';}).join('')+'</svg>'; }
  function svgDonut(rows,name,value,label) { if(!rows||!rows.length)return empty(); var total=rows.reduce(function(sum,r){return sum+Number(r[value]||0);},0);if(total<=0)return empty();var colors=['#2f7d69','#315d8a','#d28a35','#8a5c9e','#738078','#b84d55'],offset=0,circles=rows.map(function(r,i){var pct=Number(r[value]||0)/total*100,c='<circle cx="60" cy="60" r="45" pathLength="100" fill="none" stroke="'+colors[i%colors.length]+'" stroke-width="18" stroke-dasharray="'+pct+' '+(100-pct)+'" stroke-dashoffset="'+(-offset)+'"><title>'+esc(r[name])+' · '+esc(money(r[value]))+' · '+pct.toFixed(1)+'%</title></circle>';offset+=pct;return c;}).join('');var legend='<ul class="pmd-chart-legend">'+rows.map(function(r,i){var pct=Number(r[value]||0)/total*100;return '<li><i style="background:'+colors[i%colors.length]+'"></i><span>'+esc(r[name])+'</span><b>'+esc(label(r))+' · '+pct.toFixed(1)+'%</b></li>';}).join('')+'</ul>';return '<div class="pmd-dashboard2-donut"><svg viewBox="0 0 120 120" role="img" aria-label="Breakdown chart"><circle cx="60" cy="60" r="45" pathLength="100" fill="none" stroke="#edf1ef" stroke-width="18"/>'+circles+'</svg>'+legend+'</div>'; }
  ...
  function render() {
    ...
    var c=data.sales_by_category; put('categorySales',c.available?svgDonut(c.categories,'category','revenue',function(r){return money(r.revenue);}):empty(c));
    var p=data.payment_methods; put('paymentMethods',p.available?svgDonut(p.methods,'method','total',function(r){return money(r.total)+' · '+r.transactions;}):empty(p));
    var ch=data.channels; put('channelSplit',ch.available?svgDonut(ch.channels,'channel','revenue',function(r){return r.orders+' · '+money(r.revenue);}):empty(ch));
