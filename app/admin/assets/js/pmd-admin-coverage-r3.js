...
(function () {
    ...
    function patchShifts() {
        ...
        var localeTag = locale === 'tr' ? 'tr-TR' : 'de-DE';
        ...
        var dated = document.querySelector('[data-pmd-shifts-server-initial][data-date]');
        var heading = document.querySelector('.pmd-shifts-final-date h2');
        if (dated && heading) {
            var raw = dated.getAttribute('data-date') || '';
            var parts = raw.split('-').map(Number);
            if (parts.length === 3 && parts[0] && parts[1] && parts[2]) {
                var date = new Date(parts[0], parts[1] - 1, parts[2], 12, 0, 0, 0);
                try {
                    heading.textContent = new Intl.DateTimeFormat(locale === 'tr' ? 'tr-TR' : 'de-DE', {
