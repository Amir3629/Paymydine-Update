...
/*        text = text[:s] + "\n" + text[e:]
        count += 1
    removed PMD_KDS_STATIONS_INDEX_V96_2_START */
(function () {
  var MARK = "PMD_KDS_STATIONS_INDEX_V96_2";
  var STYLE_ID = "pmd-kds-stations-index-v96-2-style";
  ...
  function findOriginalTable() {
    ...
    tables.forEach(function (table) {
      var t = norm(txt(table));
      var links = qsa("a[href*='/admin/kds_stations/edit/']", table).length;
      var score = 0;

      if (t.indexOf("station") >= 0) score += 5;
      if (t.indexOf("url slug") >= 0) score += 5;
      if (t.indexOf("routing") >= 0) score += 3;
      if (t.indexOf("can update") >= 0) score += 3;
      if (links) score += 10 + links;
  ...
  function findOriginalWrap(table) {
    if (!table) return null;
    return (
      table.closest(".table-responsive") ||
      table.closest(".list-table") ||
  ...
  function getHeaders(table) {
    var headers = qsa("thead th", table).map(function (th) {
      return cleanCell(txt(th));
    });

    if (!headers.length) {
      var firstRow = table.querySelector("tr");
      headers = firstRow ? qsa("th,td", firstRow).map(function (th) { return cleanCell(txt(th)); }) : [];
    }
  ...
  function getCell(cells, idx) {
    ...
    return cleanCell(txt(cells[idx]));
  }

  function pickFallbackName(cells) {
    var ignored = /^(enabled|disabled|active|inactive|all categories|0|1|2|3|4|-1|-2|yes|no)$/i;

    for (var i = 0; i < cells.length; i++) {
      var s = cleanCell(txt(cells[i]));
      if (!s) continue;
      if (ignored.test(s)) continue;
      if (isSlug(s)) continue;
      if (/edit|open|delete|select|filter/i.test(s)) continue;
      if (s.length < 3) continue;
      if (/kitchen|bar|grill|dessert|expo|pass|main|station/i.test(s)) return s;
    }

    for (var j = 0; j < cells.length; j++) {
      var s2 = cleanCell(txt(cells[j]));
      if (!s2) continue;
      if (ignored.test(s2)) continue;
      if (isSlug(s2)) continue;
  ...
  function pickSlug(cells, slugIdx) {
    var direct = getCell(cells, slugIdx);
    if (direct && isSlug(direct)) return direct;

    for (var i = 0; i < cells.length; i++) {
      var s = cleanCell(txt(cells[i]));
      if (isSlug(s)) return s;
    }
  ...
  function pickStatus(s) {
    var v = cleanCell(s);
    if (/disabled|inactive|off/i.test(v)) return "Disabled";
    if (/enabled|active|on|yes/i.test(v)) return "Active";
    return v || "Active";
  }

  function pickType(v) {
    var s = cleanCell(v);
    if (!s || /^-?\d+$/.test(s)) return "Kitchen Display";
    if (isSlug(s)) return "Kitchen Display";
    if (/enabled|disabled|active|inactive|all categories/i.test(s)) return "Kitchen Display";
    return s;
  }

  function pickRouting(v) {
    var s = cleanCell(v);
    if (!s) return "All categories";
    if (/enabled|disabled|active|inactive/i.test(s)) return "All categories";
    return s;
  }

  function pickCanUpdate(v) {
    var s = cleanCell(v);
    if (/enabled|active|yes|on/i.test(s)) return "Enabled";
    if (/disabled|inactive|no|off/i.test(s)) return "Disabled";
    return s || "Enabled";
  }
  ...
  function parseRows(table) {
    ...
    var stationIdx = findHeaderIndex(headers, [/^station$/, /station name/, /^name$/]);
    var slugIdx = findHeaderIndex(headers, [/url slug/, /^slug$/]);
    var typeIdx = findHeaderIndex(headers, [/^type$/, /station type/]);
    var routingIdx = findHeaderIndex(headers, [/routing/, /categories/]);
    var canUpdateIdx = findHeaderIndex(headers, [/can update/, /update/]);
    var activeIdx = findHeaderIndex(headers, [/^active$/, /status/]);
    var priorityIdx = findHeaderIndex(headers, [/priority/, /sort/]);

    var rows = qsa("tbody tr", table).filter(function (tr) {
      return qsa("td", tr).length > 0;
    });

    return rows.map(function (tr, index) {
      var cells = qsa("td", tr);

      var rawName = getCell(cells, stationIdx);
      var fallbackName = pickFallbackName(cells);
      var slug = pickSlug(cells, slugIdx);

      var name = rawName || fallbackName || ("Station " + (index + 1));
      if (isSlug(name) && fallbackName && fallbackName !== name) name = fallbackName;
      if (/^enabled$|^disabled$|^active$|^all categories$/i.test(name)) name = fallbackName || ("Station " + (index + 1));

      var type = pickType(getCell(cells, typeIdx));
      var routing = pickRouting(getCell(cells, routingIdx));
      var canUpdate = pickCanUpdate(getCell(cells, canUpdateIdx));
      var active = pickStatus(getCell(cells, activeIdx));
      var priority = cleanCell(getCell(cells, priorityIdx)) || "0";

      /* PMD_KDS_INDEX_V135_FIX_V962_RAW_POSITION_PARSER_START
         Fixes the 0.1s wrong-card flash.
         v96.2 was building cards with header-based shifted values:
         Type=Kitchen Display, Routing=kitchen, Can update=All categories, Priority=Enabled.
         This uses the same slug-position logic as v96.4 before the card is ever rendered.
      */
      (function () {
        var raw = cells.map(function (td) { return cleanCell(txt(td)); });

        function isEnabledValue(v) {
          return /^(enabled|disabled|active|inactive|yes|no|on|off)$/i.test(cleanCell(v));
        }

        function isNumberValue(v) {
          return /^-?\d+$/.test(cleanCell(v));
        }

        function normalizeEnabledValue(v, fallback) {
          v = cleanCell(v);
          if (/disabled|inactive|off|no/i.test(v)) return "Disabled";
          if (/enabled|active|on|yes/i.test(v)) return "Enabled";
          return fallback || "Enabled";
        }

        function normalizeTypeValue(v) {
          v = cleanCell(v);
          if (!v || isSlug(v) || isEnabledValue(v) || isNumberValue(v) || /all categories/i.test(v)) return "Kitchen Display";
          return v;
        }

        function normalizeRoutingValue(v) {
          v = cleanCell(v);
          if (!v || isEnabledValue(v) || isNumberValue(v)) return "All categories";
          return v;
        }

        function validStationName(v) {
          v = cleanCell(v);
          if (!v) return false;
          if (isSlug(v)) return false;
          if (isEnabledValue(v)) return false;
          if (isNumberValue(v)) return false;
          if (/all categories/i.test(v)) return false;
          if (/edit|open|delete|select|filter/i.test(v)) return false;
          return v.length >= 3;
        }
        ...
        if (slugPos >= 0) {
          var posName = raw[slugPos - 1] || "";
          var posSlug = raw[slugPos] || "";
          var posType = raw[slugPos + 1] || "";
          var posRouting = raw[slugPos + 2] || "";
          var posCanUpdate = raw[slugPos + 3] || "";
          var posActive = raw[slugPos + 4] || "";
          var posPriority = raw[slugPos + 5] || "";

          var enabledCandidates = raw.filter(isEnabledValue).map(function (v) {
            return normalizeEnabledValue(v);
          });

          var numberCandidates = raw.filter(isNumberValue);

          if (validStationName(posName)) name = posName;
          if (isSlug(posSlug)) slug = posSlug;

          type = normalizeTypeValue(posType);
          routing = normalizeRoutingValue(posRouting);
          canUpdate = normalizeEnabledValue(posCanUpdate, enabledCandidates[0] || canUpdate || "Enabled");
          active = normalizeEnabledValue(posActive, enabledCandidates[1] || enabledCandidates[0] || active || "Enabled");
          priority = isNumberValue(posPriority)
            ? cleanCell(posPriority)
            : (numberCandidates[numberCandidates.length - 1] || priority || "0");
        }
      })();
      /* PMD_KDS_INDEX_V135_FIX_V962_RAW_POSITION_PARSER_END */

      var allLinks = qsa("a[href]", tr);
      var editHref = "";
      var displayHref = "";

      allLinks.forEach(function (a) {
        var href = a.href || "";
        if (!editHref && href.indexOf("/admin/kds_stations/edit/") >= 0) editHref = href;
      });

      allLinks.forEach(function (a) {
        var href = a.href || "";
        if (!href) return;
        if (href === editHref) return;
        if (/kds|display|screen|stations/i.test(href) && href.indexOf("/admin/kds_stations/edit/") < 0) {
          if (!displayHref) displayHref = href;
        }
      });
  ...
  function stationIcon(row) {
    var s = (row.name + " " + row.type + " " + row.slug).toLowerCase();
    if (/bar|drink|cocktail/.test(s)) return "🍸";
    if (/grill|fire/.test(s)) return "🔥";
    if (/dessert|cake|sweet/.test(s)) return "🧁";
    if (/expo|pass/.test(s)) return "🧾";
    return "🖥️";
  }

  function statusClass(status) {
    return /disabled|inactive/i.test(status || "") ? "is-disabled" : "is-active";
  }

  function renderRows(rows) {
    var active = rows.filter(function (r) { return !/disabled|inactive/i.test(r.active); }).length;
    var disabled = rows.length - active;
    var allCategories = rows.filter(function (r) { return /all categories/i.test(r.routing); }).length;

    var createHref = location.origin + "/admin/kds_stations/create";
    var many = rows.length > 8;

    var cards = rows.map(function (r) {
      var displayBtn = "";
      if (r.displayHref && r.displayHref !== r.editHref) {
        displayBtn = '<a class="pmd962-btn pmd962-btn-secondary" href="' + esc(r.displayHref) + '">Open display</a>';
      }

      return [
        '<article class="pmd962-card" data-pmd962-card data-station-index="' + esc(r.index) + '">',
          '<div class="pmd962-card-top">',
            '<div class="pmd962-icon">' + stationIcon(r) + '</div>',
            '<div class="pmd962-title-wrap">',
              '<div class="pmd962-name">' + esc(r.name) + '</div>',
              '<div class="pmd962-slug">' + esc(r.slug || "No slug") + '</div>',
            '</div>',
            '<div class="pmd962-pill ' + statusClass(r.active) + '">● ' + esc(r.active) + '</div>',
          '</div>',
          '<div class="pmd962-details">',
            '<div class="pmd962-detail"><span>Type</span><b>' + esc(r.type) + '</b></div>',
            '<div class="pmd962-detail"><span>Routing</span><b>' + esc(r.routing) + '</b></div>',
            '<div class="pmd962-detail"><span>Can update</span><b>' + esc(r.canUpdate) + '</b></div>',
            '<div class="pmd962-detail"><span>Priority</span><b>' + esc(r.priority) + '</b></div>',
          '</div>',
          '<div class="pmd962-actions-row">',
            r.editHref ? '<a class="pmd962-btn pmd962-btn-primary" href="' + esc(r.editHref) + '">Edit station</a>' : '',
            displayBtn,
          '</div>',
        '</article>'
      ].join("");
    ...
    return [
      '<div class="pmd962-shell ' + (many ? "pmd962-many" : "") + '" data-pmd962-shell>',
        '<section class="pmd962-hero">',
          '<div>',
            '<h1>Manage KDS Stations</h1>',
            '<p>Create, review, and manage kitchen display stations from one clean workspace.</p>',
          '</div>',
          '<div class="pmd962-hero-actions">',
            '<a class="pmd962-btn pmd962-btn-primary pmd962-new" href="' + esc(createHref) + '">＋ New KDS Station</a>',
            '<button type="button" class="pmd962-btn pmd962-btn-secondary" data-pmd962-refresh>Refresh</button>',
          '</div>',
        '</section>',

        '<section class="pmd962-stats">',
          '<div class="pmd962-stat"><span>Total stations</span><b>' + rows.length + '</b></div>',
          '<div class="pmd962-stat"><span>Active</span><b>' + active + '</b></div>',
          '<div class="pmd962-stat"><span>Disabled</span><b>' + disabled + '</b></div>',
          '<div class="pmd962-stat"><span>All categories</span><b>' + allCategories + '</b></div>',
        '</section>',

        '<section class="pmd962-grid">',
          cards || '<div class="pmd962-empty">No KDS stations found yet.</div>',
        '</section>',

        '<section class="pmd962-table-shell" data-pmd962-table-shell>',
          '<div>',
            '<h3>Advanced table</h3>',
            '<p>Use the original table only for filters, bulk actions, and detailed admin tools.</p>',
          '</div>',
          '<button type="button" class="pmd962-btn pmd962-btn-secondary" data-pmd962-toggle>Show table</button>',
        '</section>',
      '</div>'
    ].join("");
  }
  ...
  function installStyle() {
    ...
    style.id = STYLE_ID;
    style.textContent = [
      "body.pmd-kds-stations-v962 .pmd962-shell{max-width:calc(100vw - 190px);margin:24px auto 80px;padding:0 8px;box-sizing:border-box;color:#10252c;}",
      "body.pmd-kds-stations-v962 .pmd962-shell *{box-sizing:border-box;}",
      "body.pmd-kds-stations-v962 .pmd962-hero{display:flex;align-items:center;justify-content:space-between;gap:22px;padding:28px 28px;border:1px solid #cfe4f1;border-radius:24px;background:linear-gradient(135deg,#ffffff 0%,#f7fffb 52%,#f8fbff 100%);box-shadow:0 20px 56px rgba(16,37,44,.06);}",
      "body.pmd-kds-stations-v962 .pmd962-hero h1{margin:0 0 8px;font-size:34px;line-height:1.15;font-weight:950;letter-spacing:-.035em;color:#073b34;}",
      "body.pmd-kds-stations-v962 .pmd962-hero p{margin:0;color:#4d6178;font-weight:800;font-size:15px;line-height:1.45;}",
      "body.pmd-kds-stations-v962 .pmd962-hero-actions{display:flex;align-items:center;gap:12px;flex-wrap:wrap;justify-content:flex-end;}",
      "body.pmd-kds-stations-v962 .pmd962-btn{min-height:44px;display:inline-flex;align-items:center;justify-content:center;gap:8px;border-radius:14px;padding:0 18px;font-size:14px;font-weight:950;text-decoration:none;border:1px solid #cfe4f1;cursor:pointer;white-space:nowrap;line-height:1;background-color:#fff;color:#073b34;box-shadow:0 10px 28px rgba(16,37,44,.06);}",
      "body.pmd-kds-stations-v962 .pmd962-btn-primary{background-color:#07966f!important;background-image:none!important;border-color:#07966f!important;color:#fff!important;}",
      "body.pmd-kds-stations-v962 .pmd962-btn-primary:hover{background-color:#057a5d!important;color:#fff!important;}",
      "body.pmd-kds-stations-v962 .pmd962-btn-secondary{background-color:#fff!important;color:#073b34!important;}",
      "body.pmd-kds-stations-v962 .pmd962-stats{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px;margin:20px 0;}",
      "body.pmd-kds-stations-v962 .pmd962-stat{padding:20px;border:1px solid #cfe4f1;border-radius:20px;background:#fff;box-shadow:0 16px 42px rgba(16,37,44,.045);}",
      "body.pmd-kds-stations-v962 .pmd962-stat span{display:block;margin-bottom:8px;color:#4d6178;font-size:12px;text-transform:uppercase;letter-spacing:.08em;font-weight:950;}",
      "body.pmd-kds-stations-v962 .pmd962-stat b{display:block;color:#073b34;font-size:30px;line-height:1;font-weight:950;}",
      "body.pmd-kds-stations-v962 .pmd962-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:16px;}",
      "body.pmd-kds-stations-v962 .pmd962-many .pmd962-grid{grid-template-columns:repeat(auto-fit,minmax(300px,1fr));}",
      "body.pmd-kds-stations-v962 .pmd962-card{min-width:0;border:1px solid #cfe4f1;border-radius:22px;background:linear-gradient(135deg,#ffffff 0%,#f9fffc 100%);box-shadow:0 18px 50px rgba(16,37,44,.06);padding:22px;}",
      "body.pmd-kds-stations-v962 .pmd962-card-top{display:grid;grid-template-columns:48px minmax(0,1fr) auto;align-items:start;gap:14px;margin-bottom:20px;}",
      "body.pmd-kds-stations-v962 .pmd962-icon{width:48px;height:48px;border-radius:16px;display:flex;align-items:center;justify-content:center;background:#eafff4;font-size:22px;box-shadow:0 12px 28px rgba(7,150,111,.08);}",
      "body.pmd-kds-stations-v962 .pmd962-title-wrap{min-width:0;}",
      "body.pmd-kds-stations-v962 .pmd962-name{font-size:23px;line-height:1.15;font-weight:950;color:#073b34;letter-spacing:-.025em;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}",
      "body.pmd-kds-stations-v962 .pmd962-slug{margin-top:6px;font-size:13px;line-height:1.25;color:#4d6178;font-weight:850;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}",
      "body.pmd-kds-stations-v962 .pmd962-pill{display:inline-flex;align-items:center;justify-content:center;min-height:30px;padding:0 12px;border-radius:999px;font-size:13px;font-weight:950;white-space:nowrap;}",
      "body.pmd-kds-stations-v962 .pmd962-pill.is-active{background:#d9fae8;color:#047857;}",
      "body.pmd-kds-stations-v962 .pmd962-pill.is-disabled{background:#f1f5f9;color:#475569;}",
      "body.pmd-kds-stations-v962 .pmd962-details{display:grid;grid-template-columns:1fr 1fr;gap:12px;margin-bottom:18px;}",
      "body.pmd-kds-stations-v962 .pmd962-detail{min-width:0;border:1px solid #d7e7f2;border-radius:15px;background:#fbfdff;padding:13px 14px;}",
      "body.pmd-kds-stations-v962 .pmd962-detail span{display:block;color:#4d6178;font-size:11px;line-height:1.15;text-transform:uppercase;letter-spacing:.08em;font-weight:950;margin-bottom:7px;}",
      "body.pmd-kds-stations-v962 .pmd962-detail b{display:block;color:#10252c;font-size:14px;line-height:1.25;font-weight:950;overflow:hidden;text-overflow:ellipsis;white-space:nowrap;}",
      "body.pmd-kds-stations-v962 .pmd962-actions-row{display:flex;gap:10px;flex-wrap:wrap;align-items:center;}",
      "body.pmd-kds-stations-v962 .pmd962-actions-row .pmd962-btn{min-height:42px;padding:0 16px;}",
      "body.pmd-kds-stations-v962 .pmd962-table-shell{display:flex;align-items:center;justify-content:space-between;gap:16px;margin-top:20px;padding:18px 20px;border:1px solid #cfe4f1;border-radius:20px;background:#fff;box-shadow:0 16px 42px rgba(16,37,44,.045);}",
      "body.pmd-kds-stations-v962 .pmd962-table-shell h3{margin:0 0 4px;color:#073b34;font-size:18px;font-weight:950;}",
      "body.pmd-kds-stations-v962 .pmd962-table-shell p{margin:0;color:#4d6178;font-size:13px;font-weight:800;}",
      "body.pmd-kds-stations-v962 .pmd962-hidden-original{display:none!important;}",
      "body.pmd-kds-stations-v962 .pmd962-original-open{display:block!important;margin-top:18px!important;}",
      "body.pmd-kds-stations-v962 .pmd962-empty{grid-column:1/-1;padding:28px;border:1px dashed #cfe4f1;border-radius:20px;color:#4d6178;font-weight:850;text-align:center;background:#fff;}",

      "@media (max-width:1500px){body.pmd-kds-stations-v962 .pmd962-grid{grid-template-columns:repeat(2,minmax(0,1fr));}body.pmd-kds-stations-v962 .pmd962-stats{grid-template-columns:repeat(4,minmax(0,1fr));}}",
      "@media (max-width:1100px){body.pmd-kds-stations-v962 .pmd962-shell{max-width:calc(100vw - 130px);}body.pmd-kds-stations-v962 .pmd962-grid{grid-template-columns:1fr;}body.pmd-kds-stations-v962 .pmd962-stats{grid-template-columns:repeat(2,minmax(0,1fr));}body.pmd-kds-stations-v962 .pmd962-hero{align-items:flex-start;flex-direction:column;}}",
      "@media (max-width:640px){body.pmd-kds-stations-v962 .pmd962-shell{max-width:100%;width:100%;padding:0 12px;margin:16px 0 64px;}body.pmd-kds-stations-v962 .pmd962-hero{padding:20px;border-radius:20px;}body.pmd-kds-stations-v962 .pmd962-hero h1{font-size:28px;}body.pmd-kds-stations-v962 .pmd962-stats{grid-template-columns:1fr;}body.pmd-kds-stations-v962 .pmd962-card{padding:18px;}body.pmd-kds-stations-v962 .pmd962-card-top{grid-template-columns:44px minmax(0,1fr);}.pmd962-pill{grid-column:2;}body.pmd-kds-stations-v962 .pmd962-details{grid-template-columns:1fr;}body.pmd-kds-stations-v962 .pmd962-btn{width:100%;}body.pmd-kds-stations-v962 .pmd962-table-shell{flex-direction:column;align-items:stretch;}}"
    ].join("\n");
  ...
  function cleanupOldRuntime() {
    qsa(".pmd961-shell").forEach(function (el) { el.remove(); });
    qsa(".pmd962-shell").forEach(function (el) { el.remove(); });
    document.body.classList.remove("pmd-kds-stations-v961");
    document.body.classList.add("pmd-kds-stations-v962");
  }
  ...
  function apply() {
    ...
    if (!table) {
      console.warn("[PMD] KDS stations v96.2: original table not found yet");
      return null;
    }

    var oldWrap = findOriginalWrap(table);
    var rows = parseRows(table);

    if (!rows.length) {
      console.warn("[PMD] KDS stations v96.2: no rows parsed");
      return null;
    }

    var shell = document.createElement("div");
    shell.innerHTML = renderRows(rows);
    shell = shell.firstElementChild;

    var insertBefore = oldWrap || table;
    insertBefore.parentNode.insertBefore(shell, insertBefore);

    if (oldWrap) {
      oldWrap.classList.add("pmd962-hidden-original");
      oldWrap.setAttribute("data-pmd962-original-table-wrap", "1");
    }

    var toggle = shell.querySelector("[data-pmd962-toggle]");
    if (toggle && oldWrap) {
      toggle.addEventListener("click", function () {
        var isOpen = oldWrap.classList.contains("pmd962-original-open");

        if (isOpen) {
          oldWrap.classList.remove("pmd962-original-open");
          oldWrap.classList.add("pmd962-hidden-original");
          toggle.textContent = "Show table";
        } else {
          oldWrap.classList.remove("pmd962-hidden-original");
          oldWrap.classList.add("pmd962-original-open");
          toggle.textContent = "Hide table";
        }
      });
    }

    var refresh = shell.querySelector("[data-pmd962-refresh]");
    if (refresh) {
    ...
    window.PMD_KDS_STATIONS_INDEX_V96_2_ROWS = rows;

    console.info("[PMD] KDS stations index v96.2 active", {
      mark: MARK,
      rows: rows.length,
      firstRow: rows[0]
    });
  ...
  function check() {
    var shell = document.querySelector("[data-pmd962-shell]");
    var table = findOriginalTable();
    var rows = table ? parseRows(table) : [];
    var cards = qsa("[data-pmd962-card]", shell || document).filter(visible);
    var issues = [];

    if (!shell) issues.push({type:"SHELL_MISSING"});
    if (!table) issues.push({type:"ORIGINAL_TABLE_MISSING"});
    if (rows.length !== cards.length) issues.push({type:"CARD_ROW_COUNT_MISMATCH", rows: rows.length, cards: cards.length});

    rows.forEach(function (row, i) {
      var card = cards[i];
      if (!card) return;

      var shownName = txt(card.querySelector(".pmd962-name"));
      var shownSlug = txt(card.querySelector(".pmd962-slug"));
      var shownType = txt(card.querySelector(".pmd962-detail b"));

      if (shownName !== row.name) {
        issues.push({type:"NAME_MISMATCH", row: i + 1, expected: row.name, shown: shownName});
      }

      if (row.slug && shownSlug !== row.slug) {
        issues.push({type:"SLUG_MISMATCH", row: i + 1, expected: row.slug, shown: shownSlug});
      }

      if (/^Unnamed station/i.test(shownName) && /main|kitchen|bar|grill|expo|dessert/i.test(row.rowText)) {
        issues.push({type:"UNNAMED_CARD_BUT_ROW_HAS_REAL_NAME", row: i + 1, rowText: row.rowText});
      }

      if (shownType && isSlug(shownType)) {
        issues.push({type:"TYPE_SHOWS_SLUG", row: i + 1, shownType: shownType});
      }
    });

    var newBtn = shell ? shell.querySelector(".pmd962-new") : null;
    if (!newBtn || !/\/admin\/kds_stations\/create$/.test(newBtn.href)) {
      issues.push({type:"NEW_BUTTON_WRONG_HREF", href: newBtn ? newBtn.href : null});
    }
    ...
    var summary = {
      ...
      bodyHasV962: document.body.classList.contains("pmd-kds-stations-v962"),
      shellExists: !!shell,
    ...
    window.PMD_KDS_STATIONS_INDEX_V96_2_REPORT = {
      summary: summary,
      issues: issues,
      rows: rows.map(function (r) {
        return {
          ...
          priority: r.priority,
          editHref: r.editHref,
          displayHref: r.displayHref
        };
      })
    };

    console.log("✅ PMD KDS STATIONS INDEX v96.2 SUMMARY");
    console.table([summary]);
    console.log("📌 ISSUES");
    console.table(issues);
    console.log("📌 ROWS");
    console.table(window.PMD_KDS_STATIONS_INDEX_V96_2_REPORT.rows);
    console.log("Copy summary: copy(JSON.stringify(window.PMD_KDS_STATIONS_INDEX_V96_2_REPORT.summary, null, 2))");
    console.log("Copy issues: copy(JSON.stringify(window.PMD_KDS_STATIONS_INDEX_V96_2_REPORT.issues, null, 2))");

    try {
      copy(JSON.stringify(summary, null, 2));
    } catch (e) {}

    return window.PMD_KDS_STATIONS_INDEX_V96_2_REPORT;
  }

  window.PMDKdsStationsIndexV962 = {
    mark: MARK,
    apply: apply,
    check: check
  };
  ...
  if (document.readyState === "loading") {
    document.addEventListener("DOMContentLoaded", function () {
      setTimeout(function () { boot(1); }, 250);
    });
  } else {
    setTimeout(function () { boot(1); }, 250);
  }
})();
/* PMD_KDS_STATIONS_INDEX_V96_2_END */


/* PMD_KDS_STATIONS_INDEX_V96_4_FINAL_START */
(function () {
  var MARK = "PMD_KDS_STATIONS_INDEX_V96_4_FINAL";
  var STYLE_ID = "pmd-kds-stations-index-v96-4-final-style";
  ...
  function isSlug(v) {
    return /^[a-z0-9]+(?:-[a-z0-9]+)+$/i.test(clean(v));
  }

  function isEnabled(v) {
    return /^(enabled|disabled|active|inactive|yes|no|on|off)$/i.test(clean(v));
  }

  function isNum(v) {
    return /^-?\d+$/.test(clean(v));
  }
  ...
  function findTable() {
    ...
    qsa("table").forEach(function (table) {
      var t = txt(table).toLowerCase();
      var editLinks = qsa("a[href*='/admin/kds_stations/edit/']", table).length;
      var score = 0;

      if (t.indexOf("station") !== -1) score += 5;
      if (t.indexOf("url slug") !== -1) score += 5;
      if (t.indexOf("routing") !== -1) score += 4;
      if (t.indexOf("can update") !== -1) score += 4;
      if (editLinks) score += 30 + editLinks;
  ...
  function normalizeType(v) {
    v = clean(v);
    if (!v || isSlug(v) || isEnabled(v) || isNum(v) || /all categories/i.test(v)) return "Kitchen Display";
    return v;
  }

  function normalizeRouting(v) {
    v = clean(v);
    if (!v || isEnabled(v) || isNum(v)) return "All categories";
    return v;
  }

  function normalizeEnabled(v, fallback) {
    v = clean(v);
    if (/disabled|inactive|off|no/i.test(v)) return "Disabled";
    if (/enabled|active|on|yes/i.test(v)) return "Enabled";
    return fallback || "Enabled";
  ...
  function parseRow(tr, index) {
    var cells = qsa("td", tr);
    var raw = cells.map(function (td) { return clean(txt(td)); });
    var editHref = (qsa("a[href*='/admin/kds_stations/edit/']", tr)[0] || {}).href || "";
    ...
    var slugIdx = raw.findIndex(isSlug);

    if (slugIdx >= 0 && slugIdx !== 4) {
      slug = raw[slugIdx] || slug;
      name = raw[slugIdx - 1] || name;
      type = raw[slugIdx + 1] || type;
      routing = raw[slugIdx + 2] || routing;
      canUpdate = raw[slugIdx + 3] || canUpdate;
      active = raw[slugIdx + 4] || active;
      priority = raw[slugIdx + 5] || priority;
    }

    if (!name || isSlug(name) || isEnabled(name) || isNum(name) || /all categories/i.test(name)) {
      name = "Station " + (index + 1);
    }

    if (!isSlug(slug)) slug = "";

    var enabledCandidates = raw.filter(isEnabled).map(function (v) { return normalizeEnabled(v); });
    var numberCandidates = raw.filter(isNum);
    ...
    return {
      ...
      canUpdate: normalizeEnabled(canUpdate, enabledCandidates[0] || "Enabled"),
      active: normalizeEnabled(active, enabledCandidates[1] || enabledCandidates[0] || "Enabled"),
      priority: isNum(priority) ? clean(priority) : (numberCandidates[numberCandidates.length - 1] || "0"),
      editHref: editHref,
      rawCells: raw
    };
  }
  ...
  function parseRows() {
    var table = findTable();
    var rows = table
      ? qsa("tbody tr", table).filter(function (tr) { return qsa("td", tr).length; }).map(parseRow)
      : [];
  ...
  function shell() {
    return document.querySelector(".pmd962-shell, .pmd961-shell");
  }

  function cards(root) {
    return root ? qsa(".pmd962-card, .pmd961-card", root).filter(visible) : [];
  }
  ...
  function setDetail(card, label, value) {
    qsa(".pmd962-detail, .pmd961-detail", card).forEach(function (detail) {
      var labelEl = detail.querySelector(".pmd962-detail-label, .pmd961-detail-label, span");
      var valueEl = detail.querySelector(".pmd962-detail-value, .pmd961-detail-value, b, strong");

      if (!labelEl || !valueEl) return;

      if (txt(labelEl).toLowerCase() === label.toLowerCase()) {
  ...
  function installStyle() {
    ...
    style.id = STYLE_ID;
    style.textContent = [
      "body.pmd-kds-stations-v964-final .pmd962-shell,body.pmd-kds-stations-v964-final .pmd961-shell{animation:none!important;}",
      "body.pmd-kds-stations-v964-final .pmd962-name,body.pmd-kds-stations-v964-final .pmd961-name{color:#073b34!important;letter-spacing:-.02em!important;line-height:1.12!important;}",
      "body.pmd-kds-stations-v964-final .pmd962-slug,body.pmd-kds-stations-v964-final .pmd961-slug{color:#5b6e82!important;max-width:100%!important;overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important;}",
      "body.pmd-kds-stations-v964-final .pmd962-detail,body.pmd-kds-stations-v964-final .pmd961-detail{min-height:70px!important;overflow:hidden!important;}",
      "body.pmd-kds-stations-v964-final .pmd962-detail-label,body.pmd-kds-stations-v964-final .pmd961-detail-label{color:#64748b!important;}",
      "body.pmd-kds-stations-v964-final .pmd962-detail-value,body.pmd-kds-stations-v964-final .pmd961-detail-value{color:#0f172a!important;display:-webkit-box!important;-webkit-line-clamp:2!important;-webkit-box-orient:vertical!important;overflow:hidden!important;}",
      "body.pmd-kds-stations-v964-final .pmd962-btn-primary,body.pmd-kds-stations-v964-final .pmd961-btn-primary{background:#006b55!important;border-color:#006b55!important;color:#fff!important;}",
      "body.pmd-kds-stations-v964-final .pmd962-btn-secondary,body.pmd-kds-stations-v964-final .pmd961-btn-secondary{background:#fff!important;border-color:#cfe0eb!important;color:#073b34!important;}",
      "body.pmd-kds-stations-v964-final .pmd962-grid,body.pmd-kds-stations-v964-final .pmd961-grid{align-items:stretch!important;}",
      "@media(max-width:900px){body.pmd-kds-stations-v964-final .pmd962-grid,body.pmd-kds-stations-v964-final .pmd961-grid{grid-template-columns:1fr!important;}body.pmd-kds-stations-v964-final .pmd962-stats,body.pmd-kds-stations-v964-final .pmd961-stats{grid-template-columns:1fr 1fr!important;}}",
      "@media(max-width:520px){body.pmd-kds-stations-v964-final .pmd962-stats,body.pmd-kds-stations-v964-final .pmd961-stats{grid-template-columns:1fr!important;}body.pmd-kds-stations-v964-final .pmd962-actions,body.pmd-kds-stations-v964-final .pmd961-actions{flex-direction:column!important;}body.pmd-kds-stations-v964-final .pmd962-actions a,body.pmd-kds-stations-v964-final .pmd961-actions a{width:100%!important;}}"
    ].join("\n");

    document.head.appendChild(style);
    document.body.classList.add("pmd-kds-stations-v964-final");
  }
  ...
  function apply() {
    ...
    if (!root && window.PMDKdsStationsIndexV962 && typeof window.PMDKdsStationsIndexV962.apply === "function") {
      try { window.PMDKdsStationsIndexV962.apply(); } catch (e) {}
      root = shell();
    }

    var cardList = cards(root);

    cardList.forEach(function (card, i) {
      var row = parsed.rows[i];
      if (!row) return;

      setCardText(card, ".pmd962-name,.pmd961-name", row.name);
      setCardText(card, ".pmd962-slug,.pmd961-slug", row.slug || "No slug");
      setCardText(card, ".pmd962-pill,.pmd961-pill", "● " + row.active);

      setDetail(card, "Type", row.type);
      setDetail(card, "Routing", row.routing);
      setDetail(card, "Can update", row.canUpdate);
      setDetail(card, "Priority", row.priority);

      var editBtn = qsa("a", card).find(function (a) { return /edit station/i.test(txt(a)); });
      if (editBtn && row.editHref) editBtn.href = row.editHref;

      qsa("a", card).forEach(function (a) {
        if (/open display/i.test(txt(a)) && (!a.href || a.href.indexOf("/admin/kds_stations/edit/") !== -1)) {
          a.style.display = "none";
        }
      });
    });

    var newBtn = qsa("a,button", root || document).find(function (el) { return /new kds station/i.test(txt(el)); });
    if (newBtn && newBtn.tagName === "A") {
      newBtn.href = location.origin + "/admin/kds_stations/create";
    }

    return { table: parsed.table, rows: parsed.rows, root: root, cards: cardList };
  }
  ...
  function check() {
    ...
    if (!r.table) issues.push({ severity: "FAIL", type: "ORIGINAL_TABLE_NOT_FOUND" });
    if (!r.root) issues.push({ severity: "FAIL", type: "CARD_SHELL_NOT_FOUND" });

    if (r.rows.length !== r.cards.length) {
      issues.push({ severity: "FAIL", type: "ROW_CARD_COUNT_MISMATCH", rows: r.rows.length, cards: r.cards.length });
    }
    ...
    r.cards.forEach(function (card, i) {
      ...
      var shownName = txt(card.querySelector(".pmd962-name,.pmd961-name"));
      var shownSlug = txt(card.querySelector(".pmd962-slug,.pmd961-slug"));

      if (shownName !== row.name) {
        issues.push({ severity: "FAIL", type: "CARD_NAME_WRONG", card: i + 1, expected: row.name, shown: shownName });
      }

      if (row.slug && shownSlug !== row.slug) {
        issues.push({ severity: "FAIL", type: "CARD_SLUG_WRONG", card: i + 1, expected: row.slug, shown: shownSlug });
      }

      if (!isEnabled(row.canUpdate)) issues.push({ severity: "WARN", type: "CAN_UPDATE_BAD", card: i + 1, value: row.canUpdate });
      if (!isEnabled(row.active)) issues.push({ severity: "WARN", type: "ACTIVE_BAD", card: i + 1, value: row.active });
      if (!isNum(row.priority)) issues.push({ severity: "WARN", type: "PRIORITY_BAD", card: i + 1, value: row.priority });
    });
    ...
    var summary = {
      ...
      bodyHasV962: document.body.classList.contains("pmd-kds-stations-v962"),
      bodyHasV964Final: document.body.classList.contains("pmd-kds-stations-v964-final"),
      v962Loaded: !!window.PMDKdsStationsIndexV962,
      shellExists: !!r.root,
      tableExists: !!r.table,
      rows: r.rows.length,
      cards: r.cards.length,
      failIssues: issues.filter(function (x) { return x.severity === "FAIL"; }).length,
      warnIssues: issues.filter(function (x) { return x.severity === "WARN"; }).length,
      totalIssues: issues.length,
      status: issues.some(function (x) { return x.severity === "FAIL"; }) ? "FAIL_NEEDS_FIX" : issues.length ? "WARN_REVIEW" : "OK"
    };

    window.PMD_KDS_STATIONS_INDEX_V96_4_FINAL_REPORT = {
      summary: summary,
      issues: issues,
      rows: r.rows
    };

    console.log("✅ PMD KDS STATIONS INDEX v96.4 FINAL SUMMARY");
    console.table([summary]);
    console.log("📌 ISSUES");
    console.table(issues);
    console.log("📌 ROWS");
    console.table(r.rows);
    console.log("Copy summary: copy(JSON.stringify(window.PMD_KDS_STATIONS_INDEX_V96_4_FINAL_REPORT.summary, null, 2))");
    console.log("Copy issues: copy(JSON.stringify(window.PMD_KDS_STATIONS_INDEX_V96_4_FINAL_REPORT.issues, null, 2))");
    console.log("Copy rows: copy(JSON.stringify(window.PMD_KDS_STATIONS_INDEX_V96_4_FINAL_REPORT.rows, null, 2))");

    return window.PMD_KDS_STATIONS_INDEX_V96_4_FINAL_REPORT;
  }

  function scheduleApply() {
    clearTimeout(APPLY_TIMER);
    APPLY_TIMER = setTimeout(function () {
      try { apply(); } catch (e) { console.error("[PMD] v96.4 final apply failed", e); }
    }, 80);
  }

  window.PMDKdsStationsIndexV964Final = {
    apply: apply,
    check: check,
    copySummary: function () { copy(JSON.stringify(window.PMD_KDS_STATIONS_INDEX_V96_4_FINAL_REPORT.summary, null, 2)); },
    copyIssues: function () { copy(JSON.stringify(window.PMD_KDS_STATIONS_INDEX_V96_4_FINAL_REPORT.issues, null, 2)); },
    copyRows: function () { copy(JSON.stringify(window.PMD_KDS_STATIONS_INDEX_V96_4_FINAL_REPORT.rows, null, 2)); }
  };
  ...
  console.info("[PMD] KDS stations index v96.4 final active", { mark: MARK });
})();
/* PMD_KDS_STATIONS_INDEX_V96_4_FINAL_END */


/* PMD_KDS_V102_FORM_BRIDGE_AND_INDEX_CLEANUP_START */
(function () {
  var MARK = 'PMD_KDS_V102_FORM_BRIDGE_AND_INDEX_CLEANUP';
  var STYLE_ID = 'pmd-kds-v102-style';
  var CLONE_CLASS = 'pmd-kds-v102-clone';

  function pathClean() { return location.pathname.replace(/\/+$/, ''); }
  function isIndex() { return pathClean() === '/admin/kds_stations'; }
  function isSettings() { return /\/admin\/kds_stations\/(create|edit)/.test(pathClean()); }
  function qsa(sel, root) { return Array.prototype.slice.call((root || document).querySelectorAll(sel)); }
  function txt(el) { return ((el && (el.innerText || el.textContent)) || '').replace(/\s+/g, ' ').trim(); }
  function visible(el) { if (!el) return false; var cs=getComputedStyle(el), r=el.getBoundingClientRect(); return cs.display !== 'none' && cs.visibility !== 'hidden' && Number(cs.opacity) !== 0 && r.width > 0 && r.height > 0; }
  function isKdsControl(el) { return !!(el && el.name && String(el.name).indexOf('Kds_station[') === 0); }
  ...
  function installStyle() {
    ...
    s.id = STYLE_ID;
    s.textContent = [
      'body.pmd-kds-v102-index{overflow-x:hidden!important;}',
      'body.pmd-kds-v102-index .pmd962-shell,body.pmd-kds-v102-index .pmd961-shell{width:100%!important;max-width:100%!important;overflow-x:hidden!important;box-sizing:border-box!important;}',
      'body.pmd-kds-v102-index .pmd962-shell *,body.pmd-kds-v102-index .pmd961-shell *{box-sizing:border-box!important;min-width:0!important;}',
      'body.pmd-kds-v102-index .pmd962-grid,body.pmd-kds-v102-index .pmd961-grid{grid-template-columns:repeat(auto-fit,minmax(280px,1fr))!important;}',
      'body.pmd-kds-v102-index .pmd962-name,body.pmd-kds-v102-index .pmd961-name,body.pmd-kds-v102-index .pmd962-slug,body.pmd-kds-v102-index .pmd961-slug,body.pmd-kds-v102-index .pmd962-detail-value,body.pmd-kds-v102-index .pmd961-detail-value{overflow:hidden!important;text-overflow:ellipsis!important;white-space:nowrap!important;}',
      'body.pmd-kds-v102-index a[data-pmd-invalid-display="1"]{display:none!important;}',
      '@media(max-width:900px){body.pmd-kds-v102-index .pmd962-hero,body.pmd-kds-v102-index .pmd961-hero{flex-direction:column!important;align-items:stretch!important;}body.pmd-kds-v102-index .pmd962-stats,body.pmd-kds-v102-index .pmd961-stats{grid-template-columns:repeat(2,minmax(0,1fr))!important;}body.pmd-kds-v102-index .pmd962-grid,body.pmd-kds-v102-index .pmd961-grid{grid-template-columns:1fr!important;}}',
      '@media(max-width:560px){body.pmd-kds-v102-index .pmd962-stats,body.pmd-kds-v102-index .pmd961-stats,body.pmd-kds-v102-index .pmd962-details,body.pmd-kds-v102-index .pmd961-details{grid-template-columns:1fr!important;}body.pmd-kds-v102-index .pmd962-btn,body.pmd-kds-v102-index .pmd961-btn{width:100%!important;}}'
    ].join('\n');
    document.head.appendChild(s);
  }
  ...
  function cleanIndex() {
    if (!isIndex()) return;
    installStyle();
    document.body.classList.add('pmd-kds-v102-index');

    var root = document.querySelector('.pmd962-shell,.pmd961-shell');
    if (!root) return;

    qsa('a,button', root).forEach(function (el) {
      if (/new kds station/i.test(txt(el)) && el.tagName === 'A') el.href = location.origin + '/admin/kds_stations/create';
    });

    qsa('.pmd962-card,.pmd961-card', root).forEach(function (card) {
      var slug = txt(card.querySelector('.pmd962-slug,.pmd961-slug')).toLowerCase();

      qsa('a', card).forEach(function (a) {
        var label = txt(a).toLowerCase();
        var href = String(a.href || '');
        var bad = !href || /\/admin\/kitchendisplay\/?$/.test(href) || href.indexOf('/admin/kitchendisplay/-') !== -1 || !slug || slug === 'no slug' || slug === '-1' || slug === '-2';

        if (label.indexOf('open display') !== -1 && bad) {
          a.setAttribute('data-pmd-invalid-display', '1');
  ...
  function findKdsShell() {
    return document.querySelector('.pmd92-shell') || document.querySelector('.pmd-kds-modern-card-v58') || document.querySelector('.pmd-kds-settings-v57');
  }

  function findTargetForm() {
    var forms = qsa('form');
    var current = pathClean();

    var scored = forms.map(function (f) {
      var actionPath = '';
      try { actionPath = new URL(f.action || location.href, location.href).pathname.replace(/\/+$/, ''); } catch(e) {}

      var score = 0;
      if ((f.method || '').toLowerCase() === 'post') score += 20;
      if (/\/admin\/kds_stations\/(create|edit)/.test(actionPath)) score += 50;
      if (actionPath === current) score += 20;
      if (qsa('input[name="_token"],input[name="csrf_token"]', f).length) score += 10;

      return { form: f, score: score };
    }).sort(function(a,b){ return b.score - a.score; });

    return scored.length && scored[0].score > 0 ? scored[0].form : null;
  }

  function payloadKeys(form) {
    var keys = [];
    if (!form) return keys;
    var fd = new FormData(form);
    for (var pair of fd.entries()) if (keys.indexOf(pair[0]) === -1) keys.push(pair[0]);
    return keys;
  }

  function formHasKdsPayload(form) {
    return payloadKeys(form).some(function(k){ return k.indexOf('Kds_station[') === 0; });
  }

  function cloneControlToForm(form, el) {
    if (!form || !isKdsControl(el) || el.disabled) return;
    ...
    if (tag === 'select' && el.multiple) {
      qsa('option', el).forEach(function (op) {
        if (!op.selected) return;
        var h = document.createElement('input');
        h.type = 'hidden';
        h.name = el.name;
        h.value = op.value;
        h.className = CLONE_CLASS;
        form.appendChild(h);
      });
      return;
    }
  ...
  function prepareKdsForm() {
    if (!isSettings()) return null;

    var shell = findKdsShell();
    var form = findTargetForm();

    if (!shell || !form) return { shell: shell, form: form, controls: [] };

    if (!form.id) form.id = 'pmd-kds-form-bridge-' + Date.now();

    var controls = qsa('input,select,textarea', shell).filter(isKdsControl);

    qsa('.' + CLONE_CLASS, form).forEach(function (el) { el.remove(); });

    controls.forEach(function (el) {
      el.setAttribute('form', form.id);
    });

    qsa('button,input', shell).forEach(function (el) {
      if (/save|create|update/i.test(txt(el) || el.value || '') || el.type === 'submit') {
        el.setAttribute('form', form.id);
      }
    });

    return { shell: shell, form: form, controls: controls };
  }

  function syncClones(form, controls) {
    if (!form) return;
    qsa('.' + CLONE_CLASS, form).forEach(function (el) { el.remove(); });
    controls.forEach(function (el) { cloneControlToForm(form, el); });
  }

  function bridgeSettings() {
    var st = prepareKdsForm();
    if (!st || !st.form) return st;

    if (!formHasKdsPayload(st.form)) syncClones(st.form, st.controls || []);

    if (!st.form.getAttribute('data-pmd-kds-v102-bridged')) {
      st.form.setAttribute('data-pmd-kds-v102-bridged', '1');

      st.form.addEventListener('submit', function () {
        var fresh = prepareKdsForm();
        qsa('.' + CLONE_CLASS, st.form).forEach(function (el) { el.remove(); });
        if (!formHasKdsPayload(st.form)) syncClones(st.form, fresh && fresh.controls ? fresh.controls : []);
      }, true);
    }
  ...
  function check() {
    ...
    if (isIndex()) {
      ...
      qsa('a', root || document).forEach(function (a) {
        if (!visible(a)) return;

        var href = String(a.href || '');

        if (/open display/i.test(txt(a)) && (/\/admin\/kitchendisplay\/?$/.test(href) || href.indexOf('/admin/kitchendisplay/-') !== -1)) {
          issues.push({ type: 'INVALID_OPEN_DISPLAY_VISIBLE', href: href });
        }
      });

      if (document.documentElement.scrollWidth > innerWidth + 8) {
        issues.push({ type: 'REAL_HORIZONTAL_OVERFLOW', scrollWidth: document.documentElement.scrollWidth, viewport: innerWidth });
      }

      summary.pageType = 'index';
      summary.cards = qsa('.pmd962-card,.pmd961-card', root || document).filter(visible).length;
      summary.bodyHasV102 = document.body.classList.contains('pmd-kds-v102-index');

    } else if (isSettings()) {
      var st = bridgeSettings();
      var keys = payloadKeys(st && st.form);
      var orphans = (st && st.controls ? st.controls : []).filter(function (el) {
        return !el.form || el.form !== st.form;
      });

      if (!st || !st.shell) issues.push({ type: 'KDS_SHELL_NOT_FOUND' });
      if (!st || !st.form) issues.push({ type: 'TARGET_FORM_NOT_FOUND' });
      if (!keys.some(function(k){ return k.indexOf('Kds_station[') === 0; })) issues.push({ type: 'KDS_PAYLOAD_MISSING', keys: keys });
      if (orphans.length) issues.push({ type: 'KDS_CONTROLS_STILL_ORPHAN', count: orphans.length });

      summary.pageType = 'settings';
      summary.kdsControls = st && st.controls ? st.controls.length : 0;
      summary.payloadKeys = keys.length;
      summary.kdsPayloadKeys = keys.filter(function(k){ return k.indexOf('Kds_station[') === 0; }).length;
      summary.orphanControls = orphans.length;
      summary.formAction = st && st.form ? st.form.action : null;

    } else {
      summary.pageType = 'other';
    }

    summary.totalIssues = issues.length;
    summary.status = issues.length ? 'NEEDS_FIX' : 'OK';

    window.PMD_KDS_V102_REPORT = { summary: summary, issues: issues };

    console.log('✅ PMD KDS v102 SUMMARY');
    console.table([summary]);
    console.log('📌 ISSUES');
    console.table(issues);

    return window.PMD_KDS_V102_REPORT;
  }

  window.PMDKdsV102 = {
    apply: function(){ cleanIndex(); bridgeSettings(); },
    check: check
  };
  ...
  console.info('[PMD] KDS v102 form bridge + index cleanup active', { mark: MARK });
})();
/* PMD_KDS_V102_FORM_BRIDGE_AND_INDEX_CLEANUP_END */


/* PMD_KDS_V105_SUBMIT_FIX_START */
(function () {
  var MARK = 'PMD_KDS_V105_SUBMIT_FIX';
  var FALLBACK_CLONE_CLASS = 'pmd-kds-v105-fallback-clone';
  ...
  function isSettings() {
    return /\/admin\/kds_stations\/(create|edit)/.test(pathClean());
  }
  ...
  function isKdsControl(el) {
    return !!(el && el.name && String(el.name).indexOf('Kds_station[') === 0);
  }

  function findShell() {
    return document.querySelector('.pmd92-shell') ||
           document.querySelector('.pmd-kds-modern-card-v58') ||
           document.querySelector('.pmd-kds-settings-v57');
  }

  function findTargetForm() {
    var forms = qsa('form');

    var scored = forms.map(function (form) {
      var actionPath = '';

      try {
        actionPath = new URL(form.action || location.href, location.href).pathname.replace(/\/+$/, '');
      } catch (e) {}

      var score = 0;

      if ((form.method || '').toLowerCase() === 'post') score += 30;
      if (/\/admin\/kds_stations\/(create|edit)/.test(actionPath)) score += 80;
      if (actionPath === pathClean()) score += 30;
      if (qsa('input[name="_token"]', form).length) score += 10;

      return { form: form, score: score, actionPath: actionPath };
    }).sort(function (a, b) {
  ...
  function getControls() {
    var shell = findShell();
    return shell ? qsa('input,select,textarea', shell).filter(isKdsControl) : [];
  }

  function removeOldClones(form) {
    if (!form) return;

    qsa('.pmd-kds-v102-clone,.pmd-kds-v104-submit-clone,.' + FALLBACK_CLONE_CLASS, form).forEach(function (el) {
      el.remove();
    });
  }

  function associateControls(form, controls) {
    if (!form) return;

    if (!form.id) form.id = 'pmd-kds-v105-form-' + Date.now();

    controls.forEach(function (el) {
      el.setAttribute('form', form.id);
    });

    qsa('button,input[type="submit"]', document).forEach(function (el) {
      var label = [
        txt(el),
      ...
      if (
        ...
        label.indexOf('fa-save') !== -1 ||
        label.indexOf('ti-save') !== -1 ||
        label.indexOf('ri-save') !== -1 ||
        label.indexOf('mdi-content-save') !== -1
      ) {
        el.setAttribute('form', form.id);
        if (el.tagName === 'BUTTON') el.setAttribute('type', 'submit');
      }
    });
  }
  ...
  function kdsKeysFromPayload(out) {
    return Object.keys(out).filter(function (k) {
      return k.indexOf('Kds_station[') === 0;
    });
  }
  ...
  function addHidden(form, name, value) {
    ...
    h.name = name;
    h.value = value;
    h.className = FALLBACK_CLONE_CLASS;
    form.appendChild(h);
  }

  function fallbackCloneOnlyIfMissing(form, controls) {
    removeOldClones(form);

    var before = payload(form);
    var beforeKeys = kdsKeysFromPayload(before);

    if (beforeKeys.length) return 0;
    ...
    controls.forEach(function (el) {
      ...
      if (tag === 'select' && el.multiple) {
        qsa('option', el).forEach(function (op) {
          if (!op.selected) return;
          addHidden(form, el.name, op.value);
          cloned++;
        });
        return;
      }

      addHidden(form, el.name, el.value);
      cloned++;
    });
  ...
  function isAllowedDuplicateKey(k) {
    return (
      k.indexOf('[]') !== -1 ||
      /\[is_active\]|\[can_change_status\]|\[show_reservations\]|\[sound_enabled\]/.test(k)
    );
  }

  function duplicateScalarProblems(out) {
    return Object.keys(out)
      .filter(function (k) {
        return k.indexOf('Kds_station[') === 0 && !isAllowedDuplicateKey(k) && out[k].length > 1;
      })
      .map(function (k) {
        return { key: k, values: out[k] };
      });
  }
  ...
  function prepare() {
    ...
    removeOldClones(form);
    associateControls(form, controls);

    var fallbackCloned = fallbackCloneOnlyIfMissing(form, controls);

    return {
      form: form,
      controls: controls,
      fallbackCloned: fallbackCloned,
      out: payload(form)
    };
  }

  function isSaveTarget(target) {
    var node = target && target.closest ? target.closest('button,a,input[type="submit"],[role="button"]') : null;
    if (!node) return null;
    ...
    if (label.indexOf('back') !== -1) return null;
    if (label.indexOf('delete') !== -1) return null;
  ...
  function installSubmitHandlers() {
    ...
    if (!form.getAttribute('data-pmd-kds-v105-submit-hook')) {
      form.setAttribute('data-pmd-kds-v105-submit-hook', '1');
    ...
    if (!document.body.getAttribute('data-pmd-kds-v105-click-hook')) {
      document.body.setAttribute('data-pmd-kds-v105-click-hook', '1');

      document.addEventListener('click', function (ev) {
        var node = isSaveTarget(ev.target);
        if (!node) return;
        ...
        if (node.tagName === 'A') {
          ...
          if (st.form.requestSubmit) st.form.requestSubmit();
          else st.form.submit();
        }
      }, true);
    }
  ...
  function check() {
    ...
    var st = prepare();
    var out = st.out || {};
    var kdsKeys = kdsKeysFromPayload(out);
    var duplicateScalars = duplicateScalarProblems(out);

    var orphans = (st.controls || []).filter(function (el) {
      return !el.form || el.form !== st.form;
    });

    var issues = [];

    if (!findShell()) issues.push({ type: 'KDS_SHELL_NOT_FOUND' });
    if (!st.form) issues.push({ type: 'TARGET_FORM_NOT_FOUND' });
    if (!st.controls.length) issues.push({ type: 'NO_KDS_CONTROLS_FOUND' });
    if (!kdsKeys.length) issues.push({ type: 'KDS_PAYLOAD_MISSING' });
    if (orphans.length) issues.push({ type: 'KDS_CONTROLS_ORPHAN', count: orphans.length });
    if (duplicateScalars.length) issues.push({ type: 'DUPLICATE_SCALAR_KDS_FIELDS', fields: duplicateScalars });

    var summary = {
      mark: MARK,
      path: location.pathname,
      viewport: innerWidth + 'x' + innerHeight,
      formAction: st.form ? st.form.action : null,
      kdsControls: st.controls.length,
      payloadKeys: Object.keys(out).length,
      kdsPayloadKeys: kdsKeys.length,
      fallbackCloned: st.fallbackCloned,
      orphanControls: orphans.length,
      duplicateScalarFields: duplicateScalars.length,
      totalIssues: issues.length,
      status: issues.length ? 'NEEDS_FIX' : 'OK'
    };

    window.PMD_KDS_V105_SUBMIT_FIX_REPORT = {
      summary: summary,
      issues: issues,
      payload: out,
      kdsKeys: kdsKeys
    ...
    console.log('✅ PMD KDS v105 SUBMIT FIX SUMMARY');
    console.table([summary]);
    console.log('📌 ISSUES');
    console.table(issues);
    console.log('📌 PAYLOAD');
    ...
    return window.PMD_KDS_V105_SUBMIT_FIX_REPORT;
  }

  window.PMDKdsV105SubmitFix = {
    apply: function () {
      installSubmitHandlers();
      prepare();
    },
  ...
  console.info('[PMD] KDS v105 submit fix active', { mark: MARK });
})();
/* PMD_KDS_V105_SUBMIT_FIX_END */

/* PMD_KDS_V110_REMOVE_STATUS_LEAK_START */
(function () {
  var MARK = 'PMD_KDS_V110_REMOVE_STATUS_LEAK';

  function isKdsSettings() {
    return /\/admin\/kds_stations\/(create|edit)/.test(location.pathname.replace(/\/+$/, ''));
  }

  if (!isKdsSettings()) return;

  function qsa(sel, root) {
    return Array.prototype.slice.call((root || document).querySelectorAll(sel));
  }

  function findKdsForm() {
    return qsa('form').find(function (f) {
      return String(f.action || '').indexOf('/admin/kds_stations') !== -1 && String(f.method || '').toLowerCase() === 'post';
    }) || qsa('form').find(function (f) {
      return String(f.action || '').indexOf('/admin/kds_stations') !== -1;
    }) || null;
  }

  function disableLeaks() {
    var form = findKdsForm();
    if (!form) return { form: null, disabled: 0 };

    var disabled = 0;

    qsa('input[name="status"],textarea[name="message"],input[name="message"],select[name="status"],input[name="clear_after"],select[name="clear_after"],textarea[name="clear_after"]', form)
      .forEach(function (el) {
        el.setAttribute('data-pmd-kds-v110-disabled-leak', '1');
        el.disabled = true;
        disabled++;
      });

    return { form: form, disabled: disabled };
  }
  ...
  function payloadKeys(form) {
    ...
    var fd = new FormData(form);
    for (var pair of fd.entries()) {
      if (keys.indexOf(pair[0]) === -1) keys.push(pair[0]);
    }
    return keys;
  }

  function check() {
    if (window.PMDKdsV105SubmitFix && window.PMDKdsV105SubmitFix.apply) {
      window.PMDKdsV105SubmitFix.apply();
    }

    var st = disableLeaks();
    var keys = payloadKeys(st.form);
    var leakKeys = keys.filter(function (k) {
      return ['status', 'message', 'clear_after'].indexOf(k) !== -1;
    });
    var kdsKeys = keys.filter(function (k) {
      return k.indexOf('Kds_station[') === 0;
    });

    var issues = [];
    if (!st.form) issues.push({ type: 'KDS_FORM_NOT_FOUND' });
    if (!kdsKeys.length) issues.push({ type: 'KDS_KEYS_MISSING' });
    if (leakKeys.length) issues.push({ type: 'STATUS_LEAK_STILL_PRESENT', leakKeys: leakKeys });

    var summary = {
      mark: MARK,
      path: location.pathname,
      formAction: st.form ? st.form.action : null,
      disabledLeaks: st.disabled,
      keys: keys.length,
      kdsKeys: kdsKeys.length,
      leakKeys: leakKeys,
      totalIssues: issues.length,
      status: issues.length ? 'NEEDS_FIX' : 'OK'
    };

    window.PMD_KDS_V110_REMOVE_STATUS_LEAK_REPORT = {
      summary: summary,
      issues: issues,
      keys: keys
    };

    console.log('✅ PMD KDS v110 REMOVE STATUS LEAK SUMMARY');
    console.table([summary]);
    console.log('📌 ISSUES');
    console.table(issues);

    return window.PMD_KDS_V110_REMOVE_STATUS_LEAK_REPORT;
  }
  ...
  document.addEventListener('click', function (e) {
    var node = e.target && e.target.closest ? e.target.closest('a,button,input,[role="button"]') : null;
    if (!node) return;
  ...
  window.PMDKdsV110RemoveStatusLeak = {
    apply: disableLeaks,
    check: check
  };
  ...
  console.info('[PMD] KDS v110 remove status leak active', { mark: MARK });
})();
/* PMD_KDS_V110_REMOVE_STATUS_LEAK_END */





/* PMD_KDS_V120_CSRF_NATIVE_SUBMIT_START */
(function () {
  var MARK = 'PMD_KDS_V120_CSRF_NATIVE_SUBMIT';
  ...
  function payload(form) {
    ...
    try {
      var fd = new FormData(form);
      for (var pair of fd.entries()) {
        if (!out[pair[0]]) out[pair[0]] = [];
        out[pair[0]].push(String(pair[1]));
      }
    } catch (e) {}
  ...
  function findKdsForm() {
    return qsa('form').find(function (form) {
      var action = String(form.action || '');
      var method = String(form.method || '').toLowerCase();
      var p = payload(form);

      return action.indexOf('/admin/kds_stations') !== -1 &&
        method === 'post' &&
        Object.keys(p).some(function (k) { return k.indexOf('Kds_station[') === 0; });
    }) || null;
  }
  ...
  function getCsrf() {
    ...
    var input = document.querySelector('input[name="_token"]:not([disabled])');
    if (input && input.value) return input.value;

    var anyInput = document.querySelector('input[name="_token"]');
  ...
  function ensureCsrf(form) {
    var token = getCsrf();
    if (!form || !token) return { ok: false, tokenPresent: !!token };
    ...
    return { ok: true, tokenPresent: true, valueLength: token.length };
  }
  ...
  function prepareForm() {
    ...
    if (window.PMDKdsV110RemoveStatusLeak && window.PMDKdsV110RemoveStatusLeak.apply) {
      window.PMDKdsV110RemoveStatusLeak.apply();
    }
  ...
  function check() {
    var form = prepareForm();
    var p = payload(form);
    var csrf = ensureCsrf(form);

    var kdsKeys = Object.keys(p).filter(function (k) { return k.indexOf('Kds_station[') === 0; });
    var leakKeys = Object.keys(p).filter(function (k) { return ['status', 'message', 'clear_after'].indexOf(k) !== -1; });

    var issues = [];
    if (!form) issues.push({ type: 'KDS_FORM_NOT_FOUND' });
    if (!csrf.ok) issues.push({ type: 'CSRF_TOKEN_MISSING' });
    if (!kdsKeys.length) issues.push({ type: 'KDS_KEYS_MISSING' });
    if (leakKeys.length) issues.push({ type: 'STATUS_LEAK_PRESENT', leakKeys: leakKeys });
    ...
    window.PMD_KDS_V120_CSRF_NATIVE_SUBMIT_REPORT = {
      summary: summary,
      issues: issues,
      payload: p
    };

    console.log('✅ PMD KDS v120 CSRF NATIVE SUBMIT SUMMARY');
    console.table([summary]);
    console.log('📌 ISSUES');
    console.table(issues);

    return window.PMD_KDS_V120_CSRF_NATIVE_SUBMIT_REPORT;
  }

  function submitClean(clickedText) {
    if (submitting) return false;

    var form = prepareForm();
    var report = check();

    window.PMD_KDS_V120_LAST_SUBMIT = {
      mark: MARK,
    ...
    console.log('🚀 PMD KDS v120 submit', window.PMD_KDS_V120_LAST_SUBMIT);

    if (!form || report.summary.status !== 'OK') {
      alert('KDS form is not ready. Check console PMD_KDS_V120_CSRF_NATIVE_SUBMIT_REPORT.');
      return false;
    }
  ...
  window.addEventListener('click', function (ev) {
    var node = ev.target && ev.target.closest ? ev.target.closest('a,button,input,[role="button"]') : null;
    if (!isSaveLike(node)) return;

    ev.preventDefault();
    ev.stopPropagation();
    ev.stopImmediatePropagation();

    submitClean(text(node));

    return false;
  }, true);

  window.PMDKdsV120CsrfNativeSubmit = {
    check: check,
    submit: submitClean
  };
  ...
  console.info('[PMD] KDS v120 csrf native submit active', { mark: MARK });
})();
