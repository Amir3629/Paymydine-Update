...
+function ($) {
    ...
    /*
     * BULLETPROOF CALENDAR MODAL SYSTEM - PERFECT SETTINGS THAT NEVER BREAK
     * This is the FINAL solution with the correct proportions
     */
    console.log('🛡️ BULLETPROOF CALENDAR MODAL SYSTEM INITIALIZING...')

    class BulletproofCalendarModalSystem {
        constructor() {
            this.isActive = false;
            this.calendar = null;
        ...
        init() {
            console.log('🛡️ Bulletproof Calendar Modal System initialized');
            this.watchForCalendar();
        }
        
        watchForCalendar() {
            // Listen for calendar button clicks first
            document.addEventListener('click', (e) => {
                if (e.target.closest('[data-control="daterange"]')) {
                    console.log('🛡️ Calendar button clicked - waiting for calendar...');
                    // Wait a bit for calendar to appear, then apply bulletproof settings
                    setTimeout(() => {
                        const calendar = document.querySelector('.daterangepicker');
                        if (calendar && !this.isActive) {
                            console.log('🛡️ Calendar detected after button click - applying bulletproof settings...');
            ...
            // Also watch for calendar creation (as backup)
            const observer = new MutationObserver((mutations) => {
                mutations.forEach((mutation) => {
                    if (mutation.type === 'childList') {
                        const calendar = document.querySelector('.daterangepicker');
                        if (calendar && !this.isActive) {
                            // Only open if it's a new calendar that appeared after a click
                            console.log('🛡️ Calendar detected - checking if button was clicked...');
                            // Don't auto-open, wait for button click
                        } else if (!calendar && this.isActive) {
                            this.closeModal();
                        }
                    }
                });
        ...
        openModal(calendar) {
            ...
            console.log('🛡️ Opening calendar modal with bulletproof settings...');
            
            this.isActive = true;
            this.calendar = calendar;
            this.originalParent = calendar.parentNode;
            
            // ANTI-FLASH: Ensure calendar is hidden before moving
            calendar.classList.remove('positioned');
            ...
            // Add modal-open class to body (like setup card)
            document.body.classList.add('modal-open');
            
            // Apply bulletproof styling
            this.applyBulletproofStyling();
            
            // Center the calendar
            this.centerCalendar();
            
            // ANTI-FLASH: Show calendar only after positioning is complete
            requestAnimationFrame(() => {
                requestAnimationFrame(() => {
                    if (this.calendar) {
                        this.calendar.classList.add('positioned');
                        this.calendar.style.opacity = '1';
                        this.calendar.style.visibility = 'visible';
                        console.log('✅ Calendar visible after positioning');
                    }
                });
            });
            ...
            console.log('✅ Calendar modal opened with bulletproof settings');
        }
        
        applyBulletproofStyling() {
            if (!this.calendar) return;
            
            console.log('🔧 Applying BULLETPROOF settings...');
            
            // 1. OVERALL CALENDAR - PERFECT SIZE
            ...
            // 2. RANGES PANEL - PERFECT WIDTH
            const ranges = this.calendar.querySelector('.ranges');
            if (ranges) {
                ...
                // Range items
                const rangeItems = ranges.querySelectorAll('li');
            ...
            // 3. CALENDAR GRIDS - PERFECT SIZE
            const calendars = this.calendar.querySelectorAll('.drp-calendar');
            calendars.forEach((cal, index) => {
                ...
                // Calendar table
                const table = cal.querySelector('.calendar-table');
                if (table) {
                    table.style.cssText = `
                ...
                // Calendar time
                const timeDiv = cal.querySelector('.calendar-time');
                if (timeDiv) {
            ...
            // 4. BUTTONS - PERFECT SIZE
            const buttons = this.calendar.querySelectorAll('.btn');
            ...
            // 5. BUTTONS CONTAINER - PERFECT SPACING
            const buttonsContainer = this.calendar.querySelector('.drp-buttons');
            if (buttonsContainer) {
            ...
            // 6. CALENDAR CONTAINER - PERFECT LAYOUT
            const calendarContainer = this.calendar.querySelector('.drp-calendar-container');
            ...
            // 7. FORCE ALL ELEMENTS TO HAVE PROPER SPACING
            const allElements = this.calendar.querySelectorAll('*');
            allElements.forEach(el => {
                const tagName = el.tagName.toLowerCase();
                
                // Add spacing to all divs
                if (tagName === 'div' && !el.classList.contains('ranges') && !el.classList.contains('drp-calendar')) {
                    el.style.padding = '8px !important';
                }
                
                // Fix table spacing
                if (tagName === 'table') {
                    el.style.borderSpacing = '4px !important';
                    el.style.borderCollapse = 'separate !important';
                }
                
                // Fix row spacing
                if (tagName === 'tr') {
                    el.style.height = 'auto !important';
                    el.style.minHeight = '36px !important';
                }
            });
            
            // 8. FORCE THE ENTIRE CALENDAR TO BE WIDER
            const calendarBody = this.calendar.querySelector('.drp-calendar-container') || this.calendar;
            if (calendarBody) {
            ...
            // 9. FORCE RIGHT SIDE TO HAVE MORE SPACE
            const rightSide = this.calendar.querySelector('.drp-calendar.right');
            if (rightSide) {
            ...
            // 10. FORCE THE ENTIRE CALENDAR TO NEVER SHRINK
            this.calendar.style.cssText += `
                min-width: 850px !important;
                max-width: 900px !important;
                width: auto !important;
            ...
            console.log('✅ BULLETPROOF settings applied!');
        }
        
        centerCalendar() {
            if (!this.calendar) return;
            
            // Position calendar directly under the button with some distance
            ...
            if (calendarWidth > 0 && calendarHeight > 0) {
                // Find the calendar button to position relative to it
                const calendarButton = document.querySelector('[data-control="daterange"]');
                let buttonRect = null;
                
                if (calendarButton) {
                    buttonRect = calendarButton.getBoundingClientRect();
                }
                
                // Calculate position - center horizontally, but position under button
                const centerX = (window.innerWidth - calendarWidth) / 2;
                let finalTop;
                
                if (buttonRect) {
                    // Position directly under the button with 10px distance
                    finalTop = buttonRect.bottom + 10;
                    
                    // Make sure it doesn't go off screen
                    const maxTop = window.innerHeight - calendarHeight - 20;
                    ...
                    const centerY = (window.innerHeight - calendarHeight) / 2;
                    finalTop = Math.max(centerY, 100);
                }
                
                // Apply BULLETPROOF positioning that won't jump
                this.calendar.style.cssText += `
                ...
                // Store the position for stability
        ...
        addClickOutsideListener() {
            this.outsideClickHandler = (e) => {
                // Only close if click is truly outside the calendar
                const isInsideCalendar = this.calendar.contains(e.target);
                const isCalendarButton = e.target.closest('[data-control="daterange"]');
                
                // Don't close if clicking inside calendar or on calendar button
                if (!isInsideCalendar && !isCalendarButton) {
                    console.log('🛡️ Click outside calendar - closing modal');
                    this.closeModal();
                } else if (isInsideCalendar) {
                    console.log('🛡️ Click inside calendar - keeping modal open');
                }
            };
            document.addEventListener('click', this.outsideClickHandler);
            
            // Add position monitoring to prevent jumping
            this.startPositionMonitoring();
        }
        
        startPositionMonitoring() {
            // Monitor and fix position every 50ms to prevent jumping (faster detection)
            this.positionMonitor = setInterval(() => {
                if (this.calendar && this.calendarPosition) {
                    const currentRect = this.calendar.getBoundingClientRect();
                    
                    // Check if position has changed (jumped)
                    if (Math.abs(currentRect.left - this.calendarPosition.left) > 10 || 
                        Math.abs(currentRect.top - this.calendarPosition.top) > 10) {
                        
                        console.log('🛡️ Calendar jumped - fixing position instantly...');
                        
                        // ANTI-FLASH: Hide calendar during repositioning
                        this.calendar.style.opacity = '0';
                        
                        // Force it back to correct position immediately
                        requestAnimationFrame(() => {
                            if (this.calendar) {
                                this.calendar.style.cssText += `
                                    position: fixed !important;
                                    left: ${this.calendarPosition.left}px !important;
                                    top: ${this.calendarPosition.top}px !important;
                                    z-index: 1050 !important;
                                `;
                                
                                // Show calendar after repositioning
                                requestAnimationFrame(() => {
                                    if (this.calendar) {
                                        this.calendar.style.opacity = '1';
                                    }
            ...
            // Also watch for DOM mutations that might cause position changes
            if (!this.mutationObserver) {
                this.mutationObserver = new MutationObserver((mutations) => {
                    mutations.forEach((mutation) => {
                        if (mutation.type === 'childList' || mutation.type === 'attributes') {
                            // If calendar structure changes, lock position immediately
                            if (this.calendar && this.calendarPosition) {
                                requestAnimationFrame(() => {
                                    if (this.calendar) {
                                        this.calendar.style.cssText += `
                ...
                this.mutationObserver.observe(this.calendar, {
                    childList: true,
                    subtree: true,
                    attributes: true,
        ...
        closeModal() {
            ...
            console.log('🛡️ Closing calendar modal...');
            ...
            if (this.calendar && this.originalParent) {
                this.originalParent.appendChild(this.calendar);
            }
            
            // Reset calendar styles and remove positioned class
            if (this.calendar) {
                this.calendar.classList.remove('positioned');
                this.calendar.style.position = '';
                this.calendar.style.left = '';
                this.calendar.style.top = '';
                this.calendar.style.zIndex = '';
                this.calendar.style.margin = '';
    ...
    // Initialize the bulletproof calendar modal system
    const bulletproofCalendarModalSystem = new BulletproofCalendarModalSystem();

    console.log('✅ BULLETPROOF CALENDAR MODAL SYSTEM ACTIVE!')
}(window.jQuery);

// Clean scrollbar arrow hiding solution
...
// Clean scrollbar arrow hiding - no excessive logging
document.addEventListener('DOMContentLoaded', function() {
    // Apply scrollbar hiding when calendar opens
    document.addEventListener('click', function(e) {
        if (e.target.closest('.daterangepicker') || e.target.closest('[data-toggle="daterangepicker"]')) {
            setTimeout(() => {
                hideScrollbarArrows();
            }, 100);
        }
...
// Nuclear scrollbar destroyer function
function nuclearScrollbarDestroyer() {
    // Method 1: Destroy scrollbar elements
    function destroyScrollbarElements() {
        const scrollbarElements = document.querySelectorAll('.daterangepicker *');
        scrollbarElements.forEach(element => {
            if (element.scrollHeight > element.clientHeight || element.scrollWidth > element.clientWidth) {
                element.style.setProperty('scrollbar-width', 'none', 'important');
                element.style.setProperty('scrollbar-color', 'transparent transparent', 'important');
            }
        });
    }
    
    // Method 2: Force override any scrollbar styles
    function forceOverrideScrollbarStyles() {
        const calendar = document.querySelector('.daterangepicker');
        if (calendar) {
            // Force override any existing scrollbar styles
            calendar.style.setProperty('scrollbar-width', 'none', 'important');
            calendar.style.setProperty('scrollbar-color', 'transparent transparent', 'important');
            ...
            console.log('💥 FORCED scrollbar style overrides!');
        }
    }
    
    // Method 4: Continuous monitoring and destruction
    function continuousDestruction() {
        setInterval(() => {
            destroyScrollbarElements();
            forceOverrideScrollbarStyles();
        }, 100); // Run every 100ms
    }
    ...
    console.log('💥 NUCLEAR DESTRUCTION COMPLETE!');
    console.log('💥 Continuous monitoring active - arrows will be destroyed!');
}

// Run immediately
nuclearScrollbarDestroyer();

// Also run when calendar opens
document.addEventListener('click', function(e) {
    if (e.target.closest('.daterangepicker') || e.target.closest('[data-toggle="daterangepicker"]')) {
        setTimeout(() => {
            nuclearScrollbarDestroyer();
...
// Run when DOM changes (calendar might be added dynamically)
...
console.log('💥 NUCLEAR SCROLLBAR DESTROYER ACTIVE!');
console.log('💥 All scrollbar arrows will be NUCLEAR DESTROYED!');

/**
 * REAL-TIME SCROLLBAR ARROW HUNTER
 * This script will hunt down and destroy scrollbar arrows in real-time
 */
console.log('🔍 REAL-TIME SCROLLBAR ARROW HUNTER INITIALIZING...');

function huntScrollbarArrows() {
    console.log('🔍 HUNTING SCROLLBAR ARROWS IN REAL-TIME...');
    
    const calendar = document.querySelector('.daterangepicker');
    if (!calendar) {
        console.log('❌ No calendar found - waiting for calendar to appear...');
        return;
    }
    
    console.log('📋 CALENDAR FOUND - Hunting scrollbar arrows...');
    
    // 1. Check if calendar has scrollable content
    console.log('🔍 CHECKING CALENDAR SCROLLABILITY:');
    
    const calendarRect = calendar.getBoundingClientRect();
    const scrollHeight = calendar.scrollHeight;
    const clientHeight = calendar.clientHeight;
    const scrollWidth = calendar.scrollWidth;
    const clientWidth = calendar.clientWidth;
    
    console.log(`   Calendar dimensions: ${calendarRect.width}x${calendarRect.height}`);
    ...
    console.log(`   Has vertical scroll: ${scrollHeight > clientHeight}`);
    console.log(`   Has horizontal scroll: ${scrollWidth > clientWidth}`);
    
    // 2. Check computed styles for scrollbar properties
    console.log('🔍 CHECKING SCROLLBAR COMPUTED STYLES:');
    ...
    console.log(`   Scrollbar-width: ${computedStyle.scrollbarWidth}`);
    console.log(`   Scrollbar-color: ${computedStyle.scrollbarColor}`);
    
    // 3. Try to detect webkit scrollbar elements
    console.log('🔍 DETECTING WEBKIT SCROLLBAR ELEMENTS:');
    
    // Create a test element to check webkit scrollbar properties
    ...
    testElement.style.cssText = `
        ...
        background: red;
    `;
    testElement.innerHTML = '<div style="width: 200px; height: 200px; background: blue;"></div>';
    document.body.appendChild(testElement);
    
    // Check if webkit scrollbar elements exist
    const webkitScrollbar = window.getComputedStyle(testElement, '::-webkit-scrollbar');
    const webkitScrollbarButton = window.getComputedStyle(testElement, '::-webkit-scrollbar-button');
    const webkitScrollbarTrack = window.getComputedStyle(testElement, '::-webkit-scrollbar-track');
    const webkitScrollbarThumb = window.getComputedStyle(testElement, '::-webkit-scrollbar-thumb');
    
    console.log(`   Webkit scrollbar width: ${webkitScrollbar.width}`);
    console.log(`   Webkit scrollbar button: ${webkitScrollbarButton.display}`);
    console.log(`   Webkit scrollbar track: ${webkitScrollbarTrack.display}`);
    console.log(`   Webkit scrollbar thumb: ${webkitScrollbarThumb.display}`);
    
    // Clean up test element
    document.body.removeChild(testElement);
    
    // 4. Check for any scrollbar-related elements in the calendar
    console.log('🔍 CHECKING FOR SCROLLBAR ELEMENTS IN CALENDAR:');
    ...
    allElements.forEach((element, index) => {
        ...
        // Check for scrollbar-like elements
        const isScrollbarElement = (
            className.includes('scrollbar') ||
            className.includes('scroll-arrow') ||
            className.includes('scroll-indicator') ||
            tagName === 'scrollbar' ||
            tagName === 'scrollbar-button' ||
            tagName === 'scrollbar-track' ||
            tagName === 'scrollbar-thumb' ||
            tagName === 'scrollbar-corner'
        );
        
        if (isScrollbarElement) {
    ...
    console.log(`   Found ${scrollbarElements.length} scrollbar elements in calendar`);
    scrollbarElements.forEach((element, index) => {
        console.log(`   Scrollbar Element ${index + 1}: ${element.tagName}.${element.className}`);
        console.log(`   Position: top=${element.position.top}, left=${element.position.left}`);
        console.log(`   Size: ${element.size.width}x${element.size.height}`);
        console.log(`   Display: ${element.display}, Visibility: ${element.visibility}, Opacity: ${element.opacity}`);
    });
    
    // 5. NUCLEAR DESTRUCTION of any found scrollbar elements
    console.log('💥 NUCLEAR DESTRUCTION OF SCROLLBAR ELEMENTS:');
    
    scrollbarElements.forEach((element, index) => {
        console.log(`💥 DESTROYING scrollbar element ${index + 1}: ${element.tagName}.${element.className}`);
        ...
        element.element.style.left = '-9999px';
        element.element.style.top = '-9999px';
        element.element.style.clip = 'rect(0,0,0,0)';
        element.element.style.overflow = 'hidden';
        element.element.style.transform = 'scale(0)';
        element.element.style.fontSize = '0';
        element.element.style.lineHeight = '0';
        element.element.style.zIndex = '-9999';
        
        // Try to remove from DOM
        try {
            element.element.remove();
            console.log(`💥 DESTROYED scrollbar element ${index + 1}`);
        } catch (e) {
            console.log(`💥 Could not remove element ${index + 1}, but styles applied`);
        }
    });
    
    // 6. Force hide scrollbar arrows with extreme CSS injection
    console.log('💥 INJECTING EXTREME SCROLLBAR ARROW HIDING CSS:');
    
    const extremeStyle = document.createElement('style');
    extremeStyle.id = 'extreme-scrollbar-hider';
    extremeStyle.textContent = `
        /* EXTREME SCROLLBAR ARROW DESTROYER */
        .daterangepicker::-webkit-scrollbar-button,
        .daterangepicker *::-webkit-scrollbar-button,
        .daterangepicker::-webkit-scrollbar-button:start:decrement,
        .daterangepicker::-webkit-scrollbar-button:end:increment,
        .daterangepicker *::-webkit-scrollbar-button:start:decrement,
        .daterangepicker *::-webkit-scrollbar-button:end:increment,
        .daterangepicker::-webkit-scrollbar-button:vertical:start:decrement,
        .daterangepicker::-webkit-scrollbar-button:vertical:end:increment,
        .daterangepicker::-webkit-scrollbar-button:horizontal:start:decrement,
        .daterangepicker::-webkit-scrollbar-button:horizontal:end:increment,
        .daterangepicker *::-webkit-scrollbar-button:vertical:start:decrement,
        .daterangepicker *::-webkit-scrollbar-button:vertical:end:increment,
        .daterangepicker *::-webkit-scrollbar-button:horizontal:start:decrement,
        .daterangepicker *::-webkit-scrollbar-button:horizontal:end:increment {
        ...
        /* Hide entire scrollbar if arrows persist */
    ...
    const existingStyle = document.getElementById('extreme-scrollbar-hider');
    if (existingStyle) {
        existingStyle.remove();
    }
    
    document.head.appendChild(extremeStyle);
    console.log('💥 EXTREME CSS INJECTED!');
    
    // 7. Force override scrollbar styles on calendar
    console.log('💥 FORCING SCROLLBAR STYLE OVERRIDES:');
    ...
    // Summary
    console.log('');
    console.log('📊 HUNTING SUMMARY:');
    console.log(`   Calendar scrollable: ${scrollHeight > clientHeight || scrollWidth > clientWidth}`);
    console.log(`   Webkit scrollbar support: ${webkitScrollbar.width !== '0px'}`);
    console.log(`   Scrollbar elements found: ${scrollbarElements.length}`);
    console.log(`   Scrollbar arrows: ${scrollbarElements.length > 0 ? 'Found and DESTROYED' : 'Browser-generated - CSS applied'}`);
    console.log('');
    
    return {
        calendarScrollable: scrollHeight > clientHeight || scrollWidth > clientWidth,
        webkitScrollbarSupport: webkitScrollbar.width !== '0px',
        scrollbarElementsFound: scrollbarElements.length,
        scrollbarArrowsLocation: scrollbarElements.length > 0 ? 'DOM' : 'Browser-generated'
    };
}

// Run the hunting immediately
huntScrollbarArrows();
...
// Continuous hunting every 500ms
setInterval(() => {
    if (document.querySelector('.daterangepicker')) {
        huntScrollbarArrows();
    }
...
window.huntScrollbarArrows = huntScrollbarArrows;

console.log('🔍 REAL-TIME SCROLLBAR ARROW HUNTER ACTIVE!');
console.log('🔍 Hunting scrollbar arrows every 500ms!');
console.log('💡 Run huntScrollbarArrows() manually to hunt immediately');

/**
 * STANDALONE SCROLLBAR ARROW INSPECTOR
 * This is the exact script you requested - inspect and destroy
 */
console.log('🔍 STANDALONE SCROLLBAR ARROW INSPECTOR INITIALIZING...');

function inspectBrowserScrollbars() {
    const calendar = document.querySelector('.daterangepicker');
    if (!calendar) {
    ...
    console.log('📋 CALENDAR FOUND - Inspecting browser scrollbars...');
    console.log('');
    ...
    const extremeStyle = document.createElement('style');
    extremeStyle.id = 'extreme-scrollbar-hider-standalone';
    extremeStyle.textContent = `
        /* EXTREME SCROLLBAR ARROW DESTROYER - STANDALONE */
        .daterangepicker::-webkit-scrollbar-button,
        .daterangepicker *::-webkit-scrollbar-button,
        .daterangepicker::-webkit-scrollbar-button:start:decrement,
        .daterangepicker::-webkit-scrollbar-button:end:increment,
    ...
    // Remove existing style if it exists
    const existingStyle = document.getElementById('extreme-scrollbar-hider-standalone');
    if (existingStyle) {
        existingStyle.remove();
    }
    ...
    // 8. Check if the scrollbar arrows are actually visible
    console.log('🔍 CHECKING SCROLLBAR ARROW VISIBILITY:');
    
    // Force the calendar to have scrollable content to see if scrollbars appear
    const originalOverflow = calendar.style.overflow;
    calendar.style.overflow = 'auto';
    calendar.style.maxHeight = '200px';
    
    // Wait a bit for the browser to render scrollbars
    setTimeout(() => {
        const newComputedStyle = window.getComputedStyle(calendar);
        console.log(`   After forcing scroll: overflow=${newComputedStyle.overflow}`);
        console.log(`   After forcing scroll: scrollbar-width=${newComputedStyle.scrollbarWidth}`);
        
        // Restore original overflow
        calendar.style.overflow = originalOverflow;
        calendar.style.maxHeight = '';
        
        console.log('   Scrollbar arrows should be visible now if they exist');
    }, 100);
...
// Run the inspection immediately
inspectBrowserScrollbars();

// Make function available globally
window.inspectBrowserScrollbars = inspectBrowserScrollbars;
...
// Continuous inspection every 1000ms
setInterval(() => {
    if (document.querySelector('.daterangepicker')) {
        inspectBrowserScrollbars();
    }
...
console.log('✅ Browser scrollbar inspection complete!');
console.log('💡 Run inspectBrowserScrollbars() again to re-inspect');
console.log('💡 Copy the results above and send them to me');
console.log('🔍 Continuous inspection active - checking every 1000ms!');

// ULTIMATE SCROLLBAR ARROW DESTROYER - Continuous monitoring
function ultimateScrollbarDestroyer() {
    console.log('💥 ULTIMATE SCROLLBAR ARROW DESTROYER ACTIVATED!');
    
    // Method 1: Force inject extreme CSS
    const existingStyle = document.getElementById('ultimate-scrollbar-destroyer');
    if (existingStyle) {
        existingStyle.remove();
    }
    
    const style = document.createElement('style');
    style.id = 'ultimate-scrollbar-destroyer';
    style.textContent = `
        /* ULTIMATE SCROLLBAR ARROW DESTROYER - INJECTED CSS */
        *::-webkit-scrollbar-button,
    ...
    // Method 2: Force override scrollbar styles on calendar
    const calendar = document.querySelector('.daterangepicker');
    if (calendar) {
        // Prevent calendar from jumping by setting a fixed height
        calendar.style.setProperty('height', 'auto', 'important');
        calendar.style.setProperty('max-height', 'none', 'important');
        calendar.style.setProperty('min-height', 'auto', 'important');
        calendar.style.setProperty('overflow', 'visible', 'important');
        calendar.style.setProperty('overflow-x', 'visible', 'important');
        calendar.style.setProperty('overflow-y', 'visible', 'important');
        calendar.style.setProperty('scrollbar-width', 'none', 'important');
        calendar.style.setProperty('scrollbar-color', 'transparent transparent', 'important');
        ...
        console.log('💥 FORCED scrollbar style overrides on calendar!');
    }
    
    // Method 3: Destroy any scrollbar-like elements
    const scrollbarElements = document.querySelectorAll('*');
    scrollbarElements.forEach(element => {
        // Fix the className error by checking if it exists and is a string
        ...
        if (className.includes('scrollbar') ||
            ...
            element.remove();
            console.log('💥 DESTROYED scrollbar element:', element);
        }
    });
    
    console.log('💥 ULTIMATE SCROLLBAR ARROW DESTROYER COMPLETE!');
}

// Run immediately
ultimateScrollbarDestroyer();

// Run when calendar opens
document.addEventListener('click', function(e) {
    if (e.target.closest('.daterangepicker') || e.target.closest('[data-toggle="daterangepicker"]')) {
        setTimeout(() => {
            ultimateScrollbarDestroyer();
...
// SIMPLE SCROLLBAR ARROW HIDING - CLEAN SOLUTION
function hideScrollbarArrows() {
    // Inject CSS to hide all scrollbar arrows globally
    const style = document.createElement('style');
    style.textContent = `
