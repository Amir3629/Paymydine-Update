...
/* PMD_SHIFTS_HOURLY_QUICK_CREATE_V17 */
...
(function () {
  ...
  var teamRoleCards = teamModal
    ? Array.prototype.slice.call(teamModal.querySelectorAll('[data-pmd-team-role-card]'))
    : [];
  ...
  function syncTeamRoleCards() {
    if (!teamAccessRoleInput || !teamRoleCards.length) return;
    var selectedId = String(teamAccessRoleInput.value || '');
    teamRoleCards.forEach(function (card) {
      var selected = String(card.getAttribute('data-role-id') || '') === selectedId;
  ...
  function setCredentialFieldVisible(field, visible) {
    if (!field) return;
  ...
  function syncTeamAccessFields() {
    ...
    // PMD_SHIFTS_WEB_CREDENTIALS_PLUS_PIN_UI_V18C
    // Every PMD Staff member keeps username/password for web and Staff Portal.
    // Operational roles additionally receive a Quick PIN for paired POS/app
    // devices. Quick PIN is therefore an extra fast credential, not a
    // replacement for the canonical web account.
    setCredentialFieldVisible(teamUsernameField, true);
    setCredentialFieldVisible(teamPasswordField, true);
    setCredentialFieldVisible(teamPinField, quickPin);

    if (teamUsernameInput) {
      teamUsernameInput.disabled = false;
      teamUsernameInput.required = true;
    }
    ...
    if (teamPinHint) {
      teamPinHint.textContent = quickPin
        ? (teamHasExistingPin
            ? 'App / POS · leave blank to keep current PIN'
            : 'App / POS · required · 6 digits')
        : '';
    }

    if (teamPasswordInput) {
      teamPasswordInput.disabled = false;
      teamPasswordInput.required = !teamHasExistingAccess;
    }
    if (teamPasswordHint) {
      teamPasswordHint.textContent = teamHasExistingAccess
        ? 'Web / Portal · leave blank to keep current password'
        : 'Web / Portal · required';
    }

    if (teamModal) {
      teamModal.setAttribute(
        'data-pmd-credential-mode',
        quickPin ? 'web-plus-pin' : 'web'
      );
    }
  }
  ...
  // PMD_SHIFTS_DATE_VISIBLE_LOCK_JS_V9
  ...
  function finalShiftMarkup(shift, person) {
    ...
    var total = 1440;
    var left = (window.start / total) * 100;
    ...
    return '' +
      '<button type="button" class="pmd-shifts-final-shift' + (shift.confirmed ? ' is-confirmed' : '') + (state === 'absent' ? ' is-absent' : '') + '"' +
        ...
        ' style="left:' + left.toFixed(4) + '% !important;width:' + width.toFixed(4) + '% !important;max-width:none !important;min-width:24px !important;box-sizing:border-box !important"' +
  ...
  function finalShiftGroupMarkup(group, person) {
    ...
    var start = Math.max(0, Number(group.start == null ? 0 : group.start));
    var end = Math.min(1440, Number(group.end == null ? start + 30 : group.end));
    if (end <= start) end = Math.min(1440, start + 30);
    var span = Math.max(1, end - start);
    var left = (start / 1440) * 100;
  ...
  function finalTimeScaleMarkup() {
    var labels = [];
    for (var value = 0; value <= 1440; value += 120) {
      labels.push('<span>' + escapeHtml(value === 1440 ? '24:00' : minuteLabel(value)) + '</span>');
    }
    return labels.join('');
  }
  ...
  function finalSlotMarkup(person, key) {
    var slots = [];
    for (var value = 0; value < 1440; value += 60) {
      ...
      '<div class="pmd-shifts-final-screen">' +
        '<header class="pmd-shifts-final-toolbar">' +
          '<div class="pmd-shifts-final-date">' +
            ...
            '<div><h2 data-pmd-no-translate data-pmd-i18n-skip data-pmd-shifts-date-label data-pmd-fixed-date="' + escapeHtml(formattedDate(key)) + '" aria-label="' + escapeHtml(formattedDate(key)) + '" lang="' + escapeHtml(shiftsDateLocaleCode) + '"></h2></div>' +
  ...
  var pmdShiftsDayNavBusyV13 = false;
  ...
  teamRoleCards.forEach(function (card) {
    card.addEventListener('click', function () {
      if (!teamAccessRoleInput) return;
      var roleId = String(card.getAttribute('data-role-id') || '');
      if (!roleId || String(teamAccessRoleInput.value || '') === roleId) {
        syncTeamRoleCards();
        return;
      }
      teamAccessRoleInput.value = roleId;
      teamAccessRoleInput.dispatchEvent(new Event('change', {bubbles:true}));
    });
  });
  ...
  // Restore the server-first planner position on a normal page open.
  restorePlannerScrollV17O();
