 ...
 * It is intentionally separate from Owner restaurant-security MFA and from
 * Workplace approval. Each user owns their own Authenticator secret.
 */
class PmdPortalTotpService
{
    public const TABLE = 'pmd_portal_mfa';
    public const SESSION_ENROLLMENT = 'pmd_portal_totp_enrollment_v1';
    ...
    /**
     * Create Portal MFA storage lazily in the active tenant DB when an older
     * tenant predates this migration. This runs only after password auth, under
     * the tenant-selected HTTP connection, and is idempotent.
     */
    ...
    public function enabled(int $userId, int $locationId): bool
    {
    ...
    /** Verify and consume one TOTP time-step; the same code cannot be replayed. */
    public function verify(int $userId, int $locationId, string $code): bool
    {
        ...
        if (
            ...
            && $matchedStep <= (int)$record->last_used_step
        ) {
            return false;
        }
    ...
    public function sessionVerified(
        ...
        return (int)($proof['user_id'] ?? 0) === $userId
            ...
            && (int)($proof['verified_at'] ?? 0)
                > (time() - max(30, $maxAgeSeconds));
    }
    ...
    private function markSessionVerified(
        int $userId,
        int $locationId,
        string $method
    ): void {
