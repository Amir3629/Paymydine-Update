 ...
 * - one conversation per authenticated admin user + canonical location;
...
final class AdminAiConversationStore
{
    ...
    /**
     * Compact conversational continuity for the model. This is not a factual
     * authority: Pmdintelligence instructs the model to re-check PMD tools for
     * restaurant facts instead of trusting an older assistant answer.
     */
    ...
    public function appendPair(
        ...
        try {
            DB::transaction(function () use ($locationId, $userId, $question, $answer, $runId): void {
                $now = now();
                $common = [
        ...
        return ['storage_ready' => true, 'persisted' => true];
    }
