...
class PmdKitchenEtaLifecycleService
{
    ...
    public function releaseOrder(
        ...
        ?array $items = null,
        ?int $locationId = null,
        string $reason = 'kitchen_release',
        $releaseAt = null
    ): array {
        if ($orderId < 1 || !$this->ready()) return $this->emptyState();

        return DB::transaction(function () use ($orderId, $items, $locationId, $reason, $releaseAt) {
            ...
            $releasedAt = !empty($order->kitchen_released_at)
                ...
                : ($releaseAt ? Carbon::parse($releaseAt) : now());
     ...
     * analytics; the current due time may only move later.
     */
    ...
    public function onKitchenStatus(int $orderId, string $statusName): array
    {
        ...
        if (str_contains($normalized, 'received') || str_contains($normalized, 'accepted') || str_contains($normalized, 'confirmed')) {
            return $this->releaseOrder($orderId, null, null, 'kitchen_received');
        }
    ...
    public function stateForOrder($orderOrId, bool $reconcile = true): array
    {
        ...
        // Self-healing release: staff/API orders are processed immediately;
        // guest_self orders stay processed=0 until payment releases them.
        if (empty($order->kitchen_released_at) && (int)($order->processed ?? 0) === 1 && !$this->isCancelledStatus($this->statusName($order))) {
            $comment = (string)($order->comment ?? '');
            $isGuestSelf = str_contains($comment, '[pmd_origin:guest_self]');
            $anchor = $isGuestSelf
                ? ($order->updated_at ?? now())
                : ($order->created_at ?? $order->order_date ?? now());
            return $this->releaseOrder(
                $orderId,
                null,
                max(1, (int)($order->location_id ?? 1)),
                $isGuestSelf ? 'guest_paid_release_recovery' : 'processed_order_release_recovery',
                $anchor
            );
        }
