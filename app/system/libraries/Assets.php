...
class Assets
{
    ...
    {
        /* PMD_ADMIN_FAVICON_AUTHORITY_V2
         ...
         * Version from the actual SVG mtime so a replaced product mark cannot
         * remain pinned behind an old browser favicon cache key.
         */
        try {
        ...
        if ($faviconFromSettings) {
            // Use the favicon from settings
            // Auto-detect MIME type based on file extension
            $extension = strtolower(pathinfo($faviconFromSettings, PATHINFO_EXTENSION));
            $mimeType = $extension === 'ico' ? 'image/x-icon' : 
                       ($extension === 'svg' ? 'image/svg+xml' : 'image/png');
            
            $attributes = ['rel' => 'shortcut icon', 'type' => $mimeType];
            // Build the full path: assets/media/uploads + /favicon.png
            $attributes['href'] = asset('assets/media/uploads' . $faviconFromSettings);
            return '<link'.Html::attributes($attributes).'>'.PHP_EOL;
        }
        
        // Fallback to default favicon assets
