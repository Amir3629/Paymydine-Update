        ...
        if (Schema::hasTable('orders')) {
            ...
            if ($missing) {
                Schema::table('orders', function (Blueprint $table) use ($missing) {
                    if (in_array('kitchen_released_at', $missing, true)) $table->timestamp('kitchen_released_at')->nullable()->index();
                    if (in_array('kitchen_preparing_at', $missing, true)) $table->timestamp('kitchen_preparing_at')->nullable()->index();
                    if (in_array('kitchen_ready_at', $missing, true)) $table->timestamp('kitchen_ready_at')->nullable()->index();
                    if (in_array('eta_initial_minutes', $missing, true)) $table->unsignedSmallInteger('eta_initial_minutes')->nullable();
                    if (in_array('eta_due_at', $missing, true)) $table->timestamp('eta_due_at')->nullable()->index();
                    if (in_array('eta_extension_count', $missing, true)) $table->unsignedTinyInteger('eta_extension_count')->default(0);
                    if (in_array('eta_last_extended_at', $missing, true)) $table->timestamp('eta_last_extended_at')->nullable();
                });
            }
        }
    ...
    public function down()
    {
        ...
        if (Schema::hasTable('orders')) {
            ...
            foreach ([
                ...
                if (Schema::hasColumn('orders', $column)) $existing[] = $column;
            }
            if ($existing) {
