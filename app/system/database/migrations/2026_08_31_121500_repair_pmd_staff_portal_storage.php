    ...
    public function up()
    {
        if (!Schema::hasTable('pmd_staff_requests')) {
            Schema::create('pmd_staff_requests', function (Blueprint $table) {
                ...
                $table->index(
                    ['location_id', 'status'],
                    'pmd_staff_requests_location_status_idx'
                );

                $table->index(
                    ['staff_id', 'status'],
                    'pmd_staff_requests_staff_status_idx'
                );

                $table->index(
                    ['person_id', 'created_at'],
                    'pmd_staff_requests_person_created_idx'
                );
            });
        }
        ...
        if (!Schema::hasTable('pmd_staff_chat_groups')) {
            Schema::create('pmd_staff_chat_groups', function (Blueprint $table) {
                ...
                $table->index(
                    ...
                    'pmd_staff_chat_groups_location_active_idx'
                );
            });
        }
        ...
        if (!Schema::hasTable('pmd_staff_chat_group_members')) {
            Schema::create('pmd_staff_chat_group_members', function (Blueprint $table) {
                ...
                $table->unique(
                    ['group_id', 'staff_id'],
                    'pmd_staff_chat_group_member_unique'
                );

                $table->index(
                    ['staff_id', 'group_id'],
                    'pmd_staff_chat_member_staff_group_idx'
                );
            });
        }
        ...
        if (!Schema::hasTable('pmd_staff_chat_messages')) {
            Schema::create('pmd_staff_chat_messages', function (Blueprint $table) {
                ...
                $table->index(
                    ['group_id', 'created_at'],
                    'pmd_staff_chat_messages_group_created_idx'
                );

                $table->index(
                    ['location_id', 'created_at'],
                    'pmd_staff_chat_messages_location_created_idx'
                );
            });
        }
        ...
        if (!Schema::hasTable('staff_attendance')) {
            Schema::create('staff_attendance', function (Blueprint $table) {
                ...
                $table->index(
                    ...
                    'staff_attendance_staff_open_idx'
                );

                $table->index(
                    ['location_id', 'check_in_time'],
                    'staff_attendance_location_checkin_idx'
                );
            });
        }
    }
