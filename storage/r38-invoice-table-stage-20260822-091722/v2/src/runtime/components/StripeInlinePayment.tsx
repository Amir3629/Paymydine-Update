...
function copyFor(locale = 'en') {
  ...
  const rows: Record<string, Record<string, string>> = {
    de: {
      secure: 'Sichere Kartenzahlung', cardholder: 'Name auf der Karte', card: 'Kartendaten', loading: 'Stripe wird geladen...',
      pay: 'Bezahlen', preparing: 'Zahlung wird vorbereitet...', processing: 'Zahlung wird verarbeitet...',
      appleContinue: 'Weiter mit Apple Pay', googleContinue: 'Weiter mit Google Pay',
      appleUnavailable: 'Apple Pay ist auf diesem Gerät/Browser nicht verfügbar oder nicht eingerichtet.',
      googleUnavailable: 'Google Pay ist auf diesem Gerät/Browser nicht verfügbar oder nicht eingerichtet.',
    },
    fa: {
      secure: 'پرداخت امن با کارت', cardholder: 'نام دارنده کارت', card: 'اطلاعات کارت', loading: 'Stripe در حال بارگذاری...',
      pay: 'پرداخت', preparing: 'در حال آماده‌سازی پرداخت...', processing: 'در حال پردازش پرداخت...',
      appleContinue: 'ادامه با Apple Pay', googleContinue: 'ادامه با Google Pay',
      appleUnavailable: 'Apple Pay در این دستگاه یا مرورگر در دسترس نیست.', googleUnavailable: 'Google Pay در این دستگاه یا مرورگر در دسترس نیست.',
    },
    tr: {
      secure: 'Güvenli kart ödemesi', cardholder: 'Kart üzerindeki ad', card: 'Kart bilgileri', loading: 'Stripe yükleniyor...',
      pay: 'Öde', preparing: 'Ödeme hazırlanıyor...', processing: 'Ödeme işleniyor...',
      appleContinue: 'Apple Pay ile devam et', googleContinue: 'Google Pay ile devam et',
      appleUnavailable: 'Apple Pay bu cihazda veya tarayıcıda kullanılamıyor.', googleUnavailable: 'Google Pay bu cihazda veya tarayıcıda kullanılamıyor.',
    },
    ja: {
      secure: '安全なカード決済', cardholder: 'カード名義', card: 'カード情報', loading: 'Stripe を読み込んでいます...',
      pay: '支払う', preparing: '支払いを準備しています...', processing: '支払いを処理しています...',
      appleContinue: 'Apple Pay で続行', googleContinue: 'Google Pay で続行',
      appleUnavailable: 'この端末またはブラウザでは Apple Pay を利用できません。', googleUnavailable: 'この端末またはブラウザでは Google Pay を利用できません。',
    },
  }
  return rows[code] || {
    secure: 'Secure card payment', cardholder: 'Name on card', card: 'Card details', loading: 'Loading Stripe...',
    pay: 'Pay', preparing: 'Preparing payment...', processing: 'Processing payment...',
    appleContinue: 'Continue with Apple Pay', googleContinue: 'Continue with Google Pay',
    appleUnavailable: 'Apple Pay is not available on this device/browser or is not configured.',
    googleUnavailable: 'Google Pay is not available on this device/browser or is not configured.',
  }
}
...
export function StripeInlinePayment(props: Props) {
  ...
  const settleConfirmedStripe = async (paymentIntent: any, intent: SplitPaymentIntent | null, amount: number) => {
    ...
    const reference = String(paymentIntent?.id || '')
    if (!reference) throw new Error('Stripe payment reference is missing.')
    if (status !== 'succeeded') {
      if (status === 'processing') {
        capturedRef.current = true
        throw new Error('Stripe is still processing this payment. Do not pay again; refresh the order status shortly.')
      }
    ...
    try {
      ...
      throw new Error(`Stripe confirmed the payment, but PayMyDine could not finish settlement. Do not pay again. ${settlementError instanceof Error ? settlementError.message : ''}`.trim())
    }
  }
  ...
  const mountWallet = async (intent: SplitPaymentIntent | null) => {
    ...
    const mount = walletMountRef.current
    // R35c: a method switch can briefly render with the previous ready=true state.
    // Do not surface that transient lifecycle state as a payment error.
    if (!stripe || !config || !mount || configuredMethodRef.current !== method) return

    try { walletButtonRef.current?.destroy?.() } catch {}
    walletButtonRef.current = null
    walletRequestRef.current = null
    mount.replaceChildren()

    const amount = intent?.payableAmount ?? props.amount
    const paymentRequest = stripe.paymentRequest({
      country: config.countryCode || 'DE',
      currency: String(props.currency || 'EUR').toLowerCase(),
      total: { label: 'PayMyDine', amount: Math.round(amount * 100) },
    ...
    if (!supported) {
      // Capability=false is the authoritative message; clear any transient setup state.
      setError('')
      setInfo('')
    ...
    paymentRequest.on('cancel', () => {
      if (!capturedRef.current && !settledRef.current) void cancelPreparedIntent()
      setWalletPrepared(false)
      try { walletButtonRef.current?.destroy?.() } catch {}
      walletButtonRef.current = null
    })
    ...
    paymentRequest.on('paymentmethod', async (event: any) => {
      ...
      try {
        ...
        capturedRef.current = true
        event.complete('success'); completed = true
        await settleConfirmedStripe(result.paymentIntent, activeIntent, activeAmount)
      } catch (paymentError) {
        if (!completed) try { event.complete('fail') } catch {}
        if (!capturedRef.current) await cancelPreparedIntent()
    ...
    walletButtonRef.current = button
    button.mount(mount)
  }
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      try {
        ...
        const config: StripeConfig = {
          ...
          countryCode: String(raw?.countryCode || 'DE').toUpperCase(),
          methods: {
            card: Boolean(raw?.methods?.card),
            apple_pay: Boolean(raw?.methods?.apple_pay),
            google_pay: Boolean(raw?.methods?.google_pay),
          },
        }
        ...
        stripeRef.current = factory(config.publishableKey)
        configuredMethodRef.current = method
        setReady(true)
      } catch (setupError) {
        if (!cancelled) reportError(setupError instanceof Error ? setupError.message : 'Stripe payment is unavailable.')
      }
    }
    ...
    return () => {
      ...
      if (!settledRef.current && !capturedRef.current) void cancelPreparedIntent()
    }
  ...
  useEffect(() => {
    ...
    const color = computed.getPropertyValue('--pmd-text').trim() || '#161616'
    const muted = computed.getPropertyValue('--pmd-muted').trim() || '#777777'
    const elements = stripeRef.current.elements()
    const card = elements.create('card', {
      hidePostalCode: true,
      style: {
        base: { fontSize: '16px', color, iconColor: color, '::placeholder': { color: muted } },
        invalid: { color: '#b54141', iconColor: '#b54141' },
      },
    })
    card.on('change', (event: any) => {
      setCardComplete(Boolean(event?.complete))
      if (event?.error?.message) reportError(String(event.error.message))
      else if (!capturedRef.current) setError('')
    })
  ...
  useEffect(() => {
    ...
    void mountWallet(null).catch((walletError) => reportError(walletError instanceof Error ? walletError.message : 'Wallet payment is unavailable.'))
    return () => {
      try { walletButtonRef.current?.destroy?.() } catch {}
      walletButtonRef.current = null
      walletRequestRef.current = null
  ...
  const payCard = async () => {
    ...
    let intent: SplitPaymentIntent | null = null
    try {
      intent = await prepareIntent()
      const amount = intent?.payableAmount ?? props.amount
      const created = await createStripeIntent(amount, intent)
      if (!created?.clientSecret) throw new Error('Stripe did not return a client secret.')
      setInfo(copy.processing)
      const result = await stripeRef.current.confirmCardPayment(created.clientSecret, {
        payment_method: { card: cardRef.current, billing_details: { name: cardholderName.trim() || 'Customer' } },
      })
      if (result?.error) throw new Error(String(result.error.message || 'Card payment failed.'))
      await settleConfirmedStripe(result?.paymentIntent, intent, amount)
    } catch (paymentError) {
      if (!capturedRef.current) await cancelPreparedIntent()
  ...
  const prepareWallet = async () => {
    ...
    try {
      ...
      setWalletPrepared(true)
      await mountWallet(intent)
      setInfo('')
    } catch (prepareError) {
      await cancelPreparedIntent()
      reportError(prepareError instanceof Error ? prepareError.message : 'Wallet payment could not be prepared.')
    } finally { setBusy(false) }
  }
  ...
  if (isWallet) {
    ...
    return (
      <div className={styles.stripeInlineBox} data-pmd-stripe-inline="r35b" data-pmd-stripe-wallet={method}>
        ...
        {ready && props.prepareSplitIntent && !walletPrepared && walletSupported !== false && (
          <button className={styles.primary} type="button" onClick={() => void prepareWallet()} disabled={busy || capturedRef.current}>
            {busy ? <LoaderCircle /> : <CreditCard />} {method === 'apple_pay' ? copy.appleContinue : copy.googleContinue} {money(props.amount, props.currency, props.locale)}
          </button>
        )}
  ...
  return (
    <div className={styles.stripeInlineBox} data-pmd-stripe-inline="r35b" data-pmd-stripe-card="r35b">
      <div className={styles.stripeSecureTitle}><CreditCard /> <strong>{copy.secure}</strong></div>
      {!ready && !error && <p className={styles.stripeHint}><LoaderCircle className={styles.stripeSpin} /> {copy.loading}</p>}
      <label className={styles.label}>{copy.cardholder}
        <input className={styles.input} autoComplete="cc-name" value={cardholderName} onChange={(event) => setCardholderName(event.target.value)} disabled={!ready || capturedRef.current} />
      </label>
      ...
      <button className={styles.primary} type="button" onClick={() => void payCard()} disabled={!ready || busy || capturedRef.current || !cardComplete}>
        {busy ? <LoaderCircle /> : <CreditCard />} {copy.pay} {money(props.amount, props.currency, props.locale)}
      </button>
      {info && <div className={styles.stripeHint}>{info}</div>}
      {error && <div className={`${styles.statusMessage} ${styles.statusError}`}>{error}</div>}
    </div>
