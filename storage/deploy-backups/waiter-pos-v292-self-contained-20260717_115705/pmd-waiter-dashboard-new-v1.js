  ...
  async function openPos(tableId) {
    ...
    try {
      ...
      if (
        ...
        window.PMDWaiterPOSV286.install(posRoot);
      } else {
        console.error(
          '[PMD] POS V286 installer missing on the real mount path'
        );
      }
       ...
       */
      if (
        window.PMDWaiterPOSV290 &&
        typeof window.PMDWaiterPOSV290.install === 'function'
      ) {
        window.PMDWaiterPOSV290.install(posRoot);
      } else {
        console.error(
          '[PMD] POS V290 installer missing on the real POS mount'
        );
      }
