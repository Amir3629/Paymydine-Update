    ...
    Route::get('/orders/pending-qr', function (\Illuminate\Http\Request $request) {
        ...
        if ($hasSplitTables) {
            $paidRows = \Illuminate\Support\Facades\DB::table('order_payment_transactions')
                ->join('order_payment_transaction_items', 'ti_order_payment_transaction_items.transaction_id', '=', 'ti_order_payment_transactions.id')
                ->where('ti_order_payment_transactions.order_id', $order->order_id)
                ->whereNotIn('ti_order_payment_transactions.settlement_status', ['failed', 'cancelled'])
                ->selectRaw("COALESCE(ti_order_payment_transaction_items.order_item_id, ti_order_payment_transaction_items.order_menu_id, ti_order_payment_transaction_items.menu_id) as alloc_key, SUM(ti_order_payment_transaction_items.quantity_paid) as qty_paid")
                ->groupByRaw("COALESCE(ti_order_payment_transaction_items.order_item_id, ti_order_payment_transaction_items.order_menu_id, ti_order_payment_transaction_items.menu_id)")
                ->get();
        ...
        try {
            $result = \Illuminate\Support\Facades\DB::transaction(function () use ($request, $order, $normalizedPaymentMethod, $paidStatusId, $hasSplitTables, $selectedItemsPayload, $allocationColumn, $allocationMode, $hasAllocOrderMenuColumn, $hasAllocMenuIdColumn, &$transactionId) {
                ...
                if ($hasSplitTables) {
                    $paidRows = \Illuminate\Support\Facades\DB::table('order_payment_transactions')
                        ->join('order_payment_transaction_items', 'ti_order_payment_transaction_items.transaction_id', '=', 'ti_order_payment_transactions.id')
                        ->where('ti_order_payment_transactions.order_id', $lockedOrder->order_id)
                        ->whereNotIn('ti_order_payment_transactions.settlement_status', ['failed', 'cancelled'])
                        ->selectRaw("ti_order_payment_transaction_items.{$allocationColumn} as alloc_key, SUM(ti_order_payment_transaction_items.quantity_paid) as qty_paid")
                        ->groupBy("ti_order_payment_transaction_items.{$allocationColumn}")
                        ->get();
                    foreach ($paidRows as $paidRow) {
                        if ($allocationMode === 'menu_id_legacy') {
                            $paidQtyByMenu[(int)$paidRow->alloc_key] = (float)$paidRow->qty_paid;
