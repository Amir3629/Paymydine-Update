(function () {
  ...
  function pmdV32AdoptSharedFloor(workspace) {
    ...
    if (
      ...
      if (sharedFloor) {
        pmdV32MakeFloorReservationSafe(
          sharedFloor
        );
      }

      return Boolean(sharedFloor);
    }

    /*
     * Remove the V3 duplicate floor controls/canvas.
     */
    targetPanel.innerHTML = '';

    targetPanel.appendChild(sharedFloor);

    sharedFloor.classList.add(
      'pmd-owner-floor-v60--reservation'
    );

    sharedFloor.style.display = 'grid';
    ...
    console.info(
      '[PMD Reservations V3.2] Shared dashboard floor adopted',
      {
        sharedFloor: true,
        tables:
          sharedFloor.querySelectorAll(
            '.pmd-owner-floor-v60__table'
          ).length,
  ...
  async function install() {
    ...
    console.info(
      '[PMD] Reservation Layout V3.2 active',
      {
