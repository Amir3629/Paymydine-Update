(function () {
  ...
  async function json(url, options) {
    ...
    if (raw) {
      try {
        ...
        payload = {ok:false, message:raw.slice(0, 500)};
      }
    }
    ...
    if (!response.ok || payload.ok === false) {
      var message =
        ...
        (payload.errors && Object.keys(payload.errors).length
          ? Object.keys(payload.errors).map(function (key) {
  ...
  async function performMerge() {
    ...
    async function postMerge(summary) {
    ...
    try {
      var summary = await loadFreshSummary();
      var result;

      try {
        result = await postMerge(summary);
      } catch (firstError) {
        if (Number(firstError.status) !== 409) throw firstError;

        button.textContent = 'REFRESHING CHECK…';
        toast('CHECK CHANGED — REFRESHING BEFORE MERGE');

        var refreshedSummary = await loadFreshSummary();

        button.textContent = 'RETRYING MERGE…';
        result = await postMerge(refreshedSummary);
      }
      ...
      window.dispatchEvent(new CustomEvent('pmd:waiter-pos-order-updated', {
        detail: {source: 'v250-direct-merge'}
      }));
      ...
      var message = error.message || 'MERGE FAILED';

      if (Number(error.status) === 409) {
        message =
          'MERGE CONFLICT: ' + message +
          ' Close any other waiter/cashier screen editing these checks, then try again.';
      }

      console.error('[PMD V2.5.0] direct merge failed', {
  ...
  console.info('[PMD] Waiter V2.5.0 direct merge conflict recovery active');
})();
