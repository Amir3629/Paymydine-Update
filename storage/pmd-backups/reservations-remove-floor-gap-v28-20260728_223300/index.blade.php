...
{{-- PMD_R2_TOOLBAR_IN_HEADER_V27 --}}
<style id="pmd-r2-toolbar-in-header-v27-style">
  /*
   * Floor buttons live inside the same top-header action area
   * used by the Calendar / All dates controls.
   */
  #pmd-r2-header-floor-tools-v27 {
  ...
  #pmd-r2-header-floor-tools-v27::before,
  #pmd-r2-header-floor-tools-v27::after {
  ...
  /*
   * Match the compact top-header buttons.
   * Individual buttons keep their functionality.
   */
  #pmd-r2-header-floor-tools-v27
  ...
  /*
   * Old host remains only as a compatibility node,
   * but creates no visible row inside the Floor frame.
   */
  #pmd-r2-floor-toolbar-host-v464.pmd-r2-old-toolbar-host-v27 {
    display: none !important;
    visibility: hidden !important;
    opacity: 0 !important;
  ...
  #pmd-r2-floor-toolbar-host-v464.pmd-r2-old-toolbar-host-v27::before,
  #pmd-r2-floor-toolbar-host-v464.pmd-r2-old-toolbar-host-v27::after {
    display: none !important;
    content: none !important;
  }
...
<script id="pmd-r2-toolbar-in-header-v27-script">
(function () {
  'use strict';
  ...
  var DATE_BUTTON_SELECTORS = [
    '#pmd-r2-date-button-v318',
    '[data-pmd-r2-date-button]',
    '[data-pmd-date-button]',
    '[data-pmd-r2-date-trigger]',
    '.pmd-r2-date-button-v318'
  ];
  ...
  function findDateButton() {
    for (
      var index = 0;
      index < DATE_BUTTON_SELECTORS.length;
      index += 1
    ) {
      var found =
        document.querySelector(
          DATE_BUTTON_SELECTORS[index]
        );
    ...
    /*
     * Safe fallback: locate the visible All dates button by text.
     */
    var candidates =
  ...
  function mount() {
    ...
    var oldHost =
      document.getElementById(OLD_HOST_ID);
    ...
    if (!shell) {
      ...
      shell.setAttribute(
        'data-pmd-floor-tools-location',
        'top-header-v27'
      );
    }

    /*
     * Put Floor controls immediately before All dates,
     * inside the same Calendar header action group.
     */
    if (
      shell.parentElement !== headerActions ||
      shell.nextElementSibling !== dateButton
    ) {
    ...
    oldHost.classList.add(
      'pmd-r2-old-toolbar-host-v27'
    );

    oldHost.setAttribute(
      'aria-hidden',
      'true'
    );
    ...
    console.info(
      '[PMD Reservations V27] ' +
      'Floor controls mounted beside All dates'
    );
  ...
  function boot() {
     ...
     * Only bounded startup retries, so no page lag.
     */
    if (mount()) {
      return;
    }
