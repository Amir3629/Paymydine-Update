...
Route::middleware([\App\Http\Middleware\TenantDatabaseMiddleware::class])->group(function (): void {
    Route::get('/billing-groups/current', function (Request $request, BillingGroupService $groups, BillingGroupInvoiceService $invoices) {
        ...
        try {
            // A closed Final Bill is only discoverable with the exact visit session.
            // A table-only lookup intentionally returns open state only, so a future
            // guest cannot discover the previous guest's invoice after Staff Free.
            $summary = $sessionKey !== ''
