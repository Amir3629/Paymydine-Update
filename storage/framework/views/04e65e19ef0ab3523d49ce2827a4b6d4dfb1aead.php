...
<?php $__currentLoopData = $pmdMenus; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $menuItem): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                <tr>
                    <td>
                        <div class="order-bill-item-name"><?php echo e($menuItem->name); ?></div>
                        <?php
                                    ...
                                    <?php $__currentLoopData = $menuItemOptions; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $menuItemOption): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
                                        <div class="order-bill-option">
                                            <?php if($menuItemOption->quantity > 1): ?>
                                                <span class="order-bill-option-qty"><?php echo e($menuItemOption->quantity); ?>x</span>
                                            <?php endif; ?>
                                            <span class="order-bill-option-name"><?php echo e($menuItemOption->order_option_name); ?></span>
                                            <?php if((float)($menuItemOption->__pmd_display_value ?? $menuItemOption->order_option_price ?? 0) > 0): ?>
                                                <span class="order-bill-option-price">+<?php echo e(currency_format($menuItemOption->__pmd_display_value ?? 0)); ?></span>
                                            <?php endif; ?>
                                        </div>
                        ...
                        <?php if(!empty($menuItem->comment)): ?>
                            <div class="order-bill-item-comment"><?php echo e($menuItem->comment); ?></div>
                        <?php endif; ?>
                    </td>
                    <td class="order-bill-quantity text-center">
                        <div class="quantity-controls" data-order-menu-id="<?php echo e($menuItem->order_menu_id); ?>" data-menu-id="<?php echo e($menuItem->menu_id); ?>" data-price="<?php echo e($menuItem->price); ?>">
                            <button type="button" class="qty-btn qty-minus" onclick="event.preventDefault(); event.stopPropagation(); updateOrderItemQuantity(<?php echo e($menuItem->order_menu_id); ?>, -1);" title="Decrease quantity">
                                <i class="fa fa-minus"></i>
                            </button>
                            <span class="qty-display" id="qty-<?php echo e($menuItem->order_menu_id); ?>"><?php echo e($menuItem->quantity); ?></span>
                            <button type="button" class="qty-btn qty-plus" onclick="event.preventDefault(); event.stopPropagation(); updateOrderItemQuantity(<?php echo e($menuItem->order_menu_id); ?>, 1);" title="Increase quantity">
                    ...
                    <td class="order-bill-total text-right">
                        <span class="item-subtotal" id="subtotal-<?php echo e($menuItem->order_menu_id); ?>">
                            <?php if($pmdIsPosImport): ?>
                                <?php
                                ...
                                ?>
                                <?php echo e(currency_format($pmdForcedLine)); ?>

                            <?php else: ?>
                                <?php echo e(currency_format($menuItem->__pmd_display_subtotal ?? 0)); ?>
        ...
        <tfoot class="order-bill-totals">
            <tr>
                <td class="total-label">
                    Subtotal
                    <?php if($displayTotalItems > 0): ?>
                        <span class="order-bill-subtotal-note">(<?php echo e($displayTotalItems); ?> item<?php echo e($displayTotalItems > 1 ? 's' : ''); ?>)</span>
                    <?php endif; ?>
                </td>
                <td></td>
                <td class="total-value text-right"><?php echo e(currency_format($pmdDisplayedSubtotal)); ?></td>
            </tr>

            <?php if($pmdTaxIncluded): ?>
                <tr>
                    <td class="total-label order-bill-tax">VAT included</td>
                    <td></td>
                    <td class="total-value order-bill-tax text-right">
                        <?php if($pmdIsPosImport): ?>
                            <?php echo e(currency_format($pmdPosVat ?? 0)); ?>

                        <?php else: ?>
                            <?php echo e($pmdTaxLabel ?? 'VAT included'); ?>

                        <?php endif; ?>
                    </td>
                </tr>
            <?php elseif($taxTotal && (float)$taxTotal->value > 0): ?>
                <tr>
                    <td class="total-label order-bill-tax"><?php echo e($taxTotal->title ?? 'VAT'); ?></td>
                    <td></td>
                    <td class="total-value order-bill-tax text-right"><?php echo e(currency_format((float)$taxTotal->value)); ?></td>
                </tr>
            <?php endif; ?>

            <?php if($tipTotal && $tipTotal->value > 0): ?>
                <tr>
                    <td class="total-label order-bill-tip">Tip</td>
                    <td></td>
                    <td class="total-value order-bill-tip text-right"><?php echo e(currency_format($pmdDisplayedTip)); ?></td>
                </tr>
            <?php endif; ?>

            <?php if($couponTotal && $couponTotal->value != 0): ?>
                <tr>
                    <td class="total-label order-bill-discount">
                        Coupon
                        <?php if($couponCode): ?>
                            <span class="order-bill-coupon-code">(<?php echo e($couponCode); ?>)</span>
                        <?php endif; ?>
                    </td>
                    <td></td>
                    <td class="total-value order-bill-discount text-right">
                        <?php echo e($couponTotal->value < 0 ? '--' : ''); ?><?php echo e(currency_format(abs($couponTotal->value))); ?>
            ...
            <?php if($finalTotal): ?>
                <tr class="final-total">
                    ...
                    <td></td>
                    <td class="total-value text-right" id="order-final-total"><?php if($pmdIsPosImport): ?> <?php echo e(currency_format($pmdPosGross ?? 0)); ?> <?php else: ?> <?php echo e(currency_format($finalTotal->value ?? 0)); ?> <?php endif; ?></td>
                </tr>
            <?php endif; ?>
        </tfoot>
    </table>
    ...
    <div class="order-bill-actions" style="margin-top: 20px; padding-top: 15px;">
        <button type="button" class="btn btn-primary btn-add-item" id="btn-add-item" onclick="event.preventDefault(); event.stopPropagation(); addItemToOrder(<?php echo e($model->order_id); ?>);">
...
const CURRENT_ORDER_ID = <?php echo e($model->order_id); ?>;
...
function addItemToOrder(orderId) {
    ...
    const tableId = '<?php echo e($model->table_id ?? ""); ?>';
    const locationId = '<?php echo e($model->location_id ?? 1); ?>';
...
<?php /**PATH /var/www/paymydine/app/admin/views/orders/form/order_menus.blade.php ENDPATH**/ ?>