...
class ServiceProvider extends AppServiceProvider
{
    ...
    public function register()
    {
        ...
        //
        // Canonical Super Admin route authority.
        // Central DB only: tenant DB middleware must never own these requests.
        //
        Route::middleware('web')
            ->withoutMiddleware([
                \Igniter\Flame\Foundation\Http\Middleware\TenantDatabaseMiddleware::class
            ])
            ->group(function () {
                ...
                Route::get(
                    '/superadmin/login',
                    [SuperAdminController::class, 'login']
                )->name('superadmin.login');

                Route::post(
                    '/superadmin/sign',
                    [SuperAdminController::class, 'sign']
                )->name('superadmin.sign');
                ...
                Route::middleware('superadmin.auth')->group(function () {
                    ...
                    Route::get(
                        '/superadmin/index',
                        [SuperAdminController::class, 'showIndex']
                    )->name('superadmin.index');

                    Route::get(
                        '/superadmin/new',
                        [SuperAdminController::class, 'showNewPage']
                    )->name('superadmin.new');

                    Route::get(
                        '/superadmin/settings',
                        [SuperAdminController::class, 'settings']
                    )->name('superadmin.settings');

                    Route::get(
                        '/superadmin/location-requests',
                        [SuperAdminController::class, 'locationRequests']
                    )->name('superadmin.location-requests');

                    Route::get(
                        '/superadmin/signout',
                        [SuperAdminController::class, 'signOut']
                    )->name('superadmin.signout');

                    /*
                     * Recovery R1 deliberately locks tenant mutation.
                     *
                     * The legacy creator currently performs:
                     * central row -> CREATE DATABASE -> newtenantdb clone ->
                     * theme activation -> privileged Nginx/Certbot provisioning.
                     *
                     * Domain provisioning is stale and must be replaced before
                     * this endpoint is allowed to create production tenants.
                     */
                    Route::post('/superadmin/new/store', function () {
                        return response(
                            'Tenant creation is temporarily locked while PayMyDine Super Admin provisioning is being recovered.',
                            423,
    ...
    protected function registerMainMenuItems()
    {
        AdminMenu::registerCallback(function (Navigation $manager) {
            $menuItems = [
                ...
                // Location picker moved to user profile dropdown
                // 'locations' => [
                //     'type' => 'partial',
            ...
            // Location picker is now in user menu, so no need to check here
    ...
    protected function registerPermissions()
    {
        PermissionManager::instance()->registerCallback(function ($manager) {
            $manager->registerPermissions('Admin', [
                ...
                'Admin.PosDevices' => [
                    'label' => 'admin::lang.permissions.pos_devices', 'group' => 'admin::lang.permissions.name',
                ],
                'Admin.PosConfigs' => [
