...
export function WorldlineInlineCardForm({
  ...
  const updateField = (name: keyof typeof formData, value: any) => {
    ...
    const formattedValue =
      name === "cardNumber" || name === "expiryDate" || name === "cvv"
        ? applyFieldMask(name, raw)
        : raw
    const nextData = { ...formData, [name]: String(formattedValue ?? "") }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const rawExpiry = getExpiryDigits(formData.expiryDate)
      ...
      const getReqField = (fieldId: string) => {
        try {
          if (typeof paymentRequest?.getField === "function") {
            const f = paymentRequest.getField(fieldId)
            if (f) return f
          }
        } catch {}
        try {
          if (typeof paymentRequest?.field === "function") {
            const f = paymentRequest.field(fieldId)
            if (f) return f
          }
        } catch {}
        return null
      ...
      const safeSet = (fieldId: string, value: string) => {
        const result: any = {
          fieldId,
          value,
          viaFieldSetValue: False if False else None,
          viaRequestSetValue: False if False else None,
          errors: [],
        }

        const fieldObj = getReqField(fieldId)

        try {
          if (fieldObj && typeof fieldObj.setValue === "function") {
            fieldObj.setValue(value)
            result.viaFieldSetValue = True
          }
        } catch (e: any) {
          result.errors.push("field.setValue: " + String(e?.message || e))
        }

        try {
          if (typeof paymentRequest?.setValue === "function") {
            paymentRequest.setValue(fieldId, value)
            result.viaRequestSetValue = True
          }
        } catch (e: any) {
          result.errors.push("paymentRequest.setValue: " + String(e?.message || e))
        }

        return result
      }

      const safeGetRequestValue = (fieldId: string) => {
        try {
          if (typeof paymentRequest?.getValue === "function") {
        ...
        try {
          const f = getReqField(fieldId)
          if (f && typeof f.getValue === "function") return f.getValue()
          if (f && "value" in f) return (f as any).value
        } catch {}
      ...
      const safeFieldValidate = (fieldId: string, value: string) => {
        const out: any = {
          fieldId,
          value,
          fieldExists: false,
        ...
        try {
          const fieldObj = getReqField(fieldId)
          out.fieldExists = !!fieldObj
          if (fieldObj && typeof fieldObj.validate === "function") {
            out.result = fieldObj.validate(value)
          }
        } catch (e: any) {
      ...
      const setResults = {
        cardNumber: safeSet("cardNumber", rawCardNumber),
        expiryDate: safeSet("expiryDate", rawExpiry),
        cvv: safeSet("cvv", rawCvv),
        cardholderName: safeSet("cardholderName", rawCardholder),
      }

      const requestValuesAfterSet = {
        cardNumber: safeGetRequestValue("cardNumber"),
        expiryDate: safeGetRequestValue("expiryDate"),
        cvv: safeGetRequestValue("cvv"),
        cardholderName: safeGetRequestValue("cardholderName"),
        securityCode: safeGetRequestValue("securityCode"),
        expiryMonth: safeGetRequestValue("expiryMonth"),
        expiryYear: safeGetRequestValue("expiryYear"),
      }

      const fieldValidation = {
        cardNumber: safeFieldValidate("cardNumber", rawCardNumber),
        expiryDate: safeFieldValidate("expiryDate", rawExpiry),
        cvv: safeFieldValidate("cvv", rawCvv),
        cardholderName: safeFieldValidate("cardholderName", rawCardholder),
      }

      let validationResult: any = null
      try {
        if (typeof paymentRequest?.validate === "function") {
          validationResult = paymentRequest.validate()
        }
      } catch (e: any) {
        validationResult = {
          threw: true,
      ...
      const sdkMethods = {
        preparePaymentRequest: typeof sdk?.preparePaymentRequest,
        encryptPaymentRequest: typeof sdk?.encryptPaymentRequest,
        getEncryptor: typeof sdk?.getEncryptor,
      }
      ...
      if (typeof sdk?.encryptPaymentRequest === "function") {
      ...
      } else if (typeof sdk?.preparePaymentRequest === "function") {
        ...
        if (typeof encryptor?.encrypt === "function") {
          ...
          throw new Error("Worldline encryptor.encrypt is not available")
        }
      } else {
        throw new Error("Worldline SDK has no supported encryption method")
      }
      ...
      const debugPayload = {
        ...
        availableFieldIds: Array.isArray(paymentProduct?.paymentProductFields)
          ? paymentProduct.paymentProductFields.map((f: any) => String(f?.id ?? "")).filter(Boolean)
          : (
              typeof paymentProduct?.getFields === "function"
                ? paymentProduct.getFields().map((f: any) => String(f?.id ?? "")).filter(Boolean)
        ...
        sentValues: {
          ...
          expiryDate: getExpiryDigits(formData.expiryDate),
          cvv: normalizeCvv(formData.cvv),
          cardholderName: formData.cardholderName,
          email: formData.email,
        },
        sdkMethods: {
        ...
        setResults: typeof setResults !== "undefined" ? setResults : null,
        requestValuesAfterSet: typeof requestValuesAfterSet !== "undefined" ? requestValuesAfterSet : null,
        fieldValidation: typeof fieldValidation !== "undefined" ? fieldValidation : null,
        validationResult: typeof validationResult !== "undefined" ? validationResult : null,
      }
