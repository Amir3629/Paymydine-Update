...
function resolveWorldlineRuntimeSdk(mod: any): WorldlineRuntimeSdk | null {
  ...
  const directCandidates = [
    ...
    { value: mod?.Worldline, source: "esm:Worldline" },
    { value: mod?.default?.Worldline, source: "esm:default.Worldline" },
    ...
    { value: worldlineGlobal?.onlinepaymentssdk, source: "global:window.Worldline.onlinepaymentssdk" },
    { value: worldlineGlobal?.OnlinePaymentsSdk, source: "global:window.Worldline.OnlinePaymentsSdk" },
  ]

  for (const candidate of directCandidates) {
    const hit = asRuntimeSdkCandidate(candidate.value, candidate.source)
    if (hit) return hit
  }

  const nestedContainers = [
    { value: mod, label: "mod" },
    { value: mod?.default, label: "mod.default" },
    { value: mod?.default?.default, label: "mod.default.default" },
  ]

  for (const container of nestedContainers) {
    const keys = getObjectKeysSafe(container.value)
    for (const key of keys) {
      const hit = asRuntimeSdkCandidate(container.value?.[key], `${container.label}.${key}`)
      if (hit) return hit
    }
  }
    ...
    ;(async () => {
      try {
        ...
        const initPromise = inFlightInit ?? (async () => {
          console.info("[WorldlineInlineCardForm] SDK import diagnostics", {
            topLevelKeys: getObjectKeysSafe(OnlinePaymentsSdk as any),
            defaultKeys: getObjectKeysSafe((OnlinePaymentsSdk as any)?.default),
            defaultDefaultKeys: getObjectKeysSafe((OnlinePaymentsSdk as any)?.default?.default),
            typeofInit: typeof (OnlinePaymentsSdk as any)?.init,
            typeofDefaultInit: typeof (OnlinePaymentsSdk as any)?.default?.init,
            typeofDefaultDefaultInit: typeof (OnlinePaymentsSdk as any)?.default?.default?.init,
            typeofPaymentRequest: typeof (OnlinePaymentsSdk as any)?.PaymentRequest,
            typeofDefaultPaymentRequest: typeof (OnlinePaymentsSdk as any)?.default?.PaymentRequest,
            typeofDefaultDefaultPaymentRequest: typeof (OnlinePaymentsSdk as any)?.default?.default?.PaymentRequest,
            typeofWindowOnlinepaymentssdk: typeof (window as any)?.onlinepaymentssdk,
            typeofWindowOnlinePaymentsSdk: typeof (window as any)?.OnlinePaymentsSdk,
            typeofWindowWorldline: typeof (window as any)?.Worldline,
          })
