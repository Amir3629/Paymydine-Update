...
export function WorldlineInlineCardForm({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    if (!sdk || !paymentProduct) {
      ...
      console.error("[WorldlineInlineCardForm][submit]", msg, {
        hasSdk: !!sdk,
    ...
    try {
      ...
      const safeSet = (fieldId: string, value: string) => {
        ...
        try {
          if (typeof paymentRequest?.getField === "function") {
            fieldObj = paymentRequest.getField(fieldId)
          }
        } catch {}
        try {
          if (!fieldObj && typeof paymentRequest?.field === "function") {
            fieldObj = paymentRequest.field(fieldId)
          }
        } catch {}
        ...
        try {
          ...
          console.warn("[WorldlineInlineCardForm] field.setValue failed", {
            fieldId,
        ...
        try {
          if (typeof paymentRequest?.setValue === "function") {
            ...
            return "paymentRequest.setValue"
          }
        } catch (e) {
          console.warn("[WorldlineInlineCardForm] paymentRequest.setValue failed", {
            fieldId,
            error: String((e as any)?.message || e)
          })
        }
      ...
      try {
        ...
        validationResult = { threw: true, message: String((e as any)?.message || e) }
      }
