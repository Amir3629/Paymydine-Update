...
class WorldlineHostedCheckoutService
{
    ...
    public function createInlinePayment(array $payload): array
    {
        ...
        $response = $merchantClient->payments()->create($this->pmdNormalizeCreatePaymentRequest($request));
        $raw = json_decode(json_encode($response), true);
        $raw = is_array($raw) ? $raw : [];
    ...
    {
        return $this->pmdMapToDomainObject(\Worldline\Connect\Sdk\V1\Domain\CreatePaymentRequest::class, $body);
    }
     ...
     */
    private function pmdMapToDomainObject(string $class, $data)
    {
        ...
        foreach (get_object_vars($data) as $key => $value) {
            ...
            try {
                $rm = new \ReflectionMethod($object, $setter);
                ...
                if (
                    ...
                    $value = $this->pmdMapToDomainObject($ptype->getName(), $value);
