...
Route::middleware(['cors'])->group(function () {
    ...
    Route::get('/images', function (Request $request) {
        ...
        if ($requested === '') {
            ...
            return response()->file($fallbackPath, [
                ...
                'X-PMD-Image-Fallback' => '1',
                'X-PMD-Image-Reason' => 'empty-file-param',
            ]);
        }

        // اگر full URL پاس داده شد فقط path را نگه دار
        if (filter_var($requested, FILTER_VALIDATE_URL)) {
            $parsedPath = parse_url($requested, PHP_URL_PATH);
            $requested = $parsedPath ?: $requested;
        }
        ...
        $candidates = [
            storage_path('app/public/assets/media/attachments/public/' . $relative),
            storage_path('app/public/assets/media/attachments/public/' . $basename),
            storage_path('app/public/' . $relative),
            storage_path('app/public/' . $basename),
            storage_path('app/media/' . $relative),
            storage_path('app/media/' . $basename),
            public_path('assets/media/uploads/' . $relative),
            public_path('assets/media/uploads/' . $basename),
            public_path($relative),
            public_path($basename),
        ];

        // حالت فایل‌های hash-based
        if (preg_match('/^(\w{3})(\w{3})(\w{3}).*$/', $basename, $m)) {
            $candidates[] = storage_path("app/public/assets/media/attachments/public/{$m[1]}/{$m[2]}/{$m[3]}/{$basename}");
            $candidates[] = storage_path("app/public/{$m[1]}/{$m[2]}/{$m[3]}/{$basename}");
            $candidates[] = storage_path("app/media/{$m[1]}/{$m[2]}/{$m[3]}/{$basename}");
        }

        $seen = [];
        foreach ($candidates as $imagePath) {
            if (!$imagePath || isset($seen[$imagePath])) {
                continue;
            }
            $seen[$imagePath] = true;
            ...
            if (is_file($imagePath)) {
                ...
                return response()->file($imagePath, [
                    ...
                    'Cache-Control' => 'public, max-age=60',
                    'X-PMD-Image-Fallback' => '0',
                    'X-PMD-Image-Path' => $imagePath,
                ]);
            }
        }

        // جستجوی recursive بر اساس basename
        $searchRoots = [
            ...
            try {
                ...
                foreach ($it as $file) {
                    if ($file->isFile() && $file->getFilename() === $basename) {
                        ...
                        $mimeType = @mime_content_type($real) ?: 'application/octet-stream';

                        return response()->file($real, [
                            'Content-Type' => $mimeType,
                            'Cache-Control' => 'public, max-age=60',
                            'X-PMD-Image-Fallback' => '0',
                            'X-PMD-Image-Path' => $real,
                        ]);
                    }
                }
            } catch (\Throwable $e) {
        ...
        return response()->file($fallbackPath, [
            ...
            'Cache-Control' => 'public, max-age=60',
            'X-PMD-Image-Fallback' => '1',
            'X-PMD-Image-Reason' => 'not-found',
            'X-PMD-Requested-File' => $requested,
        ]);
    });
    ...
    Route::prefix('v1')->middleware(['web', 'detect.tenant'])->group(function () {
        ...
        // --- COMPAT ALIASES (frontend expects these) ---
        // No tenant hardcoding; DetectTenant middleware already applied to v1 group.
        ...
        Route::get('/settings-wrapped', function () {
            ...
            // "wrapped" response used by frontend
...
/*
|---------------------------------------------------------------------------
| PayMyDine Compatibility Endpoint
|---------------------------------------------------------------------------
| Frontend expects settings response in: { success: true, data: {...} }
| We wrap existing /api/v1/settings output.
*/
Route::get('api/v1/settings-wrapped', function () {
    $req = \Illuminate\Http\Request::create('/api/v1/settings', 'GET');
    $res = app()->handle($req);
...
Route::get('/payment-methods-public', function () {
    ...
    $mapped = $rows->map(function ($row) {
        ...
        return [
            'id' => $row->id,
            'method_code' => $row->code,
            'provider_code' => $row->provider_code,
            'code' => $meta['frontend_code'] ?? $row->code,
            'name' => $row->name,
