...
export function WorldlineInlineCardForm({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const rawCardNumber = String(formData.cardNumber ?? "").replace(/\s+/g, "").trim()
      const rawExpiry = String(formData.expiryDate ?? "").trim()
      const rawCvv = String(formData.cvv ?? "").trim()
      const rawCardholder = String(formData.cardholderName ?? "").trim()

      const getReqField = (fieldId: string) => {
        try {
          return (
            (typeof paymentRequest?.getField === "function" ? paymentRequest.getField(fieldId) : null) ||
            (typeof paymentRequest?.field === "function" ? paymentRequest.field(fieldId) : null) ||
            null
          )
        } catch {
          return null
      ...
      const safeFieldValidate = (fieldId: string) => {
        try {
          const field = getReqField(fieldId)
      ...
      const fieldValidation = {
        cardNumber: safeFieldValidate("cardNumber"),
        expiryDate: safeFieldValidate("expiryDate"),
        cvv: safeFieldValidate("cvv"),
        cardholderName: safeFieldValidate("cardholderName"),
      }
