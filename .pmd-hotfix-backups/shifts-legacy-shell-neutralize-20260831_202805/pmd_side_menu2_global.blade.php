...
@if(trim(request()->path(), '/') === 'admin/shifts')
<script id="pmd-shifts-paint-trace-temp">
/* PMD_SHIFTS_PAINT_TRACE_TEMP */
(function () {
    'use strict';
    ...
    function snap(label) {
        ...
        var data = {
            ...
            label: label,
            html: html.className,
            panelVar: getComputedStyle(html)
                .getPropertyValue('--pmd-sm2-panel')
                .trim(),
            gapVar: getComputedStyle(html)
                .getPropertyValue('--pmd-admin-gap')
                .trim(),
            menu: rect('#pmd-side-menu2'),
            wrapper: rect('.page-wrapper'),
            content: rect('.page-content'),
            shifts: rect('#pmd-shifts'),
            final: rect('.pmd-shifts-final-screen'),
            kpis: rect('#pmd-r2-reservation-kpis-v307'),
            board: rect('.pmd-shifts-final-board')
        };

        var signature = JSON.stringify({
            html: data.html,
            panelVar: data.panelVar,
            menu: data.menu && [data.menu.x, data.menu.w],
            wrapper: data.wrapper && [data.wrapper.x, data.wrapper.w],
            content: data.content && [
                data.content.x,
                data.content.w,
                data.content.paddingLeft
            ],
            shifts: data.shifts && [
                data.shifts.x,
                data.shifts.w,
                data.shifts.marginLeft,
                data.shifts.paddingLeft,
                data.shifts.maxWidth
            ],
            final: data.final && [data.final.x, data.final.w],
            kpis: data.kpis && [data.kpis.x, data.kpis.w]
        });
    ...
    function frame() {
        ...
        if (performance.now() - start < 1800) {
            ...
            window.PMDShiftsPaintTrace = rows;

            console.warn(
                '=== PMD SHIFTS PAINT TRACE COMPLETE ==='
            );
            ...
            console.table(
                rows.map(function (r) {
                    return {
                        ...
                        html: r.html,
                        panel: r.panelVar,
                        menuX: r.menu && r.menu.x,
                        menuW: r.menu && r.menu.w,
                        wrapperX: r.wrapper && r.wrapper.x,
                        wrapperW: r.wrapper && r.wrapper.w,
                        contentX: r.content && r.content.x,
                        contentPad: r.content && r.content.paddingLeft,
                        shiftsX: r.shifts && r.shifts.x,
                        shiftsW: r.shifts && r.shifts.w,
                        shiftsMargin: r.shifts && r.shifts.marginLeft,
                        shiftsPad: r.shifts && r.shifts.paddingLeft,
                        finalX: r.final && r.final.x,
                        kpiX: r.kpis && r.kpis.x
                    };
                })
            );
        }
