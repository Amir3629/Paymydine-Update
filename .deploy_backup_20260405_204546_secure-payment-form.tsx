...
import { Session, PaymentRequest, CreditCardTokenRequest } from "onlinepayments-sdk-client-js"
...
export function WorldlineInlineCardForm({
  ...
  const buildPaymentRequest = (nextData: typeof formData) => {
    ...
    paymentRequest.setValue("expiryDate", expiryDigits)
  ...
  const validateWorldlineFields = (nextData: typeof formData) => {
    ...
    const expiryValidation = paymentProduct.getField("expiryDate")?.validate?.((nextData.expiryDate || "").replace(/\D/g, "").slice(0, 4))
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const validationResultAny = paymentRequest.validate?.()
      console.info("[WorldlineInlineCardForm] validationResult", validationResultAny)

      if (
        validationResultAny &&
        (
          (typeof validationResultAny === "object" && "isValid" in validationResultAny && validationResultAny.isValid === false) ||
          (typeof validationResultAny === "object" && Array.isArray(validationResultAny.errors) && validationResultAny.errors.length > 0)
        )
      ) {
        validateWorldlineFields(formData)
        throw new Error("Please check your card details")
...
// ✅ REAL FIX: use CreditCardTokenRequest
...
tokenRequest.setValue("cardNumber", digits)
tokenRequest.setValue("expiryDate", mm + yy)
tokenRequest.setValue("cvv", formData.cvv)
tokenRequest.setValue("cardholderName", formData.cardholderName || "")

console.info("[WORLDLINE FINAL FIX] using CreditCardTokenRequest")

preparedOrEncrypted = await encryptor.encrypt(tokenRequest)
