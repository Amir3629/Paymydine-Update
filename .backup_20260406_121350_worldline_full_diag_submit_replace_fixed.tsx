...
export function WorldlineInlineCardForm({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const setResults = {
        cardNumber: setFieldValue("cardNumber", cardNumber),
        expiryDate: setFieldValue("expiryDate", expiryDigits),
        cvv: setFieldValue("cvv", cvv),
        cardholderName: setFieldValue("cardholderName", cardholderName),
      }

      const safeGet = (fieldId: string) => {
        try {
          if (typeof paymentRequest?.getValue === "function") return paymentRequest.getValue(fieldId)
        } catch {}
        try {
          const f =
            (typeof paymentRequest?.getField === "function" ? paymentRequest.getField(fieldId) : null) ||
      ...
      const validationResult =
        typeof paymentRequest?.validate === "function"
      ...
      console.info("[WorldlineInlineCardForm] VALIDATION PROBE", {
        ...
        validationIsValid: !!(validationResult as any)?.isValid,
        validationErrors,
        paymentProductId: paymentProduct?.id,
        paymentProductFields: Array.isArray(paymentProduct?.paymentProductFields)
      ...
      if (
        ...
        (validationResult as any).isValid === false
      ) {
        validateWorldlineFields(formData)
        throw new Error("Worldline validate() failed: " + JSON.stringify(validationErrors))
      }
