...
function getObjectKeysSafe(value: any): string[] {
  try {
...
function asRuntimeSdkCandidate(value: any, source: string): WorldlineRuntimeSdk | null {
  if (value && typeof value.init === "function" && typeof value.PaymentRequest === "function") {
    return {
      init: value.init,
...
function resolveWorldlineRuntimeSdk(mod: any): WorldlineRuntimeSdk | null {
  ...
  const candidates = [
    { value: mod, source: "esm:namespace" },
    { value: mod?.default, source: "esm:default" },
    { value: mod?.default?.default, source: "esm:default.default" },
    { value: mod?.OnlinePaymentsSdk, source: "esm:OnlinePaymentsSdk" },
    { value: mod?.default?.OnlinePaymentsSdk, source: "esm:default.OnlinePaymentsSdk" },
    { value: onlinePaymentsGlobal, source: "global:window.onlinepaymentssdk" },
    { value: onlinePaymentsSdkGlobal, source: "global:window.OnlinePaymentsSdk" },
    { value: worldlineGlobal, source: "global:window.Worldline" },
  ]

  for (const candidate of candidates) {
    const resolved = asRuntimeSdkCandidate(candidate.value, candidate.source)
...
function getWorldlineRuntimeDiagnostics(mod: any) {
  const onlinePaymentsGlobal = typeof window !== "undefined" ? (window as any)?.onlinepaymentssdk : null
  const onlinePaymentsSdkGlobal = typeof window !== "undefined" ? (window as any)?.OnlinePaymentsSdk : null
  const worldlineGlobal = typeof window !== "undefined" ? (window as any)?.Worldline : null

  return {
    typeof_mod: typeof mod,
    mod_keys: getObjectKeysSafe(mod),
    mod_default_keys: getObjectKeysSafe(mod?.default),
    mod_default_default_keys: getObjectKeysSafe(mod?.default?.default),
    mod_onlinePaymentsSdk_keys: getObjectKeysSafe(mod?.OnlinePaymentsSdk),
    mod_default_onlinePaymentsSdk_keys: getObjectKeysSafe(mod?.default?.OnlinePaymentsSdk),
    typeof_mod_init: typeof mod?.init,
    typeof_mod_paymentRequest: typeof mod?.PaymentRequest,
    typeof_mod_default_init: typeof mod?.default?.init,
    typeof_mod_default_paymentRequest: typeof mod?.default?.PaymentRequest,
    typeof_mod_default_default_init: typeof mod?.default?.default?.init,
    typeof_mod_default_default_paymentRequest: typeof mod?.default?.default?.PaymentRequest,
    typeof_window_onlinepaymentssdk: typeof onlinePaymentsGlobal,
    typeof_window_OnlinePaymentsSdk: typeof onlinePaymentsSdkGlobal,
    typeof_window_Worldline: typeof worldlineGlobal,
    window_onlinepaymentssdk_keys: getObjectKeysSafe(onlinePaymentsGlobal),
    window_OnlinePaymentsSdk_keys: getObjectKeysSafe(onlinePaymentsSdkGlobal),
    window_Worldline_keys: getObjectKeysSafe(worldlineGlobal),
  }
}
    ...
    ;(async () => {
      try {
        ...
        const initPromise = inFlightInit ?? (async () => {
          const runtimeDiagnostics = getWorldlineRuntimeDiagnostics(OnlinePaymentsSdk as any)
          console.info("[WorldlineInlineCardForm] SDK import diagnostics", runtimeDiagnostics)

          const runtimeSdk = resolveWorldlineRuntimeSdk(OnlinePaymentsSdk as any)
          if (!runtimeSdk) {
            console.error("[WorldlineInlineCardForm] SDK runtime resolve failed", runtimeDiagnostics)
