...
Route::group([
    ...
    $defaultPaymentProviders = [
        ['code' => 'stripe', 'name' => 'Stripe', 'enabled' => true, 'config' => ['publishable_key' => '', 'secret_key' => '', 'mode' => 'test']],
        ['code' => 'paypal', 'name' => 'PayPal', 'enabled' => true, 'config' => ['client_id' => '', 'secret' => '']],
        ['code' => 'worldline', 'name' => 'Worldline', 'enabled' => false, 'config' => ['api_key' => '', 'merchant_id' => '']],
        ['code' => 'sumup', 'name' => 'SumUp', 'enabled' => false, 'config' => ['api_key' => '']],
    ];
    ...
    Route::post('/payment-methods-admin', function (\Illuminate\Http\Request $request) use ($defaultPaymentMethods, $saveJsonSetting) {
    ...
    Route::get('/payment-providers-admin', function () use ($defaultPaymentProviders, $loadJsonSetting) {
        $defaults = $defaultPaymentProviders;
        $load = $loadJsonSetting;
        return response()->json([
            'success' => true,
            'data' => array_values($load('payment_providers', $defaults)),
        ]);
    });
