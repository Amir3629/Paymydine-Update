...
Route::get('/debug-host', function() {
    ...
    return response()->json([
        'request_getHost' => $host,
        'request_getHttpHost' => $httpHost,
        'SERVER_HTTP_HOST' => $_SERVER['HTTP_HOST'] ?? 'not set',
        'SERVER_SERVER_NAME' => $_SERVER['SERVER_NAME'] ?? 'not set',
        'tenant_found' => $tenant ? true : false,
        'tenant' => $tenant,
        'all_tenants' => DB::connection('mysql')->table('tenants')->get(['domain', 'database', 'status'])
    ]);
})->withoutMiddleware([\Igniter\Flame\Foundation\Http\Middleware\TenantDatabaseMiddleware::class]);
