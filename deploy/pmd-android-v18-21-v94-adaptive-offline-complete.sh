...
# PMD_ANDROID_V18_21_V94_ADAPTIVE_OFFLINE_COMPLETE
PMD_ROOT="${PMD_ROOT:-/var/www/paymydine}"

PRE_V1817_SNAPSHOT="${PRE_V1817_SNAPSHOT:-04919ec9fb0a2bb54bf47c45f29014dbd505a9eb}"
QPOS_V89_BASE="${QPOS_V89_BASE:-74cf7fdf0a31b3cf50cf1ecb2f394d9cf1751b4f}"
...
log(){ printf '\n[PMD V18.21 / ANDROID 0.3.20 / ADAPTIVE V94] %s\n' "$*"; }
fail(){ printf '\n[PMD V18.21][ERROR] %s\n' "$*" >&2; exit 1; }
...
for ref in "$PRE_V1817_SNAPSHOT" "$QPOS_V89_BASE" "$QPOS_V92_SOURCE" "$QPOS_V94_SOURCE" "$PHP_V23_TARGET" "$ANDROID_SOURCE_COMMIT"; do
  "${GIT[@]}" cat-file -e "${ref}^{commit}" 2>/dev/null ||
    fail "Required commit unavailable: $ref"
done

"${GIT[@]}" merge-base --is-ancestor "$QPOS_V89_BASE" "$QPOS_V92_SOURCE" ||
  fail "V89 is not an ancestor of the V92 source."
...
STAGE="$PMD_ROOT/storage/pmd-v1821-v94-0320-$STAMP"
CONFLICT="$PMD_ROOT/storage/pmd-v1821-v94-conflicts-$STAMP"
BACKUP_DIR="$PMD_ROOT/storage/pmd-deploy-backups"
BACKUP="$BACKUP_DIR/android-v1821-0320-v94-before-$STAMP.tar.gz"
META="$BACKUP_DIR/android-v1821-0320-v94-before-$STAMP.txt"
...

{
  ...
  echo "qpos_v89_base=$QPOS_V89_BASE"
...
# ------------------------------------------------------------------
# JS: reconstruct the exact V18.17 merge baseline, then upgrade that
# baseline V92 -> V94 while preserving pre-V18.17 live customizations.
# If live already has V93 offline-order controls, leave it untouched.
# ------------------------------------------------------------------
log "Preparing JS from the actual V18.17 merge lineage..."
JS_PRE="$STAGE/source/js-pre-v1817.js"
JS_V89="$STAGE/source/js-v89.js"
JS_V92="$STAGE/source/js-v92.js"
JS_V94="$STAGE/source/js-v94.js"
JS_EXPECTED_V92="$STAGE/source/js-expected-v1817.js"
JS_EXPECTED_V94="$STAGE/source/js-expected-v94.js"
JS_CANDIDATE="$STAGE/candidate/$JS_FILE"

"${GIT[@]}" show "$PRE_V1817_SNAPSHOT:$JS_FILE" > "$JS_PRE"
"${GIT[@]}" show "$QPOS_V89_BASE:$JS_FILE" > "$JS_V89"
"${GIT[@]}" show "$QPOS_V92_SOURCE:$JS_FILE" > "$JS_V92"
"${GIT[@]}" show "$QPOS_V94_SOURCE:$JS_FILE" > "$JS_V94"

set +e
git merge-file -p --diff3   -L "PRE_V1817" -L "V89" -L "V92"   "$JS_PRE" "$JS_V89" "$JS_V92" > "$JS_EXPECTED_V92"
rc=$?
set -e
[[ $rc -eq 0 ]] ||
  fail "Could not reconstruct the V18.17 JS baseline from its original lineage."

set +e
git merge-file -p --diff3   -L "V18.17" -L "V92_SOURCE" -L "V94_SOURCE"   "$JS_EXPECTED_V92" "$JS_V92" "$JS_V94" > "$JS_EXPECTED_V94"
rc=$?
set -e
[[ $rc -eq 0 ]] ||
  fail "Could not reconstruct the V94 JS target while preserving pre-V18.17 customizations."
...
if grep -q "PMD_QPOS_NATIVE_PROVISIONAL_CHECK_IDS_V93" "$PMD_ROOT/$JS_FILE" &&
  ...
  set +e
  git merge-file -p --diff3     -L "LIVE:$JS_FILE"     -L "EXPECTED_V18.17:$JS_FILE"     -L "EXPECTED_V94:$JS_FILE"     "$PMD_ROOT/$JS_FILE" "$JS_EXPECTED_V92" "$JS_EXPECTED_V94"     > "$JS_CANDIDATE"
  rc=$?
  set -e
  if [[ $rc -ne 0 ]] ||
    ...
    fail "JS lineage merge conflict. Nothing installed. Candidate: $CONFLICT/$JS_FILE"
  fi
fi
...
sed -E -i   's#pmd-quick-pos-v1\.css\?v=[^"]+#pmd-quick-pos-v1.css?v=20260924-offline-refresh-v1821#g'   "$VIEW"
sed -E -i   's#pmd-quick-pos-v1\.js\?v=[^"]+#pmd-quick-pos-v1.js?v=20260924-offline-complete-v1821#g'   "$VIEW"
...
grep -q 'pmd-quick-pos-v1.css?v=20260924-offline-refresh-v1821' "$VIEW" ||
  fail "CSS refresh identity missing."
grep -q 'pmd-quick-pos-v1.js?v=20260924-offline-complete-v1821' "$VIEW" ||
...
apk_tmp="$WEB_APK.pmd-v1821-new"
sha_tmp="$WEB_SHA.pmd-v1821-new"
...
grep -n 'pmd-quick-pos-v1.js?v=20260924-offline-complete-v1821' "$PMD_ROOT/$VIEW_FILE" | head -1
...
=============================================================
PayMyDine V18.21 / Android 0.3.20 / Adaptive V94 installed.
...
Adaptive deployment:
  ...
  - JS V18.17 baseline is reconstructed from the actual Clean VPS snapshot
    and original V89 -> V92 merge before applying V92 -> V94 changes.
