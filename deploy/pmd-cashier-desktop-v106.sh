...
BRANCH="feature/cashier-desktop-hardware-truth-v106"
VIEW="app/admin/views/pmddevices/index.blade.php"
PATCH="deploy/pmd-cashier-desktop-v106-downloads-patch.py"
WORKFLOW=".github/workflows/cashier-desktop-v106.yml"
...
STAGE="$(mktemp -d /tmp/pmd-cashier-v106-stage.XXXXXX)"
BACKUP=""
ACTIVATED=0
...
rollback() {
  ...
  echo "AUTOMATIC PMD CASHIER V1.0.6 DOWNLOAD UI ROLLBACK"
  ...
  echo "PMD CASHIER V1.0.6 DOWNLOAD UI ROLLBACK COMPLETE"
}

refuse() {
  echo "PMD CASHIER V1.0.6 REFUSED: $1" >&2
  rollback
  exit 1
}
...
log "PMD CASHIER DESKTOP V1.0.6 - HARDWARE TRUTH + FULLSCREEN"
...
[[ -n "$BUILD_SOURCE_SHA" ]] || refuse "could not determine latest V1.0.6 build source commit"

log "3. REQUIRE RELEASE FROM LATEST V1.0.6 BUILD SOURCE"
...
[[ "$RELEASE_TARGET" == "$BUILD_SOURCE_SHA" ]] || refuse "release is not built from latest V1.0.6 source yet"

asset_ok "PayMyDine-Cashier-Setup-1.0.6.exe" || refuse "Windows V1.0.6 release asset is not ready"
asset_ok "PayMyDine-Cashier-1.0.6-mac-arm64.dmg" || refuse "Mac Apple Silicon V1.0.6 release asset is not ready"
asset_ok "PayMyDine-Cashier-1.0.6-mac-x64.dmg" || refuse "Mac Intel V1.0.6 release asset is not ready"

mkdir -p "$STAGE/$(dirname "$VIEW")" "$STAGE/deploy"
cp -p "$PMD_ROOT/$VIEW" "$STAGE/$VIEW" || refuse "could not stage live Devices view"
git show "FETCH_HEAD:$PATCH" > "$STAGE/$PATCH" || refuse "could not read V1.0.6 patcher"
...
grep -q 'PMD_CASHIER_DESKTOP_DOWNLOADS_V106' "$STAGE/$VIEW" || refuse "V1.0.6 marker missing"
grep -q '/brand/paymydine-logo.svg' "$STAGE/$VIEW" || refuse "PayMyDine card logo missing"
grep -q 'PayMyDine-Cashier-Setup-1.0.6.exe' "$STAGE/$VIEW" || refuse "Windows V1.0.6 link missing"
grep -q 'PayMyDine-Cashier-1.0.6-mac-arm64.dmg' "$STAGE/$VIEW" || refuse "Mac arm64 V1.0.6 link missing"
grep -q 'PayMyDine-Cashier-1.0.6-mac-x64.dmg' "$STAGE/$VIEW" || refuse "Mac x64 V1.0.6 link missing"

STAGE_SHA="$(sha256sum "$STAGE/$VIEW" | awk '{print $1}')"
echo "STAGE_VIEW_SHA=$STAGE_SHA"

log "5. BACKUP + ACTIVATE ONE VIEW ONLY"
BACKUP="$PMD_ROOT/storage/app/pmd-backups/cashier-v106-$(date +%Y%m%d-%H%M%S)"
...
log "PMD CASHIER DESKTOP V1.0.6 DOWNLOADS DEPLOYED"
echo "FILES_CHANGED=1_VIEW_ONLY"
echo "DESKTOP_APP_VERSION=1.0.6"
echo "HARDWARE_TESTS_TRUTHFUL=YES"
echo "OFFLINE_PRINTER_PREFLIGHT=YES"
echo "PHYSICAL_DRAWER_CONFIRMATION=HUMAN_REQUIRED"
echo "CASHIER_FULLSCREEN=YES"
echo "LOGIN_LOGOUT_COMPACT_WINDOW=YES"
echo "VIRTUAL_PDF_MODE=YES"
echo "MAC_FINAL_BUNDLE_SIGNATURE=VALID_ADHOC_UNLESS_DEVELOPER_ID_CONFIGURED"
echo "APPLE_NOTARIZED=NO_UNLESS_CI_CREDENTIALS_ARE_CONFIGURED"
echo "PAYMENT_BACKEND_CHANGED=NO"
echo "DB_CHANGES=NO"
echo "LIVE_GIT_HEAD_MOVED=NO"
echo "Next: hard-refresh Devices, download V1.0.6, replace the old app, and test direct opening + Virtual PDF."
