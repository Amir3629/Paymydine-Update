...
BRANCH="feature/cash-drawer-local-agent-r2-3"
TEST_HOST="${TEST_HOST:-a.paymydine.com}"
PHP_FPM="${PHP_FPM:-php8.3-fpm}"
BOOTSTRAP_TEST_DRAWER="${BOOTSTRAP_TEST_DRAWER:-0}"
STAMP="$(date +%Y%m%d_%H%M%S)"
STAGE="$(mktemp -d /tmp/pmd-cash-drawer-r23.XXXXXX)"
BACKUP="$ROOT/storage/pmd-cash-drawer-r23-backups/$STAMP"
ACTIVATED=0
...
fail() {
  echo "PMD CASH DRAWER R2.3 REFUSED: $*" >&2
  exit 1
}
...
rollback_code() {
  ...
  echo
  echo "AUTOMATIC PMD CASH DRAWER R2.3 CODE ROLLBACK"
...
PRE_SETTINGS="$(http_code "https://$TEST_HOST/api/v1/settings?r23=$(date +%s)")"
PRE_MENU="$(http_code "https://$TEST_HOST/api/v1/menu?r23=$(date +%s)")"
PRE_ADMIN="$(http_code "https://$TEST_HOST/admin/pmddevices?r23=$(date +%s)")"
PRE_ROOT="$(http_code "https://$TEST_HOST/?r23=$(date +%s)")"
...
log "2. FETCH CASH-DRAWER-ONLY R2.3"
...
log "3. PATCH ONLY CASH DRAWER AUTHORITIES"
...
python3 -m py_compile "$STAGE/r2-patch.py" "$STAGE/r23-patch.py" || fail "Python patcher syntax failed"
...
grep -q "PMD_LOCAL_POS_AGENT_R1_ROUTE_LOADER" "$STAGE/files/routes/api.php" || fail "Agent route loader missing"
grep -q "prefix('v1/pmd-pos-agent')" "$STAGE/files/routes/pmd-pos-agent-r1.php" || fail "Agent API v1 route marker missing"
grep -q '/api/v1/pmd-pos-agent/agent.js' "$STAGE/files/app/admin/controllers/CashDrawers.php" || fail "Windows connector is not using tenant-safe API v1 Agent URL"
grep -q 'PMD_CASH_DRAWER_SNAKE_ACTION_ALIASES_R23' "$STAGE/files/app/admin/controllers/CashDrawers.php" || fail "Connector download action aliases missing"
...
# Explicit guard: this deploy must not depend on or modify receipt authorities.
if grep -q 'PMD_CASHIER_RECEIPT_TENANT_AUTHORITY' "$STAGE/r23-patch.py"; then
  fail "R2.3 patcher unexpectedly contains receipt authority logic"
fi
...
echo "PMD CASH DRAWER R2.3 CODE ROLLBACK COMPLETE"
echo "Additive tenant schema changes are intentionally preserved."
ROLLBACK
chmod 700 "$BACKUP/rollback.sh"

log "7. ACTIVATE CASH DRAWER R2.3"
...
log "9. POST-DEPLOY CASH DRAWER VERIFICATION"
POST_SETTINGS="$(http_code "https://$TEST_HOST/api/v1/settings?r23=$(date +%s)")"
POST_MENU="$(http_code "https://$TEST_HOST/api/v1/menu?r23=$(date +%s)")"
POST_ADMIN="$(http_code "https://$TEST_HOST/admin/pmddevices?r23=$(date +%s)")"
POST_ROOT="$(http_code "https://$TEST_HOST/?r23=$(date +%s)")"
AGENT_CODE="$(curl -k -sS -o /tmp/pmd-agent-r23.out -w '%{http_code}' "https://$TEST_HOST/api/v1/pmd-pos-agent/agent.js?r23=$(date +%s)")"
PULL_CODE="$(curl -k -sS -o /tmp/pmd-agent-r23-pull.out -w '%{http_code}' "https://$TEST_HOST/api/v1/pmd-pos-agent/pull?device_code=PMD-UNAUTHORIZED-R23")"
...
grep -q 'PayMyDine Local POS Agent' /tmp/pmd-agent-r23.out || fail "Served Agent body is invalid"
grep -q '/api/v1/pmd-pos-agent/pair' /tmp/pmd-agent-r23.out || fail "Served Agent is not using tenant-safe API v1 pairing"

grep -q 'PMD_CASH_DRAWER_SNAKE_ACTION_ALIASES_R23' "$ROOT/app/admin/controllers/CashDrawers.php" || fail "Live connector download aliases missing"
...
do
  ...
  SERVED_HASH="$(curl -k -fsSL "https://$TEST_HOST/$rel?r23=$(date +%s%N)" | sha256sum | awk '{print $1}')"
  echo "$rel"
...
ACTIVATED=0
log "PMD CASH DRAWER + LOCAL POS AGENT R2.3 DEPLOYED"
echo "Receipt/invoice authorities were intentionally not touched by R2.3."
echo "Next: re-download Windows connector, verify /health and /printers, then Test drawer/Diagnose."
