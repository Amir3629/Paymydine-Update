...
# PMD_ANDROID_V18_19_V94_OFFLINE_COMPLETE_SAFE
...
PHP_BASE_COMMIT="${PHP_BASE_COMMIT:-01b91443b0c7fcf2a0dd75079c0c78e678a7c284}"
PHP_TARGET_COMMIT="${PHP_TARGET_COMMIT:-aed74be6250a1f125ac89dfb09a1dadd25b2d364}"
...
log(){ printf '\n[PMD V18.19 / ANDROID 0.3.20 / V94 SAFE] %s\n' "$*"; }
fail(){ printf '\n[PMD V18.19][ERROR] %s\n' "$*" >&2; exit 1; }
...
log "Checking expected live V18.17/V18.14 baseline..."
grep -q "PMD_QPOS_REQUEST_FAILOVER_V91" "$PMD_ROOT/app/admin/assets/js/pmd-quick-pos-v1.js" ||
  fail "Live Quick POS is not on the expected V91/V92 failover baseline."
grep -q "PMD_QPOS_NATIVE_DURABLE_MUTATIONS_V92" "$PMD_ROOT/app/admin/assets/js/pmd-quick-pos-v1.js" ||
  fail "Live Quick POS durable V92 marker missing."
grep -q "PMD_ANDROID_CANONICAL_QPOS_SETTINGS_V18" "$PMD_ROOT/app/Services/PmdMobileSync/PmdMobileBootstrapService.php" ||
  fail "Live canonical V18 bootstrap marker missing."
grep -q "PMD_MOBILE_OFFLINE_ORIGINAL_TIME_V18" "$PMD_ROOT/app/Services/PmdMobileSync/PmdMobileCommandProcessor.php" ||
  fail "Live original-time V18 server baseline missing."

log "Validating Android 0.3.20 source contract..."
...
git_show_contains "$ANDROID_SOURCE_COMMIT" "app/admin/assets/js/pmd-quick-pos-v1.js" "PMD_QPOS_NATIVE_PROVISIONAL_CHECK_IDS_V93" ||
  fail "Negative provisional check selection missing."
...
git_show_contains "$ANDROID_SOURCE_COMMIT" "mobile/android/app/src/main/java/com/paymydine/mobile/data/local/LocalPosRepository.kt" "PMD_ANDROID_PROVISIONAL_ORDER_EDIT_V23" ||
  fail "Queued provisional editing missing."
...
git_show_not_contains "$ANDROID_SOURCE_COMMIT" "mobile/android/app/src/main/java/com/paymydine/mobile/ui/PmdStaffLogin.kt" "Continue offline as " ||
  fail "Legacy separate offline button is present."

STAMP="$(date -u +%Y%m%dT%H%M%SZ)"
STAGE="$PMD_ROOT/storage/pmd-v1819-v94-0320-$STAMP"
CONFLICT="$PMD_ROOT/storage/pmd-v1819-v94-conflicts-$STAMP"
BACKUP_DIR="$PMD_ROOT/storage/pmd-deploy-backups"
BACKUP="$BACKUP_DIR/android-v1819-0320-v94-before-$STAMP.tar.gz"
META="$BACKUP_DIR/android-v1819-0320-v94-before-$STAMP.txt"
...
merge_protected() {
  ...
  if [[ $rc -ne 0 ]] ||
    ...
    fail "Protected merge conflict in $rel. No V18.19 files installed. Candidate: $CONFLICT/$rel"
  fi
}

log "Building protected V92 -> V94 CSS/JS candidates..."
for rel in "${QPOS_FILES[@]}"; do
  merge_protected "$rel" "$QPOS_BASE_COMMIT" "$QPOS_TARGET_COMMIT" "V94"
done

log "Building protected V18 -> V23 server-sync candidates..."
for rel in "${PHP_FILES[@]}"; do
  merge_protected "$rel" "$PHP_BASE_COMMIT" "$PHP_TARGET_COMMIT" "V23"
done

log "Preserving live Blade/Settings and applying semantic release pointers..."
...
log "Validating V94 offline-complete candidates..."
...
grep -q "PMD_QPOS_FLOATING_CART_MODAL_SAFETY_V94" "$CSS" ||
  fail "V94 modal-safe floating cart missing."
...
! grep -Fq '\n<script src="/app/admin/assets/js/pmd-quick-pos-v1.js' "$VIEW" ||
  fail "Literal backslash-n found before Quick POS JS asset."
...
grep -q "PMD_MOBILE_OFFLINE_CASH_CAPABILITY_V23" "$BOOTSTRAP" ||
  fail "Offline cash capability V23 missing."
grep -Fq "'offline_payment_enabled' => true" "$BOOTSTRAP" ||
  fail "Offline payment capability not enabled."
grep -Fq "'offline_cash_payment_enabled' => true" "$BOOTSTRAP" ||
  fail "Offline cash capability not enabled."
...
grep -q "client_created_at_ms" "$COMMAND" ||
  fail "Original client order timestamp missing."
...
if command -v node >/dev/null 2>&1; then
  node --check "$JS" >/dev/null ||
    fail "Quick POS JS syntax failed."
fi

log "Installing validated V18.19 candidates..."
ALL_INSTALL_FILES=(
  "${QPOS_FILES[@]}"
  "${PHP_FILES[@]}"
  "$VIEW_FILE"
...
apk_tmp="$WEB_APK.pmd-v1819-new"
sha_tmp="$WEB_SHA.pmd-v1819-new"
...
=============================================================
PayMyDine V18.19 / Android 0.3.20 / Quick POS V94 installed.

Deployment safety:
  - CSS/JS merged incrementally from the actually deployed V92 source.
  - PHP sync files merged from the actually deployed V18.14 server baseline.
  - Blade View preserved from live; only CSS/JS cache identities changed.
  - Settings preserved from live; only Android 0.3.20 pointer/label changed.

Offline POS:
  - local provisional orders are selectable
  ...
  - client placement time is preserved through later sync
