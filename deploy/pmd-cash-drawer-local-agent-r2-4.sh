...
STAGE="$(mktemp -d /tmp/pmd-cash-drawer-r24.XXXXXX)"
BACKUP="$ROOT/storage/pmd-cash-drawer-r24-backups/$STAMP"
ACTIVATED=0
...
fail() {
  echo "PMD CASH DRAWER R2.4 REFUSED: $*" >&2
  exit 1
}
...
rollback_code() {
  ...
  echo
  echo "AUTOMATIC PMD CASH DRAWER R2.4 CODE ROLLBACK"
...
PRE_SETTINGS="$(http_code "https://$TEST_HOST/api/v1/settings?r24=$(date +%s)")"
PRE_MENU="$(http_code "https://$TEST_HOST/api/v1/menu?r24=$(date +%s)")"
PRE_ADMIN="$(http_code "https://$TEST_HOST/admin/pmddevices?r24=$(date +%s)")"
PRE_ROOT="$(http_code "https://$TEST_HOST/?r24=$(date +%s)")"
...
repo_git show "$FETCH_REF:deploy/pmd-cash-drawer-local-agent-r2-4-patch.py" > "$STAGE/r24-patch.py" || fail "R2.4 patcher missing"
...
log "3. PATCH CASH DRAWER + EXPLICIT ADMIN ROUTE AUTHORITY"
...
python3 "$STAGE/r24-patch.py" "$STAGE/files" || fail "R2.4 route authority patch failed"
...
python3 -m py_compile "$STAGE/r2-patch.py" "$STAGE/r23-patch.py" "$STAGE/r24-patch.py" || fail "Python patcher syntax failed"
...
# One route owner only: Admin ServiceProvider -> app/admin/routes.php.
...
echo "PMD CASH DRAWER R2.4 CODE ROLLBACK COMPLETE"
echo "Additive tenant schema changes are intentionally preserved."
ROLLBACK
chmod 700 "$BACKUP/rollback.sh"

log "7. ACTIVATE CASH DRAWER R2.4"
...
log "8. LARAVEL ROUTE REGISTRY PROOF"
...
log "9. OPTIONAL TEST-TENANT BOOTSTRAP"
...
log "10. POST-DEPLOY CASH DRAWER HTTP VERIFICATION"
POST_SETTINGS="$(http_code "https://$TEST_HOST/api/v1/settings?r24=$(date +%s)")"
POST_MENU="$(http_code "https://$TEST_HOST/api/v1/menu?r24=$(date +%s)")"
POST_ADMIN="$(http_code "https://$TEST_HOST/admin/pmddevices?r24=$(date +%s)")"
POST_ROOT="$(http_code "https://$TEST_HOST/?r24=$(date +%s)")"
AGENT_CODE="$(curl -k -sS -o /tmp/pmd-agent-r24.out -w '%{http_code}' "https://$TEST_HOST/api/v1/pmd-pos-agent/agent.js?r24=$(date +%s)")"
PULL_CODE="$(curl -k -sS -o /tmp/pmd-agent-r24-pull.out -w '%{http_code}' "https://$TEST_HOST/api/v1/pmd-pos-agent/pull?device_code=PMD-UNAUTHORIZED-R24")"
...
[[ "$AGENT_CODE" == "200" ]] || fail "Nginx/Laravel Agent package route is not 200"
[[ "$PULL_CODE" == "401" ]] || fail "Unauthenticated Agent pull must be 401, got $PULL_CODE"
grep -q 'PayMyDine-LocalPosAgent' /tmp/pmd-agent-r24.out || fail "Served Agent body is invalid"
grep -q '/api/v1/pmd-pos-agent/pair' /tmp/pmd-agent-r24.out || fail "Served Agent is not using API v1 pairing"

grep -q 'PMD_LOCAL_POS_AGENT_ADMIN_ROUTE_LOADER_R24' "$ROOT/app/admin/routes.php" || fail "Live Admin Agent route loader missing"
...
do
  ...
  SERVED_HASH="$(curl -k -fsSL "https://$TEST_HOST/$rel?r24=$(date +%s%N)" | sha256sum | awk '{print $1}')"
  echo "$rel"
...
ACTIVATED=0
log "PMD CASH DRAWER + LOCAL POS AGENT R2.4 DEPLOYED"
echo "Agent route authority: Admin ServiceProvider -> app/admin/routes.php -> /api/v1/pmd-pos-agent/*"
echo "Receipt/invoice authorities were intentionally not touched by R2.4."
echo "Next: re-download Windows connector, verify /health and /printers, then Diagnose/Test drawer."
