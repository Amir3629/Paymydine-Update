...
rollback() {
  ...
  echo "AUTOMATIC PMD CASHIER V1.0.5 DOWNLOAD UI ROLLBACK"
  ...
  echo "PMD CASHIER V1.0.5 DOWNLOAD UI ROLLBACK COMPLETE"
}

refuse() {
  echo "PMD CASHIER V1.0.5 REFUSED: $1" >&2
  rollback
  exit 1
}
...
git show "FETCH_HEAD:$PATCH" > "$STAGE/$PATCH" || refuse "could not read V1.0.5 patcher"
...
BACKUP="$PMD_ROOT/storage/app/pmd-backups/cashier-v105-$(date +%Y%m%d-%H%M%S)"
