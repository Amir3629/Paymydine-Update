...
class WorldlineHostedCheckoutService
{
    ...
    private function pmdGetInputEmailFromGlobals()
    {
        try {
            ...
            $pmdHostedProbeResult = null;
            $pmdHostedProbeError = null;
            ...
            try {
                ...
                if (is_object($pmdHostedProbeResponse) && method_exists($pmdHostedProbeResponse, 'toObject')) {
                    $pmdHostedProbeResult = json_decode(json_encode($pmdHostedProbeResponse->toObject()), true);
                } else {
                    $pmdHostedProbeResult = json_decode(json_encode($pmdHostedProbeResponse), true);
                }

                \Log::warning('PMD WORLDLINE HOSTED CHECKOUT PROBE SUCCESS', [
                    'message' => 'Direct create-payment failed but hosted checkout create succeeded',
                    'hosted_probe_response' => $this->sanitizeForLogs(is_array($pmdHostedProbeResult) ? $pmdHostedProbeResult : []),
                ]);
                ...
                try {
                    if (method_exists($probeEx, 'getErrors')) {
                        $pmdProbeErrors = json_decode(json_encode($probeEx->getErrors()), true);
                    }
                ...
                \Log::warning('PMD WORLDLINE HOSTED CHECKOUT PROBE FAILED', $pmdHostedProbeError);
            }
            ...
            \Log::error('WORLDLINE INLINE CREATE PAYMENT ERROR', [
                ...
                'statusCode' => method_exists($e, 'getStatusCode') ? $e->getStatusCode() : null,
                'responseBody' => method_exists($e, 'getResponseBody') ? $e->getResponseBody() : null,
                'errors' => method_exists($e, 'getErrors') ? json_decode(json_encode($e->getErrors()), true) : null,
                'hosted_probe_result' => $this->sanitizeForLogs(is_array($pmdHostedProbeResult) ? $pmdHostedProbeResult : []),
                'hosted_probe_error' => $pmdHostedProbeError,
