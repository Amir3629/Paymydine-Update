...
export function WorldlineInlineCardForm({
  ...
  const validateWorldlineFields = (nextData: typeof formData) => {
    ...
    const expiryMonthField = getProductField(paymentProduct, "expiryMonth")
    const expiryYearField = getProductField(paymentProduct, "expiryYear")
    const cvvField = getProductField(paymentProduct, "cvv") || paymentProduct.getField("securityCode")
  ...
  const applyFieldMask = (fieldId: "cardNumber" | "expiryDate" | "cvv", value: string) => {
    if (!paymentProduct) return value
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const availableFieldIds = Array.isArray(paymentProduct?.paymentProductFields)
        ? paymentProduct.paymentProductFields.map((f: any) => f?.id).filter(Boolean)
        : (
