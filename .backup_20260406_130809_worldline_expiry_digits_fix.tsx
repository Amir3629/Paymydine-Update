...
export function WorldlineInlineCardForm({
  ...
  const updateField = (name: keyof typeof formData, value: any) => {
    ...
    if (name === "cardNumber") {
      const digits = nextValue.replace(/\D/g, "").slice(0, 16)
      ...
      nextValue = nextValue.slice(0, 80)
    } else if (name === "email") {
      nextValue = nextValue.slice(0, 120)
    }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const cardNumber = normalizeCardNumber(formData.cardNumber)
      const expiryDigits = getExpiryDigits(formData.expiryDate)
      const expiryMasked =
        expiryDigits.length >= 3
      ...
      const cvv = normalizeCvv(formData.cvv)
      const cardholderName = String(formData.cardholderName ?? "").trim()

      const getReqField = (fieldId: string) => {
        try {
          if (typeof paymentRequest?.getField === "function") return paymentRequest.getField(fieldId)
        } catch {}
        try {
          if (typeof paymentRequest?.field === "function") return paymentRequest.field(fieldId)
        } catch {}
        return null
      }
      ...
      const safeSet = (fieldId: string, value: string) => {
        ...
        try {
          const field = getReqField(fieldId)
      ...
      const safeValidateField = (fieldId: string) => {
        try {
          const field = getReqField(fieldId)
          if (!field || typeof field.validate !== "function") return null
          return field.validate()
        } catch (e: any) {
          return { isValid: false, errorMessage: String(e?.message || e) }
        }
      }

      const setResults = {
        cardNumber: safeSet("cardNumber", cardNumber),
        expiryDate: safeSet("expiryDate", expiryMasked),
        expiryMonth: expiryMonth ? safeSet("expiryMonth", expiryMonth) : null,
        expiryYear: expiryYear ? safeSet("expiryYear", expiryYear) : null,
        cvv: safeSet("cvv", cvv),
        cardholderName: safeSet("cardholderName", cardholderName),
      }
      ...
      const fieldValidation = {
        cardNumber: safeValidateField("cardNumber"),
        expiryDate: safeValidateField("expiryDate"),
        expiryMonth: safeValidateField("expiryMonth"),
        expiryYear: safeValidateField("expiryYear"),
        cvv: safeValidateField("cvv"),
        cardholderName: safeValidateField("cardholderName"),
      }
      ...
      console.info("[WorldlineInlineCardForm] VALIDATION PROBE", {
