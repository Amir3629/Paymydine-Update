...
foreach ($lastPayments as $payment) {
    ...
    foreach ($historyTables as $label => $table) {
        try {
            ...
            if (!in_array('object_id', $columns) && !in_array('order_id', $columns)) {
                echo "  $label: جدول یا ستون مناسب برای فیلتر پیدا نشد!\n";
                continue;
            }

            if (in_array('object_id', $columns)) {
                $rows = DB::select("SELECT * FROM `$table` WHERE object_type = 'order' AND object_id = ?", [$payment->payment_id]);
            } else {
                $rows = DB::select("SELECT * FROM `$table` WHERE `order_id` = ?", [$payment->payment_id]);
            }
