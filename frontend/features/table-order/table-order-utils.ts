...
export function isVisibleTableOrderDraft(draft: TableOrderDraftResponse | null | undefined): draft is TableOrderDraftResponse {
  if (!draft?.success || !draft.status) return false

  // PMD_FIX_TABLE_ORDER_VISIBILITY_ACTIVE_ONLY_20260613
  // Visible means: draft/submitted/unpaid table order only.
  // Paid/completed/delivered/cancelled orders must not keep the Table Order icon alive.
  const status = String((draft as any)?.status || "").toLowerCase()
  const paymentStatus = String((draft as any)?.paymentStatus || (draft as any)?.payment_status || (draft as any)?.totals?.paymentStatus || "").toLowerCase()
  const remainingRaw = Number(
    (draft as any)?.remainingAmount ??
    (draft as any)?.remaining_amount ??
    (draft as any)?.totals?.remainingAmount ??
    (draft as any)?.totals?.remaining_amount ??
    Number.NaN
  )

  if (status === "empty") return false
  if (["paid", "completed", "complete", "delivered", "cancelled", "canceled"].includes(status)) return false
  if (["paid", "settled"].includes(paymentStatus)) return false
  if (Number.isFinite(remainingRaw) && remainingRaw <= 0 && status !== "draft") return false
