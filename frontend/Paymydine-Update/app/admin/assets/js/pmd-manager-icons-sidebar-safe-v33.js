(function () {
  ...
  function svg(name) {
    ...
    return '<svg class="pmd-svg-v33" viewBox="0 0 24 24" aria-hidden="true">' + (icons[name] || icons.alert) + '</svg>';
  }
  ...
  function kpi(label, value, sub, icon, href, tone) {
    return '<div class="pmd-role-kpi ' + (tone || '') + '" data-pmd-role-href="' + href + '">' +
  ...
  function action(label, icon, href) {
    return '<a class="pmd-manager-action-v29" href="' + href + '"><i class="pmd-inline-icon-v33">' + svg(icon) + '</i><span>' + label + '</span></a>';
  }

  function card(cls, icon, tone, title, sub, body, pill) {
    return '<section class="pmd-manager-card-v29 ' + cls + '">' +
      '<div class="pmd-manager-card-head-v29">' +
        '<div class="pmd-manager-title-v29">' +
          '<div class="pmd-manager-title-icon-v29 ' + (tone || '') + '">' + svg(icon) + '</div>' +
          '<div><div class="pmd-manager-card-title-v29">' + title + '</div>' +
          (sub ? '<div class="pmd-manager-card-sub-v29">' + sub + '</div>' : '') +
          '</div>' +
        '</div>' +
        (pill ? '<div class="pmd-manager-live-pill-v29">' + svg('arrow') + pill + '</div>' : '') +
      '</div>' +
      body +
    '</section>';
  }
  ...
  function financeItem(label, value, note) {
    return '<div class="pmd-manager-finance-item-v29"><div class="pmd-manager-finance-label-v29">' + label + '</div><div class="pmd-manager-finance-value-v29">' + value + '</div>' + (note ? '<div class="pmd-manager-finance-note-v29">' + note + '</div>' : '') + '</div>';
  }
  ...
  function alertItem(icon, title, detail, badge) {
    return '<div class="pmd-manager-alert-v29"><i class="pmd-inline-icon-v33">' + svg(icon) + '</i><div><strong>' + title + '</strong><span>' + detail + '</span></div><em class="pmd-manager-badge-v29">' + badge + '</em></div>';
  }
  function eventItem(icon, title, detail, time) {
    return '<div class="pmd-manager-event-v29"><i class="pmd-inline-icon-v33">' + svg(icon) + '</i><div><strong>' + title + '</strong><span>' + detail + '</span></div><time>' + time + '</time></div>';
  }
  ...
  function insight(icon, title, detail, badge) {
    return '<div class="pmd-manager-insight-v29"><i class="pmd-inline-icon-v33">' + svg(icon) + '</i><div><strong>' + title + '</strong><span>' + detail + '</span></div><em class="pmd-manager-badge-v29">' + badge + '</em></div>';
  }
  ...
  function managerHtml() {
    ...
    var openChecks = '8', activeTables = '17', alerts = '4';

    return '<div class="pmd-manager-ops-v29 pmd-manager-ops-v33" data-pmd-manager-ops="v33">' +
      ...
      '<div class="pmd-manager-grid-v29">' +
        ...
        card('wide', 'map', 'green', 'Live Restaurant Floor', 'Live status of every table: available, reserved, dining, waiting payment and delayed.',
          ...
          '</div><div class="pmd-manager-legend-v29">' +
        ...
        card('half', 'chart', 'gold', 'Performance Analytics', 'Revenue by hour, payment breakdown, average guest spend and lost revenue.',
          '<div class="pmd-manager-analytics-grid-v29"><div><div class="pmd-manager-card-sub-v29">Revenue by Hour</div><div class="pmd-manager-chart-bars-v29">' +
          ...
          '</div></div><div class="pmd-manager-metric-list-v29">' +
  ...
  function applyManagerDashboard(force) {
    var rt = root(), pn = panel();
    if (!rt || !pn || rt.getAttribute('data-pmd-role') !== 'manager') return;
    if (!force && pn.getAttribute('data-pmd-manager-ops-v33') === '1') return;
    renderCount += 1;
    if (renderCount > 20) return;
    pn.innerHTML = managerHtml();
    pn.setAttribute('data-pmd-manager-ops-v33', '1');
  }

  function bindRoleButtons() {
    document.querySelectorAll('[data-pmd-role-btn="manager"]').forEach(function (btn) {
      if (btn.getAttribute('data-pmd-manager-v33-bound')) return;
      btn.setAttribute('data-pmd-manager-v33-bound', '1');
      btn.addEventListener('click', function () {
        renderCount = 0;
        setTimeout(function () { applyManagerDashboard(true); }, 40);
        setTimeout(function () { applyManagerDashboard(true); }, 220);
      }, true);
    });
  }
  ...
  function stabilizeToggle() {
    var m = menu();
    if (!m) return;

    var slot = m.querySelector('.pmd-sidebar-toggle-safe-slot-v33');
    if (!slot) {
      slot = document.createElement('li');
      slot.className = 'pmd-sidebar-toggle-safe-slot-v33';
      m.appendChild(slot);
    }

    var buttons = Array.prototype.slice.call(document.querySelectorAll('.pmd-sidebar-icons-toggle'));
    if (!buttons.length) return;

    var keep = slot.querySelector('.pmd-sidebar-icons-toggle') || buttons[0];

    buttons.forEach(function (btn) {
      if (btn !== keep) {
        btn.classList.add('pmd-sidebar-toggle-hidden-duplicate-v33');
        if (btn.parentNode) btn.parentNode.removeChild(btn);
      }
    });

    if (keep.parentNode !== slot) slot.appendChild(keep);
    keep.classList.remove('pmd-sidebar-toggle-hidden-duplicate-v33');
    keep.style.position = 'static';

    var label = keep.querySelector('.pmd-sidebar-toggle-label-v33');
    if (!label) {
      label = document.createElement('span');
      label.className = 'pmd-sidebar-toggle-label-v33';
      keep.appendChild(label);
    }
    label.textContent = keep.getAttribute('aria-expanded') === 'false' ? 'Expand' : 'Collapse';

    if (m.lastElementChild !== slot) m.appendChild(slot);
  }
  ...
  ready(function () {
    ...
    // Finite retries only. No MutationObserver, no infinite loop.
    [150, 350, 700, 1200, 2000, 3200].forEach(function (delay) {
      setTimeout(function () {
        bindRoleButtons();
        applyManagerDashboard(false);
        stabilizeToggle();
    ...
    // Also sync label on click, finite and cheap.
    document.addEventListener('click', function (e) {
      if (e.target && e.target.closest && e.target.closest('.pmd-sidebar-icons-toggle')) {
        setTimeout(stabilizeToggle, 80);
      }
    }, true);

    window.PMDManagerSafeV33 = {
      refresh: function () { renderCount = 0; applyManagerDashboard(true); },
      stabilizeToggle: stabilizeToggle
    };
  });
})();
