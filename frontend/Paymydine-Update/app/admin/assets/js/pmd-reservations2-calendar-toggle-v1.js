(function () {
    ...
    return '' +
      '<div class="pmd-yc__toolbar">' +
        '<div class="pmd-yc__legend">' +
          ...
          '<span><i class="is-note">✎</i>Note</span>' +
          '<span><i class="is-event">★</i>Event</span>' +
        '</div>' +
        '<div class="pmd-yc__month-nav">' +
          '<button type="button" data-r2-yc-prev aria-label="Previous">←</button>' +
          '<strong data-r2-yc-label></strong>' +
          '<button type="button" data-r2-yc-next aria-label="Next">→</button>' +
        '</div>' +
        '<div class="pmd-yc__toolbar-right">' +
          '<div class="pmd-yc__view-switch">' +
            '<button type="button" data-r2-yc-view="year">Year</button>' +
            '<button type="button" data-r2-yc-view="month" class="is-active">Month</button>' +
          '</div>' +
          '<div class="pmd-yc__filters">' +
            '<button type="button" data-r2-yc-filter="all" class="is-active">All</button>' +
            '<button type="button" data-r2-yc-filter="reservations">Reservations</button>' +
            '<button type="button" data-r2-yc-filter="events">Events</button>' +
          '</div>' +
          '<button type="button" class="pmd-r2-yc-note-btn" data-r2-yc-note>＋ Note</button>' +
        '</div>' +
      '</div>' +
      '<main class="pmd-yc__months" data-r2-yc-months></main>' +
      '<section class="pmd-r2-yc-selected" data-r2-yc-selected hidden></section>' +
      '<div class="pmd-r2-yc-modal" data-r2-yc-modal hidden>' +
        ...
        '<section class="pmd-r2-yc-modal__card" role="dialog" aria-modal="true" aria-labelledby="pmd-r2-yc-modal-title">' +
          '<div class="pmd-r2-yc-modal__head">' +
            '<div><small>Calendar note</small><h2 id="pmd-r2-yc-modal-title" data-r2-yc-modal-title>Add note</h2></div>' +
            '<button type="button" data-r2-yc-modal-close aria-label="Close">×</button>' +
          '</div>' +
          '<textarea data-r2-yc-note-input rows="5" placeholder="Write a note for this day"></textarea>' +
          '<div class="pmd-r2-yc-modal__actions">' +
            '<button type="button" data-r2-yc-modal-delete>Delete</button>' +
            '<button type="button" data-r2-yc-modal-cancel>Cancel</button>' +
            '<button type="button" class="primary" data-r2-yc-modal-save>Save note</button>' +
          '</div>' +
        '</section>' +
      '</div>';
  }
  ...
  function monthRows(state) {
    ...
    if (filter === 'all' && state.note) {
      rows.push('<span class="pmd-r2-yc-entry is-note">' + esc(state.note) + '</span>');
    }
    if ((filter === 'all' || filter === 'events') && state.events.length) {
      state.events.slice(0, 2).forEach(function (event) {
  ...
  function yearDots(state) {
    var dots = [];
    if ((filter === 'all' || filter === 'reservations') && state.reservations.length) {
      dots.push('<i class="pmd-r2-yc-dot is-reservation" title="' + state.reservations.length + ' reservation' + (state.reservations.length === 1 ? '' : 's') + '"></i>');
    }
    if (filter === 'all' && state.note) {
      dots.push('<i class="pmd-r2-yc-dot is-note" title="Note"></i>');
    }
    if ((filter === 'all' || filter === 'events') && state.events.length) {
      dots.push('<i class="pmd-r2-yc-dot is-event" title="' + esc(state.events[0].title) + '"></i>');
    }
    return dots.join('');
  }
  ...
  function reservationCard(reservation) {
    ...
    return '' +
      '<article class="pmd-w5-card is-old pmd-v35-ready pmd-r2-yc-detail-card" data-reservation="' + esc(id) + '">' +
        '<div class="pmd-v35-card-head">' +
          '<div class="pmd-v35-table-no">' + esc(table || '—') + '</div>' +
          (editUrl !== '#' ? '<a class="pmd-v35-edit-btn" href="' + esc(editUrl) + '" title="Edit reservation" aria-label="Edit reservation">✎</a>' : '') +
        '</div>' +
        ...
        '<div class="pmd-w5-meta">' +
          ...
          '<div class="pmd-w5-box"><small>Time</small><b>' + esc(time) + '</b></div>' +
          '<div class="pmd-w5-box pmd-v35-order-box"><small>Reservation</small><b>' + esc(id || '—') + '</b></div>' +
        '</div>' +
        '<div class="pmd-w5-card-actions">' +
          (editUrl !== '#' ? '<a class="primary" href="' + esc(editUrl) + '">Edit</a>' : '') +
          '<button type="button" disabled><span>Status</span><small>' + esc(status) + '</small></button>' +
        '</div>' +
      '</article>';
  }
  ...
  function eventCard(event) {
    return '' +
      '<article class="pmd-w5-card is-old pmd-v35-ready pmd-r2-yc-detail-card is-event-card">' +
        '<div class="pmd-v35-card-head"><div class="pmd-v35-table-no">★</div></div>' +
        '<div class="pmd-w5-card-top"><span class="pmd-w5-pill warn">Germany event</span></div>' +
        '<h2>' + esc(event.title) + '</h2>' +
        '<div class="pmd-w5-meta"><div class="pmd-w5-box"><small>Date</small><b>' + esc(formatDate(event.date)) + '</b></div></div>' +
      '</article>';
  }

  function noteCard(key, text) {
    return '' +
      '<article class="pmd-w5-card is-old pmd-v35-ready pmd-r2-yc-detail-card is-note-card">' +
        '<div class="pmd-v35-card-head"><div class="pmd-v35-table-no">✎</div><button type="button" class="pmd-v35-edit-btn" data-r2-yc-edit-note="' + esc(key) + '" aria-label="Edit note">✎</button></div>' +
        '<div class="pmd-w5-card-top"><span class="pmd-w5-pill warn">Note</span></div>' +
        '<h2>' + esc(formatDate(key)) + '</h2>' +
        '<div class="pmd-w5-items"><small>Day note</small><div class="pmd-w5-item"><span>' + esc(text) + '</span></div></div>' +
      '</article>';
  }
  ...
  function renderSelected() {
    ...
    section.innerHTML = '' +
      '<div class="pmd-r2-yc-selected__head">' +
        '<div><small>Selected day</small><h2>' + esc(formatDate(selectedDate)) + '</h2><p>' + cards.length + ' item' + (cards.length === 1 ? '' : 's') + '</p></div>' +
        '<div class="pmd-r2-yc-selected__actions">' +
          '<button type="button" data-r2-yc-add-note="' + esc(selectedDate) + '">＋ Note</button>' +
          '<a href="' + esc((boot().createUrl || '/admin/reservations/create') + '?reserve_date=' + encodeURIComponent(selectedDate)) + '">＋ Reservation</a>' +
          '<button type="button" data-r2-yc-clear-selection>Show calendar only</button>' +
        '</div>' +
      '</div>' +
  ...
  function render() {
    ...
    months.innerHTML = view === 'month' ? monthMarkup(month, true) : monthNames.map(function (_, index) {
      return monthMarkup(index, false);
    }).join('');
    root.classList.toggle('is-month-mode', view === 'month');
    root.classList.toggle('is-year-mode', view === 'year');
    root.querySelectorAll('[data-r2-yc-view]').forEach(function (button) {
      button.classList.toggle('is-active', button.dataset.r2YcView === view);
    });
    root.querySelectorAll('[data-r2-yc-filter]').forEach(function (button) {
      button.classList.toggle('is-active', button.dataset.r2YcFilter === filter);
    });
    renderSelected();
  }
  ...
  function openNoteModal(key) {
    ...
    var input = root.querySelector('[data-r2-yc-note-input]');
    var title = root.querySelector('[data-r2-yc-modal-title]');
    var deleteButton = root.querySelector('[data-r2-yc-modal-delete]');
    var noteMap = notes();
    modal.dataset.date = key;
    title.textContent = 'Note for ' + formatDate(key);
    input.value = noteMap[key] || '';
    deleteButton.hidden = !noteMap[key];
    modal.hidden = false;
  ...
  function saveModalNote(remove) {
    ...
    var input = root && root.querySelector('[data-r2-yc-note-input]');
    if (!modal || !input) return;
    var key = modal.dataset.date;
    var noteMap = notes();
    var text = clean(input.value);
    if (remove || !text) delete noteMap[key];
    else noteMap[key] = text;
    saveNotes(noteMap);
    selectedDate = key;
    closeNoteModal();
    render();
  ...
  function bind(root) {
    root.addEventListener('click', function (event) {
      var prev = event.target.closest('[data-r2-yc-prev]');
      ...
      var modalClose = event.target.closest('[data-r2-yc-modal-close], [data-r2-yc-modal-cancel]');
      ...
      if (viewButton) {
        view = viewButton.dataset.r2YcView;
        render();
        return;
      }
      if (filterButton) {
        filter = filterButton.dataset.r2YcFilter;
        render();
        return;
      }
      if (noteButton) {
        openNoteModal(selectedDate || dateKey(new Date()));
        return;
      }
      if (addNote) {
        openNoteModal(addNote.dataset.r2YcAddNote);
        return;
      }
      if (editNote) {
        openNoteModal(editNote.dataset.r2YcEditNote);
        return;
      }
      if (clearSelection) {
        selectedDate = null;
  ...
  function audit() {
    ...
    return {
      ...
      headerOrder: actions ? [dateButton, toggle, create, notification].filter(Boolean).map(function (element) {
        return Array.prototype.indexOf.call(actions.children, element);
      }) : [],
  ...
  function init() {
    ...
    console.info('[PMD Reservations2 Calendar Toggle V1.9] Ready', audit());
  }
