/* PayMyDine Owner/Admin Dashboard Clean v20
   Role-split restaurant dashboards using the proven v19 real-data model. */
(function () {
  'use strict';

  var VERSION = 'owner-clean-v20-role-split-dashboards-20260626';
  var root = null;
  var lastData = null;
  ...
  function roleTabs(active, mode) {
    ...
    return '<div class="pmd-v20-role-tabs pmd-v20-role-tabs--' + esc(mode || 'native') + '" role="tablist" aria-label="Dashboard role views">' + ROLE_ORDER.map(function (role) {
  ...
  function applyRoleChrome(role) {
    var roles = ROLE_ORDER.map(function (r) { return 'pmd-v20-role-' + r; });
    ...
    document.body.classList.add('pmd-v20-role-' + role);
    document.documentElement.classList.add('pmd-v20-role-' + role);
    document.body.classList.toggle('pmd-v20-waiter-clean-mode', role === 'waiter');
    document.documentElement.classList.toggle('pmd-v20-waiter-clean-mode', role === 'waiter');
  }
  ...
  function syncNativeRoleTabs(active) {
    var id = 'pmd-v20-native-role-tabs';
  ...
  function dashboardHeader(data, role) {
    ...
    return '' +
      '<header class="pmd-v15-topbar pmd-v19-topbar pmd-v20-topbar">' +
  ...
  function renderRoleDashboard(data, role) {
    var s = data.sections || {};
    var staff = s.staff || {};
    var shellClass = 'pmd-v15-shell pmd-v19-shell pmd-v20-shell pmd-v20-shell--' + role;
    var header = dashboardHeader(data, role);
    var kpis = '<section class="pmd-v15-kpi-grid pmd-v19-kpi-grid pmd-v20-kpi-grid">' + kpiGrid(data, role) + '</section>';

    if (role === 'manager') {
      return '<div class="' + shellClass + '">' + header + kpis +
        '<section class="pmd-v19-reference-grid pmd-v20-reference-grid">' +
        ...
        '</section>' +
        '<section class="pmd-v20-two-grid">' +
        ...
        '</section>' +
        '<section class="pmd-v20-three-grid">' +
    ...
    if (role === 'kds') {
      return '<div class="' + shellClass + '">' + header + kpis +
        '<section class="pmd-v20-three-grid pmd-v20-role-main">' +
          section('Kitchen Performance', 'Queue, ready rows, delay risk and KDS health', kitchenPerformance(data), 'pmd-v15-card--kitchen pmd-v20-card-large') +
    ...
    if (role === 'reception') {
      return '<div class="' + shellClass + '">' + header + kpis +
        '<section class="pmd-v20-two-grid pmd-v20-role-main">' +
    ...
    if (role === 'waiter') {
      return '<div class="' + shellClass + '">' + header + kpis +
        '<section class="pmd-v20-two-grid pmd-v20-role-main">' +
          section('Live Restaurant Floor', 'Service tables, open checks and waiting payments', floorPlan(s.floor_plan), 'pmd-v15-card--floor pmd-v19-area-floor', '<div class="pmd-v15-card-actions"><button type="button" data-pmd-owner-refresh>Refresh</button><a href="/admin/orders/create">New Order</a></div>') +
          section('Service Alerts', 'Calls, open checks and ready kitchen rows', alerts(data), 'pmd-v15-card--alerts', '<a href="/admin/orders">View all</a>') +
        '</section>' +
        '<section class="pmd-v20-three-grid">' +
          section('Top Open Items', 'Items inside current open checks', barList(s.top_items, 'label', 'count', 5), 'pmd-v15-card--items') +
          section('Recent Live Orders', 'Service-relevant order activity', listRows(s.recent_orders, 'order'), 'pmd-v15-card--recent-orders') +
          section('Quick Actions', 'Waiter actions', quickActions(role), 'pmd-v15-card--actions') +
        '</section></div>';
    }
    ...
    return '<div class="' + shellClass + '">' + header + kpis +
      '<section class="pmd-v19-reference-grid pmd-v20-reference-grid">' +
        ...
        section('Live Timeline', 'Latest recorded orders and reservations', timeline(data), 'pmd-v15-card--timeline pmd-v19-area-timeline', '<a href="/admin/orders">View all</a>') +
      '</section>' +
      '<section class="pmd-v19-analytics-grid pmd-v20-analytics-grid">' +
        ...
        section('Upcoming Reservations', 'Reception/table-booking view', listRows(s.upcoming_reservations, 'reservation'), 'pmd-v15-card--reservations') +
      '</section>' +
      '<section class="pmd-v19-bottom-grid pmd-v20-bottom-grid">' +
        section('Top Servers', 'Staff activity from assignments when available; otherwise team overview', staffOverview(staff), 'pmd-v15-card--servers') +
        section('Kitchen Performance', 'Prep time, delay risk, queue and KDS health', kitchenPerformance(data), 'pmd-v15-card--kitchen') +
  ...
  function bind() {
    ...
    bindRoleTabs(root);
    bindRoleTabs(document.getElementById('pmd-v20-native-role-tabs'));
  }
