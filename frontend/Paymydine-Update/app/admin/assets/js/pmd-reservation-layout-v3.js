(function () {
  ...
  function renderFloor() {
     ...
     * but it must never render, clone or load another Floor.
     */
    if (
      window.PMDReservationFloorV4 ||
      window.PMDReservationFloorV41Authority ||
  ...
  function pmdV35FindLargestFinalFloor(frameDocument) {
    ...
    console.info(
      '[PMD Reservations V3.6.1] dashboardwaiter floor candidates',
  ...
  function pmdV33SanitizeWaiterFloor(clone) {
    ...
    /*
     * Remove source-only recovery notices and empty-state messages.
     */
    clone
      .querySelectorAll(
        'div,section,p,span'
      ...
      .forEach(function (node) {
        ...
        if (
          (
            /floor layout recovered/i.test(text) ||
            /no tables found in ti_tables/i.test(text)
          ) &&
          !node.querySelector(
            '.pmd-w5-table[data-table]'
          )
  ...
  function pmdV33MountWaiterFloor(
    ...
    console.info(
      '[PMD Reservations V3.6.3] Correctly scaled waiter floor mounted',
      {
  ...
  function pmdV33LoadExactFloor(
    ...
    /*
     * V3.6.3:
     * dashboardwaiter calculates percentage-based table coordinates
     * from its actual viewport dimensions.
     *
     * A 1x1 iframe caused values such as left: 18522px.
     * Render it off-screen at a real desktop size instead.
     */
    frame.style.cssText =
      'position:fixed;' +
      'width:1600px;' +
        ...
        var timer = setInterval(
          function () {
            ...
            try {
              ...
              /*
               * Do not clone a map whose coordinates were calculated
               * from an invalid tiny viewport.
               */
              var coordinateRows = map
                ? Array.prototype.map.call(
                    map.querySelectorAll(
                      '.pmd-w5-table[data-table]'
                    ),
              ...
              var coordinatesAreSane =
                coordinateRows.length >= 5 &&
                coordinateRows.every(function (row) {
                  return (
                    row.left >= -200 &&
                    row.left <= 2200 &&
                    row.top >= -200 &&
  ...
  async function install() {
    ...
    console.info(
      '[PMD] Reservation Layout V3.6.3 active',
      {
