...
(function () {
  ...
  function miniRows(rows, type) {
    ...
    return '<div class="pmd-owner-clean-list">' + rows.map(function (row) {
      ...
      var tableLabel = row.table_ref ? ('Table ' + row.table_ref) : 'No table';
      var orderInner = '' +
        '<div><strong>Order #' + esc(row.id || '—') + '</strong><small>' + esc(tableLabel + ' · ' + status) + '</small></div>' +
        '<span>' + esc(total) + '</span>';
      if (row.edit_url) {
