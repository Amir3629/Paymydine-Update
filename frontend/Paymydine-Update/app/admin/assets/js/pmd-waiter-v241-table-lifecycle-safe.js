(function () {
  ...
  function paymentState(row) {
    ...
    var due = Number(row.amount_due || 0);
    var openOrders = Number(row.open_order_count || 0);
    ...
    if (
      ...
      /*
       * A positive due value always wins over a stale settlement label.
       */
    ...
    /*
     * Do not infer PAID merely because amount_due is absent or zero.
     * Some tenant feeds omit amount_due while a newly-created order is open.
     */
    if (openOrders > 0) return 'none';
  ...
  function safeTick() {
    ...
    if (grid) {
      signature = Array.prototype.slice.call(
        grid.querySelectorAll(':scope > [data-v2-open-table]')
      ).map(function (card) {
        return String(card.getAttribute('data-v2-open-table') || '');
      }).join('|');
    }
    ...
    /*
     * Core may replace cards while preserving the same table IDs.
     * Always inspect current card nodes. decorateCard() has its own stable
     * signature guard, so existing cards are skipped and only fresh nodes
     * receive lifecycle markup.
     */
    decorateAllCards();
    buildRail();
    applyFilter();
