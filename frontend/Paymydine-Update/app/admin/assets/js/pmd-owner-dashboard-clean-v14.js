/* PayMyDine Owner/Admin Dashboard Clean v14
   Reference-style restaurant operations dashboard using the real v12 data model. */
(function () {
  'use strict';

  var VERSION = 'owner-clean-v14-polished-ops-20260626';
  var root = null;
  var lastData = null;
  ...
  function section(title, subtitle, inner, cls, action) {
    return '' +
      '<section class="pmd-v14-card ' + esc(cls || '') + '">' +
        '<header class="pmd-v14-card-head">' +
          '<div><h2>' + esc(title) + '</h2>' + (subtitle ? '<p>' + esc(subtitle) + '</p>' : '') + '</div>' +
          (action || '') +
        '</header>' +
        '<div class="pmd-v14-card-body">' + inner + '</div>' +
      '</section>';
  }
  ...
  function topKpi(data, key, icon, meta, tone) {
    ...
    return '' +
      '<article class="pmd-v14-kpi pmd-v14-kpi--' + esc(tone || 'neutral') + '">' +
        '<div class="pmd-v14-kpi-icon">' + esc(icon) + '</div>' +
        '<div class="pmd-v14-kpi-copy">' +
  ...
  function miniStat(label, value, note, tone) {
    return '<div class="pmd-v14-mini-stat pmd-v14-mini-stat--' + esc(tone || 'neutral') + '"><span>' + esc(label) + '</span><strong>' + esc(value) + '</strong><small>' + esc(note || '') + '</small></div>';
  }

  function empty(text) {
    return '<div class="pmd-v14-empty">' + esc(text || 'No live rows detected yet.') + '</div>';
  }
  ...
  function barList(items, labelKey, valueKey, limit) {
    ...
    return '<div class="pmd-v14-bars">' + items.map(function (x, idx) {
      var val = Number(x[valueKey || 'count'] || 0);
      var pct = Math.max(4, Math.min(100, Math.round(val / max * 100)));
      return '' +
        '<div class="pmd-v14-bar-row">' +
          '<div class="pmd-v14-bar-label"><span><b>' + esc(idx + 1) + '</b>' + esc(x[labelKey || 'label'] || 'Item') + '</span><strong>' + esc(val) + '</strong></div>' +
          '<div class="pmd-v14-bar-track"><i style="width:' + pct + '%"></i></div>' +
        '</div>';
    }).join('') + '</div>';
  }
  ...
  function sparklineFromOrders(rows) {
    ...
    return '' +
      '<svg class="pmd-v14-spark" viewBox="0 0 200 90" aria-hidden="true">' +
        '<defs><linearGradient id="pmdV14Spark" x1="0" y1="0" x2="1" y2="0"><stop offset="0" stop-color="#7c3aed"/><stop offset="1" stop-color="#22c55e"/></linearGradient></defs>' +
        '<polyline points="' + esc(points) + '" fill="none" stroke="url(#pmdV14Spark)" stroke-width="5" stroke-linecap="round" stroke-linejoin="round"/>' +
      '</svg>';
  }
  ...
  function donut(percent, center) {
    ...
    return '' +
      '<div class="pmd-v14-donut" style="--p:' + p + '">' +
  ...
  function floorPlan(fp) {
    ...
    var nodes = floorTables.map(function (t) {
      ...
      return '' +
        '<button type="button" data-pmd-table="' + esc(t.id) + '" class="pmd-v14-floor-node pmd-v14-floor-node--' + esc(t.status || 'free') + (isSelected ? ' is-selected' : '') + '" style="left:' + left + '%;top:' + top + '%;width:' + w + '%;height:' + h + '%">' +
    ...
    var side = selected ? tableSide(selected) : '<div class="pmd-v14-floor-empty"><strong>' + esc(floor.label || 'Main Floor') + '</strong><span>Click a table to inspect live status, capacity, reservations and quick links.</span></div>';
    return '' +
      '<div class="pmd-v14-floor-tools"><div class="pmd-v14-floor-tabs">' + floorTabs + '</div><div class="pmd-v14-floor-legend">' + legend.map(function (l) { return '<span class="pmd-v14-dot-' + esc(l[0]) + '"><i></i>' + esc(l[1]) + ' <b>' + esc(l[2]) + '</b></span>'; }).join('') + '</div></div>' +
      '<div class="pmd-v14-floor-wrap">' +
        '<div class="pmd-v14-floor-canvas"><div class="pmd-v14-floor-bg"></div>' + nodes + '</div>' +
        '<aside class="pmd-v14-floor-side">' + side + '</aside>' +
      '</div>';
  }
  ...
  function tableSide(t) {
    ...
    ].map(function (r) { return '<div class="pmd-v14-side-row"><span>' + esc(r[0]) + '</span><strong>' + esc(r[1]) + '</strong></div>'; }).join('');
    return '' +
      '<div class="pmd-v14-table-card pmd-v14-table-card--' + esc(t.status || 'free') + '">' +
  ...
  function alerts(data) {
    ...
    return '<div class="pmd-v14-alerts">' + items.map(function (a) {
      return '<a class="pmd-v14-alert pmd-v14-alert--' + esc(a.level || 'info') + '" href="' + esc(a.url || '#') + '"><i>⚠</i><div><strong>' + esc(a.title || 'Alert') + '</strong><small>' + esc(a.note || '') + '</small></div><b>' + esc(a.value || '') + '</b></a>';
    }).join('') + '</div>';
  }
  ...
  function timeline(data) {
    ...
    return '<div class="pmd-v14-timeline">' + rows.map(function (r) {
      return '<a href="' + esc(r.url) + '" class="pmd-v14-timeline-row pmd-v14-timeline-row--' + esc(r.tone) + '"><span>' + esc(r.time || 'Live') + '</span><i></i><div><strong>' + esc(r.title) + '</strong><small>' + esc(r.note) + '</small></div></a>';
    }).join('') + '</div>';
  }
  ...
  function listRows(rows, type) {
    ...
    return '<div class="pmd-v14-list">' + rows.slice(0, 6).map(function (row) {
      if (type === 'reservation') {
        return '<a href="/admin/reservations" class="pmd-v14-list-row"><div><strong>' + esc(row.guest_name || 'Reservation') + '</strong><small>' + esc([row.reserve_date, row.reserve_time].filter(Boolean).join(' ') + (row.table_ref ? ' · Table ' + row.table_ref : '')) + '</small></div><span>' + esc(row.guests || '—') + '</span></a>';
      }
      return '<a href="' + esc(row.edit_url || '/admin/orders') + '" class="pmd-v14-list-row"><div><strong>Order #' + esc(row.id || '—') + '</strong><small>' + esc((row.table_label || 'No table') + ' · ' + (row.status_label || row.status || '—') + ' · ' + (row.customer_label || 'Guest')) + '</small></div><span>' + esc(row.total_label || fmtMoney(row.total || 0)) + '</span></a>';
    }).join('') + '</div>';
  }
  ...
  function revenueByHour(data) {
    ...
    return '' +
      '<div class="pmd-v14-chart-head"><strong>' + esc(metric(data, 'revenue_today').value || '€0.00') + '</strong><small>today · recent live order trend below</small></div>' +
      sparklineFromOrders(orders) +
      '<div class="pmd-v14-chart-note">Connected to <b>ti_orders</b> / <b>ti_order_totals</b>. All-time context: ' + esc(totalRevenue ? totalRevenue.value : fmtMoney(todayRevenue)) + '</div>';
  }
  ...
  function paymentBreakdown(data) {
    ...
    return '<div class="pmd-v14-payment-grid">' + donut(pctPaid, fmtMoney(pending + paidToday)) + '<div class="pmd-v14-payment-lines">' +
      '<div><i class="pmd-v14-pay-open"></i><span>Open / pending</span><b>' + esc(fmtMoney(pending)) + '</b></div>' +
      '<div><i class="pmd-v14-pay-paid"></i><span>Paid today</span><b>' + esc(fmtMoney(paidToday)) + '</b></div>' +
      '<small>Payment transaction rows are empty right now, so this card uses open-check fallback value.</small>' +
      '</div></div>';
  }
  ...
  function smallFinance(data) {
    ...
    return '' +
      '<div class="pmd-v14-small-finance"><strong>' + esc(avgLabel) + '</strong><span>Average Check Value</span><small>' + esc(avgNote) + '</small></div>' +
      '<div class="pmd-v14-small-finance pmd-v14-small-finance--risk"><strong>' + esc(pending.value || '€0.00') + '</strong><span>Open Check Value</span><small>' + esc(pending.note || 'Open unpaid checks; transaction payments table is empty') + '</small></div>';
  }
  ...
  function kitchenPerformance(data) {
    ...
    return '' +
      '<div class="pmd-v14-kitchen-metrics">' +
      ...
      '</div>' +
      '<div class="pmd-v14-rush"><span>Rush Status</span><b>' + (Number(q.raw || 0) > 8 ? 'High' : (Number(q.raw || 0) > 0 ? 'Medium' : 'Calm')) + '</b></div>';
  }
  ...
  function quickActions() {
    ...
    return '<div class="pmd-v14-actions">' + items.map(function (i) { return '<a href="' + esc(i[1]) + '"><span>' + esc(i[2]) + '</span><b>' + esc(i[0]) + '</b></a>'; }).join('') + '</div>';
  }
  ...
  function dataProof(data) {
    ...
    return '' +
      '<details class="pmd-v14-proof"><summary>Audit / data proof — developer only</summary>' +
        '<div class="pmd-v14-proof-grid">' +
        ...
        '<h3>Connected tables</h3><div class="pmd-v14-source-chips">' + chips + '</div>' +
      '</details>';
  }
  ...
  function render(data) {
    ...
    root.innerHTML = '' +
      '<div class="pmd-v14-shell">' +
        '<header class="pmd-v14-topbar">' +
          '<div><h1>' + esc(greeting()) + ', ' + esc(findAdminName()) + '! 👋</h1><p>Here\'s what\'s happening at <b>Mimoza / PayMyDine</b></p></div>' +
          '<div class="pmd-v14-top-controls"><span>📅 ' + esc(nowLabel()) + '</span><span>🍽️ Live Restaurant Dashboard</span><a href="/admin/pmd-owner-dashboard-clean-v1-audit" target="_blank" rel="noopener">Data audit</a></div>' +
        '</header>' +

        '<section class="pmd-v14-kpi-grid">' +
        ...
        '<section class="pmd-v14-main-layout">' +
          section('Live Restaurant Floor', 'Live · table coordinates + open-check status', floorPlan(s.floor_plan), 'pmd-v14-card--floor', '<div class="pmd-v14-card-actions"><button type="button" data-pmd-owner-refresh>Refresh</button><a href="/admin/orders/create">New Order</a></div>') +
          section('AI Alerts', 'Operational alerts from connected PMD sources', alerts(data), 'pmd-v14-card--alerts', '<a href="/admin/orders">Open checks</a>') +
          section('Recent Activity', 'Latest real orders and reservations from the database', timeline(data), 'pmd-v14-card--timeline', '<a href="/admin/orders">View all</a>') +
        '</section>' +

        '<section class="pmd-v14-analytics-grid">' +
          section('Revenue by Order Trend', 'Connected to real orders; today may be zero', revenueByHour(data), 'pmd-v14-card--chart') +
          section('Open Check Value', 'Payment transactions are empty, so open-check fallback is shown', paymentBreakdown(data), 'pmd-v14-card--payment') +
          section('Average Check / Risk', 'Financial operations snapshot', smallFinance(data), 'pmd-v14-card--small') +
          section('Upcoming Reservations', 'Reception/table-booking view', listRows(s.upcoming_reservations, 'reservation'), 'pmd-v14-card--reservations') +
        '</section>' +

        '<section class="pmd-v14-bottom-grid">' +
          section('Team Overview', 'Staff role distribution; server sales will be added after assignments are reliable', staffOverview(staff), 'pmd-v14-card--servers') +
          section('Kitchen Performance', 'Kitchen queue + KDS station health', kitchenPerformance(data), 'pmd-v14-card--kitchen') +
          section('Top Open Items', 'Current open-check items from ti_order_menus', barList(s.top_items, 'label', 'count', 6), 'pmd-v14-card--items') +
          section('Quick Actions', 'Fast owner/manager actions', quickActions(), 'pmd-v14-card--actions') +
        '</section>' +

        '<section class="pmd-v14-bottom-grid pmd-v14-bottom-grid--wide">' +
          section('Recent Live Orders', 'Draft/test rows are excluded', listRows(s.recent_orders, 'order'), 'pmd-v14-card--orders') +
          section('Current Service Mix', 'Distribution from current open checks', serviceMix(data), 'pmd-v14-card--mix') +
        '</section>' +
  ...
  function staffOverview(staff) {
    ...
    return '<div class="pmd-v14-staff-total"><strong>' + esc(staff.total || 0) + '</strong><span>Total staff detected</span></div>' + barList(roles, 'label', 'count', 8);
  }
  ...
  function renderError(err) {
    root.innerHTML = '<div class="pmd-v14-error"><h1>Owner dashboard failed to load</h1><p>' + esc(err && err.message ? err.message : err || 'Unknown error') + '</p><button type="button" data-pmd-owner-refresh>Try again</button></div>';
