(function () {
  ...
  function pmdInstallFinalPOSLayout(posRoot) {
    ...
    if (!app || !workspace || !catalog || !categories || !cart) {
      console.error('[PMD] V2.9.2 required POS nodes missing', {
        app: !!app,
        workspace: !!workspace,
        catalog: !!catalog,
    ...
    var tableTitle = clean(titleNode) || 'TABLE';
    var areaTitle = clean(areaNode) || 'WAITER POS';
    ...
    console.info(
      '[PMD] Waiter POS V2.9.2 self-contained final layout active'
    );
  }
  ...
  async function openPos(tableId) {
    ...
    try {
      ...
      pmdInstallFinalPOSLayout(posRoot);
