(function () {
  ...
  function pmdV33MountWaiterFloor(
    ...
    console.info(
      '[PMD Reservations V3.6.1] Largest dashboardwaiter floor mounted',
      {
  ...
  async function install() {
    ...
    console.info(
      '[PMD] Reservation Layout V3.6.1 active',
      {
...
/* =========================================================
   PMD RESERVATION V3.6.2 — POST-MOUNT VISIBILITY RECOVERY
   ========================================================= */
(function () {
  'use strict';

  if (window.PMDReservationFloorRecoveryV362)
    return;

  window.PMDReservationFloorRecoveryV362 = true;
  ...
  function repair() {
    ...
    /*
     * Remove the source dashboard's temporary recovery message.
     * It is not relevant in the Reservations read-only clone.
     */
    Array.prototype.forEach.call(
      map.querySelectorAll('div,p,span,section'),
      function (node) {
        var text = clean(node.textContent);

        if (
          text.startsWith(
            'Floor layout recovered'
          ) &&
          !node.querySelector(
            '.pmd-w5-table[data-table]'
          )
    ...
    tables.forEach(function (table) {
      ...
      table.setAttribute(
        'data-pmd-reservation-visible',
        '1'
      );
    });

    console.info(
      '[PMD Reservations V3.6.2] Floor visibility repaired',
      {
        mapFound: true,
        tables: tables.length,
        tableNumbers: tables.map(function (table) {
          return table.getAttribute(
            'data-table'
          );
        }),
