...
/** Resolve menu item image URL to backend so images load from Laravel (e.g. /api/media/... → http://localhost:8000/api/media/...). */
export function getMenuImageUrl(image: string | null | undefined): string {
  if (!image) return '/placeholder.svg';
  if (image.startsWith('http://') || image.startsWith('https://')) return image;
  const base = typeof window !== 'undefined' ? envConfig.getApiBaseUrl() : (process.env.NEXT_PUBLIC_API_BASE_URL || 'http://localhost:8000');
  if (image.startsWith('/api') || image.startsWith('/images')) return base.replace(/\/$/, '') + image;
  return image;
}
...
export class ApiClient {
  ...
  async getMenu(): Promise<MenuResponse> {
    try {
      ...
      if (endpoint.startsWith('http')) {
        ...
        // Relative URL: use backend base URL so menu comes from same source as admin
        const base = this.envConfig.getApiBaseUrl();
        url = new URL(endpoint, base || (typeof window !== 'undefined' ? window.location.origin : 'http://localhost'));
      }
  ...
  async getTableMenu(tableId: string, locationId?: number): Promise<MenuResponse> {
    try {
      ...
      console.error('Table menu fetch failed:', error);
      return fallbackMenuData;
    }
  }
