...
import { categories, type MenuItem, getMenuData, getCategories } from "@/lib/data"
...
import { ApiClient, type PaymentMethod, getMenuImageUrl } from "@/lib/api-client"
                    ...
                    {items.slice(toolbarState === "preview" ? -1 : 0).map((item: CartItem) => (
                      ...
                      >
                        <div className="flex items-center space-x-3">
                          ...
                          >
                <OptimizedImage
                              src={getMenuImageUrl(item.item.image) || "/placeholder.svg"}
                              alt={item.item.name}
                              fill
...
function LoadingSpinner() {
  return (
    ...
    >
      <div className="w-8 h-8 border-4 border-paydine-champagne border-t-transparent rounded-full animate-spin" />
    </div>
  )
}
...
function MenuContent() {
  ...
  const { taxSettings, loadTaxSettings } = useCmsStore()
  ...
  const filteredItems = useMemo(() => {
    // Only use API data – no fallback to store or static list
    const availableItems = apiMenuItems;
    ...
    return itemsWithAdjustedPrices.filter((item) => (item.category_name || item.category) === currentCategory);
  }, [apiMenuItems, selectedCategory, taxSettings.enabled, taxSettings.percentage, taxSettings.menuPrice]);
