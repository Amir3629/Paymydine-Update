...
type PaymentMethod = {
  ...
  name: string
  provider_code: "stripe" | "paypal" | "worldline" | "sumup" | null
  enabled: boolean
  priority: number
}

type PaymentProvider = {
  code: "stripe" | "paypal" | "worldline" | "sumup"
  name: string
  enabled: boolean
}

const METHOD_PROVIDER_OPTIONS: Record<PaymentMethod["code"], Array<{ code: PaymentMethod["provider_code"]; label: string }>> = {
  card: [
    { code: "stripe", label: "Stripe" },
    { code: "sumup", label: "SumUp" },
    { code: "worldline", label: "Worldline" },
  ],
  apple_pay: [{ code: "stripe", label: "Stripe" }],
  google_pay: [{ code: "stripe", label: "Stripe" }],
  paypal: [{ code: "paypal", label: "PayPal" }],
  cod: [{ code: null, label: "No Provider" }],
}
      ...
      <div className="bg-white p-6 rounded-xl shadow space-y-4">
        {isLoading ? (
          ...
          methods
            ...
            .map((method) => {
              const options = METHOD_PROVIDER_OPTIONS[method.code] || []
