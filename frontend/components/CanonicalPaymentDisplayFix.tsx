...
function fixPaidSummary(root: HTMLElement, latest: PaymentBreakdown | null): void {
  ...
  const inferredTipAmount = Math.max(
    0,
    Math.round((
      Number(breakdown.paidAmount || 0)
      - Number(breakdown.baseAmount || 0)
      + Number(breakdown.couponAmount || 0)
    ) * 100) / 100
  )
  ...
  const normalizedBreakdown = {
    ...
    tipAmount: Number(breakdown.tipAmount || 0) > 0
      ? Number(breakdown.tipAmount)
      : inferredTipAmount,
  }
...
// PMD_KAZEN_CASH_CONFIRM_DEDUP_V5
//
// Two payment layers currently expose the same Cash confirmation action.
// Keep only the final canonical Cash action.
function pmdDeduplicateCashConfirmButtons(): void {
  document
    .querySelectorAll<HTMLElement>(
      '.kzco-overlay[data-kzco-root="1"]'
    )
    .forEach((root) => {
      ...
      const buttons =
        ...
        .filter((button) =>
          String(button.textContent || '')
            .replace(/\s+/g, ' ')
            .trim()
            .toLowerCase() === 'confirm cash payment'
        )
      ...
      buttons.slice(0, -1).forEach((button) => {
        ...
        button.tabIndex = -1
        button.dataset.pmdDuplicateCashConfirm = '1'
      })
    })
}
...
export default function CanonicalPaymentDisplayFix() {
  useEffect(() => {
    ...
    const apply = () => {
      ...
      pmdDeduplicateCashConfirmButtons()
      // PMD_KAZEN_REACT_COUNT_AUTHORITY_PAYMENT_TIP_V4
      // KazenStandalonePage owns the stable +/quantity child tree.
      // Do not rewrite menu add-button innerHTML after React renders it.
