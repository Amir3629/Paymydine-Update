...
type VelvetTerracottaBridgeThemeProps = {
  src: string
  sourceItems: any[]
  cartItems: any[]
  totalItems: number
...
function pmdVelvetNormalizeMenuLayoutFromAdmin(value: unknown): "accordion" | "tabs" | "" {
...
function pmdVelvetReadMenuLayoutFromPayload(payload: any): "accordion" | "tabs" | "" {
  const keys = [
    "velvet_menu_layout",
    "velvetMenuLayout",
    "menu_layout",
  ...
  for (const source of sources) {
    ...
    for (const key of keys) {
      const normalized = pmdVelvetNormalizeMenuLayoutFromAdmin(source?.[key])
...
export function VelvetTerracottaBridgeTheme({
  src,
  sourceItems,
  cartItems,
  totalItems,
  ...
  tableOrderCount = 0,
  children,
}: VelvetTerracottaBridgeThemeProps) {

  // PMD_VELVET_FETCH_ADMIN_LOGO_LIKE_HOMEPAGE_20260611
  const pmdVelvetBridgeLogoRef = useRef("")
  ...
  const [pmdVelvetResolvedMenuLayout, setPmdVelvetResolvedMenuLayout] = useState<"accordion" | "tabs">(
    pmdVelvetNormalizeMenuLayoutFromAdmin(menuLayout) || "accordion"
  )

  useEffect(() => {
    const fromProp = pmdVelvetNormalizeMenuLayoutFromAdmin(menuLayout)
    if (fromProp) setPmdVelvetResolvedMenuLayout(fromProp)
  }, [menuLayout])
  ...
  useEffect(() => {
    ...
    const readMenuLayout = async () => {
      // PMD_VELVET_BRIDGE_SIMPLE_THEME_ONLY_V5
      const endpoints = [
      ...
      for (const endpoint of endpoints) {
        try {
          ...
          const nextLayout = pmdVelvetReadMenuLayoutFromPayload(payload)
          console.info("PMD_VELVET_MENU_LAYOUT_VALUE", nextLayout || null, endpoint)

          console.info("PMD_VELVET_MENU_LAYOUT_FROM_API", {
            endpoint,
            nextLayout: nextLayout || null,
            hasVelvetMenuLayout: Boolean(
              payload?.velvet_menu_layout ||
              payload?.data?.velvet_menu_layout ||
              payload?.settings?.velvet_menu_layout ||
              payload?.theme?.settings?.velvet_menu_layout
            ),
          })

          if (nextLayout && !cancelled) {
            setPmdVelvetResolvedMenuLayout(nextLayout)
            ;(window as any).__PMD_VELVET_MENU_LAYOUT = nextLayout
            window.dispatchEvent(new Event("PMD_VELVET_FORCE_SYNC"))
            return
          }
        } catch {
          // try next endpoint
    ...
    const loadAdminLogo = async () => {
      ...
      for (const endpoint of endpoints) {
        try {
          ...
          if (logo) {
            pmdVelvetBridgeLogoRef.current = logo
  ...
  useEffect(() => {
    ...
    const sendSync = () => {
      const frame = document.getElementById("pmd-velvet-japanese-frame") as HTMLIFrameElement | null
      if (!frame?.contentWindow) return

      const lastItem = lastInteractedItem?.item || cartItems?.[cartItems.length - 1]?.item || null

      // PMD_VELVET_ADMIN_LOGO_ONLY_BRIDGE_20260611
      ...
      const bridgeLogoUrl = String(
        ...
        pmdVelvetBridgeLogoRef.current ||
      ...
      frame.contentWindow.postMessage({
        ...
        menuLayout: pmdVelvetResolvedMenuLayout,
        velvet_menu_layout: pmdVelvetResolvedMenuLayout,
        categories,
    ...
    const handleMessage = (event: MessageEvent) => {
      ...
      if (type === "PMD_VELVET_READY") {
        sendSync()
        return
      }

      if (type === "PMD_VELVET_ADD_ITEM") {
      ...
      if (type === "PMD_VELVET_OPEN_ITEM") {
        const found = findItem((msg as any).itemId)
        if (found) onOpenItem(found)
        return
      }

      if (type === "PMD_VELVET_CHECKOUT") {
        onCheckout()
        return
      }

      if (type === "PMD_VELVET_CALL_WAITER") {
        void onCallWaiter()
        return
      }

      if (type === "PMD_VELVET_ADD_NOTE") {
        void onOpenNote(String((msg as any).note || ""))
        return
      }

      if (type === "PMD_VELVET_TABLE_ORDER" || type === "pmd:table-order") {
        void onTableOrder?.()
        return
      }

      if (type === "PMD_VELVET_GO_VALET") {
    ...
    window.addEventListener("PMD_VELVET_FORCE_SYNC", handleForceSync)

    return () => {
      timers.forEach((timer) => window.clearTimeout(timer))
      window.removeEventListener("message", handleMessage)
      window.removeEventListener("PMD_VELVET_FORCE_SYNC", handleForceSync)
    }
  }, [
    sourceItems,
    cartItems,
