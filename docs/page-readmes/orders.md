...
Cashier/order operations, order inspection, table operations, status changes, notes, invoices and item adjustments.

## Route and authority chain

- **Visible URL:** `/admin/orders`
- **Internal route/controller target:** `cashierlab for clean listing; canonical Orders controller for CRUD/edit/invoice paths`
- **Primary controller/composer:** `app/admin/controllers/Orders.php`
- **Server gate:** Admin.Orders; assignment/deletion additionally use Admin.AssignOrders/Admin.DeleteOrders.
- **Canonical sources:** Orders_model, order_menus, Statuses_model/history, Payments_model, tables/locationables and Order_notes_model.
- **Public/AJAX handlers:** list/form handlers; update status; move/merge/unmerge table; update/remove items; add note; invoice/customerInvoice; send invoice email; admin route JSON helpers.
- **Write authority:** Orders controller/framework actions and POS/order services are writers. Multi-row table and item changes use DB transactions where shown. Payment settlement remains provider/payment transaction authority—not browser redirects or status labels.
- **Frontend authority:** orders Blade/config plus shared admin list/form assets; Cashierlab is the clean listing display host.
...
- `app/admin/requests/Order.php`
- `app/admin/views/orders`
