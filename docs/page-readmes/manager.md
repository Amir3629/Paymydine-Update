...
Manager operations workspace with four KPIs, reservation schedule, online staff, owner analytics, and shared Floor.

## Route and authority chain

- **Visible URL:** `/admin/managerdashboard`
- **Internal route/controller target:** `managerlab`
- **Primary controller/composer:** `app/admin/controllers/Managerlab.php`
- **Server gate:** Admin.Dashboard; role landing selects this page for manager groups.
- **Canonical sources:** PmdCleanWorkspaceSharedV1, PmdRoleDashboardDataV1, PmdReservationsLabScheduleV1 and PmdAdminPresenceService.
- **Public/AJAX handlers:** index; `?pmd_analytics=1`; inherited onSaveFloorViewPreference.
- **Write authority:** Only inherited per-user Floor preference writes here; operational cards are read-only links to owning pages.
- **Frontend authority:** PmdCleanWorkspaceControllerV1 shared assets, role-dashboard/analytics, manager-online-staff, and shared Floor assets.
...
- `app/admin/Services/PmdCleanWorkspaceSharedV1.php`
