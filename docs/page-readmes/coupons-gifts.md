...
Create, inspect, enable/disable and delete coupons/gift-card style promotions.

## Route and authority chain

- **Visible URL:** `/admin/coupons`
- **Internal route/controller target:** `coupons`
- **Primary controller/composer:** `app/admin/controllers/Coupons.php`
- **Server gate:** Admin.Coupons (controller access assertion); authenticated admin.
- **Canonical sources:** Coupons_model, coupon histories/uses and gift-card models where the selected card type delegates there.
- **Public/AJAX handlers:** index/create/edit/preview; onPmdCouponSaveV1; onPmdCouponToggleStatusV1; onPmdCouponDeleteV1.
- **Write authority:** JSON handlers validate payloads, scope records, and transactionally save/delete. Public redemption/validation is separately owned by main API coupon routes.
- **Frontend authority:** coupons views plus controller-loaded PMD coupon workspace assets.
...
- `app/admin/models/GiftCardDesign_model.php`
- `app/admin/models/GiftCardTransaction_model.php`
- `app/main/routes/api-v1-coupon.php`
- `app/main/routes/coupon.php`
