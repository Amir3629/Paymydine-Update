...
Inventory/entry point for POS, payment terminals, KDS, cash drawers, biometric devices and integrations.

## Route and authority chain

- **Visible URL:** `/admin/settings/devices`
- **Internal route/controller target:** `pmddevices`
- **Primary controller/composer:** `app/admin/controllers/Pmddevices.php`
- **Server gate:** Site.Settings.
- **Canonical sources:** Pos_devices/configs, Terminal_devices, Kds_stations, Cash_drawers and FingerDevices models.
- **Public/AJAX handlers:** index; pos/terminals/kds/drawers/biometric/integrations redirect/build detail routes; terminal-payments routes own provider operations.
- **Write authority:** Overview is mostly composer. Device-specific controllers/APIs own pairing, sync, test, removal and configuration. Provider credentials never belong in rendered inventory.
- **Frontend authority:** pmddevices views plus scoped device settings assets; provider setup UI may delegate to terminal-specific controllers.
