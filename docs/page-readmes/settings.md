...
Owner settings hub linking restaurant, customer menu, menu/checkout, customers, team, devices, finance, brand and advanced settings.

## Route and authority chain

- **Visible URL:** `/admin/settings`
- **Internal route/controller target:** `pmdsettings`
- **Primary controller/composer:** `app/admin/controllers/Pmdsettings.php`
- **Server gate:** Site.Settings.
- **Canonical sources:** tenant settings plus location context; index composes navigation and health summaries.
- **Public/AJAX handlers:** index only for hub; subsection handlers are documented on their owning pages.
- **Write authority:** The hub is display/navigation authority. It must not become write authority for payment, device, profile or theme subsystems.
- **Frontend authority:** pmd-owner-settings-v1 and settings-center/polish assets; clean URL translation is server-owned, not JS-owned.
...
- `app/admin/assets/js/pmd-settings-center-v1.js`
- `app/admin/assets/js/pmd-settings-polish-r6.js`
