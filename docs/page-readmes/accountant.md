...
Read-only accounting overview with four finance KPIs and owner analytics; it intentionally has no Floor.

## Route and authority chain

- **Visible URL:** `/admin/accountantdashboard`
- **Internal route/controller target:** `accountantlab`
- **Primary controller/composer:** `app/admin/controllers/Accountantlab.php`
- **Server gate:** Admin.Dashboard; role landing selects this page for accountant groups.
- **Canonical sources:** PmdCleanWorkspaceSharedV1 and PmdRoleDashboardDataV1 over canonical orders/payment/report data.
- **Public/AJAX handlers:** index; `?pmd_analytics=1&period=...`.
- **Write authority:** No business writer. KPI selection may use shared user preference behavior; payment configuration belongs to Payments & Finance.
- **Frontend authority:** shared clean-workspace shell plus dashboard analytics and role-dashboard assets.
