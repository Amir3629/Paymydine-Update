...
Configure payment methods/providers and regional finance/fiscal settings for the current location market.

## Route and authority chain

- **Visible URL:** `/admin/settings/finance`
- **Internal route/controller target:** `pmdfinance`
- **Primary controller/composer:** `app/admin/controllers/Pmdfinance.php`
- **Server gate:** Site.Settings.
- **Canonical sources:** Payments_model/payment_methods-or-payments compatibility storage, LocationPlatformContext, CountryPlatformProfileRegistry, terminal inventory and fiscal settings.
- **Public/AJAX handlers:** index; onSaveFinance; provider/terminal routes and webhooks are separate public authorities.
- **Write authority:** Validated finance save uses a transaction and direct setting persistence compatibility. METHOD is customer choice, PROVIDER processes it, TERMINAL is device, SURFACE is UI. Settlement truth is the durable payment/transaction reconciliation path; never redirect, webhook, or provider status alone.
- **Frontend authority:** pmdfinance view, owner-settings and inline-detail scoped assets.
