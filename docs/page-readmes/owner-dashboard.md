...
Owner operational overview: configurable KPIs, analytics, reviews/events and the shared live Floor.

## Route and authority chain

- **Visible URL:** `/admin/ownerdashboard`
- **Internal route/controller target:** `dashboardlab`
- **Primary controller/composer:** `app/admin/controllers/Dashboardlab.php`
- **Server gate:** Admin.Dashboard.
- **Canonical sources:** PmdRoleDashboardDataV1; Dashboard2-derived server adapters; Reservations_model; orders, payments, reviews and settings tables.
- **Public/AJAX handlers:** GET/POST index (POST delegates framework AJAX); `?pmd_analytics=1&period=...`; `onSaveFloorViewPreference`.
- **Write authority:** Only the floor-view preference handler writes user display preferences; dashboard facts and analytics are read-only.
- **Frontend authority:** pmd-dashboard-lab-*.css/js, shared Floor CSS/JS and the dashboard live-refresh client.
...
- `app/admin/Services/PmdRoleDashboardDataV1.php`
