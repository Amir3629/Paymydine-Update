...
## Official Documentation Cross-Check (VR Payment + Worldline)

Checked on: 2026-04-20

### A) VR Payment official integration docs (gateway.vr-payment.de)

#### Hosted checkout / payment page
- VR Payment “Payment Page” flow is documented as:
  1) create transaction object,
  2) build payment page URL,
  3) redirect customer,
  4) process webhook and update merchant order state.
- This confirms the intended model is hosted page + server-side transaction orchestration.

#### Server-to-server API initiation
- VR web-service API examples show server-side transaction creation and build payment page URL calls.
- API reference indicates v1 is deprecated and v2.0 is latest.

#### Authentication scheme
- VR web-service authentication is documented with MAC headers:
  - `x-mac-version`
  - `x-mac-userid`
  - `x-mac-timestamp`
  - `x-mac-value` (HMAC-SHA512 over canonical request parts).
- User ID and authentication key come from application-user credentials.

#### Webhook signature verification
- VR webhook docs describe signed webhook payload headers (`x-signature`) and SDK-based verification.
- Payload-signature mode includes `state` and identity-driven signature validation flow.

#### Transaction status lookup
- VR webhook docs instruct reading the changed entity using the API `read` endpoint with `entityId` after notification.

#### Sandbox vs production
- VR docs surface API versioning (v1 deprecated / v2 latest).
- Processor docs (e.g., PayPal processor configuration in VR platform docs) explicitly support sandbox vs live operation modes.

#### Method relevance (VR side)
- Apple Pay / Google Pay are visible in VR API connector capability enums (device token types and connector feature metadata).
- PayPal exists as a distinct offsite connector.
- A direct “Wero” entry is not discoverable in VR public API docs sampled during this audit.

### B) Worldline official integration docs (docs.connect.worldline-solutions.com)

#### Hosted checkout / hosted payment page
- Worldline docs for iDEAL|Wero, PayPal, Apple Pay, Google Pay all document MyCheckout hosted pages as first-class flow.
- `POST /v1/{merchantId}/hostedcheckouts` + redirect via `partialRedirectUrl` are documented repeatedly.

#### Server-to-server API initiation
- Worldline docs also provide direct API mode (`POST /v1/{merchantId}/payments`) alongside hosted checkout.

#### Authentication scheme
- SDK docs define credential pair `apiKeyId` + `secretApiKey` and authorization type `v1HMAC`.

#### Webhook signature verification
- Webhook docs define `X-GCS-Signature` (HMAC-SHA256), `X-GCS-KeyId`, and explicit verification steps with webhook key material.

#### Transaction status lookup
- Hosted flow status is retrieved via `GET /v1/{merchantId}/hostedcheckouts/{hostedCheckoutId}`.
- After hostedCheckoutId validity window, docs recommend storing and using `payment.id` with `GET /v1/{merchantId}/payments/{paymentId}`.

#### Sandbox vs production
- Documentation portal explicitly separates pre-production/sandbox and production centers.
- Hosted checkout examples use preprod checkout domain samples.

#### Method relevance (Worldline side)
- iDEAL|Wero documented as a single payment product line on GlobalCollect.
- Apple Pay and Google Pay have hosted-page and own-checkout integration variants.
- PayPal has hosted checkout and direct API variants with recurring specifics.

---

## Mapping Official Requirements to Current Platform Fields

### Existing platform fields under review
- `API Base URL`
- `Space ID`
- `User ID`
- `Auth Key`
...
### Field-by-field mapping verdict

1. **API Base URL**
   - **Maps to official?** Partially.
   - **VR docs reality:** official VR web-service docs point to gateway API + signed MAC requests; base URL is relevant, but expected API path/auth pattern differs from current implementation (`/checkout/sessions`, `/checkout/status` with custom headers).
   - **Worldline reality:** equivalent concept exists as endpoint host (preprod/prod), but with `apiKeyId/secretApiKey` auth.
   - **Verdict:** **partially aligned**; name is generic enough, semantics likely provider-specific.

2. **Space ID**
   - **Maps to official?** Yes (VR uses `spaceId` / `space_id` concept extensively).
   - **Scope note:** must be tenant-scoped (merchant/space specific).
   - **Verdict:** **aligned**.

3. **User ID**
   - **Maps to official?** Yes for VR app-user auth (`x-mac-userid`).
   - **Scope note:** tenant-scoped application user.
   - **Verdict:** **aligned**.

4. **Auth Key**
   - **Maps to official?** Partially / potentially mis-specified.
   - **VR docs reality:** auth key is used to compute MAC (`x-mac-value`) with canonical request and timestamp, not as plain `x-auth-key` bearer-like header in official examples.
   - **Verdict:** **name acceptable, implementation semantics likely divergent**.

5. **Webhook Signing Key**
   - **Maps to official?** Potentially **misnamed / mismatched** for VR official webhook model.
   - **VR docs reality:** webhook payload signing is documented with `x-signature` and identity/public-key style verification (SDK-driven), not a simple shared secret HMAC key.
   - **Worldline reality:** webhook signing does use secret key lookup by key id (`X-GCS-KeyId`), but that belongs to Worldline Connect webhook key model.
   - **Verdict:** **likely misnamed or provider-model mixed**.

6. **Preferred Integration**
   - **Maps to official?** Yes conceptually (`payment_page` / hosted flow selection).
   - **Scope note:** this is integration-mode control and is provider-global, but method availability still method-specific.
   - **Verdict:** **aligned (conceptually)**.

---

## Gaps and Field Hygiene Findings

### Missing fields (relative to official patterns)
1. **Explicit auth algorithm/version field** (e.g., MAC version/HMAC mode) for VR official MAC contract.
2. **Webhook identity / key-id material** (if VR official webhook signing is identity/key-based rather than single secret).
3. **Environment / mode discriminator coupled to endpoint set** (test/live host and behavior) as first-class normalized field in VR runtime config.
4. **Method-level enablement fields** for wallets/payment products when provider supports mixed capability per method.

### Obsolete or questionable fields
1. `preferred_integration_mode` is currently hard-fixed to one value (`payment_page`) and rejects others.
   - If no alternate integration is planned, this may be configuration noise rather than a meaningful operator choice.

### Misnamed fields
1. `auth_key` (current usage suggests static header secret) may need to reflect official VR MAC credential semantics (e.g., `user_secret` / `mac_secret`).
2. `webhook_signing_key` may be inaccurate if official VR webhook verification relies on webhook identity/public-key metadata rather than shared HMAC secret.

### Fields that should be tenant-scoped
- `api_base_url` (or endpoint environment pair)
- `space_id`
- `user_id`
- `auth_key`
- webhook verification material (`webhook_signing_key` and/or key-id/identity bindings)

### Fields that are method-specific rather than provider-global
- Wero-specific boarding/availability controls
- Apple Pay merchant/display/domain setup parameters
- Google Pay merchant display/configuration-center data
- PayPal recurring/billing agreement flags

These should not all live only in a single provider-global blob if methods are selectively enabled; they need method-level schema and validation to avoid false “provider ready” states.

---

## Implementation Impact for This Codebase

1. Keep current fields for backward compatibility, but introduce a **provider-versioned credential schema** for VR (`auth_scheme`, `auth_secret_type`, key-id support).
2. Split VR config into:
   - provider-global base transport/auth settings,
   - method-level capability + required metadata (wero/apple/google/paypal).
3. Align webhook verification implementation with officially documented signature mechanism for the selected VR API family.
4. Add explicit `environment` (sandbox/live) with validated endpoint presets to reduce operator misconfiguration.
