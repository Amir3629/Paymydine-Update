...
export default function CheckoutPage() {
  ...
  useEffect(() => {
    const api = new ApiClient();
    console.log('🔍 Checkout: Fetching tax settings...');
    api.getSettings()
      .then(response => {
        console.log('📡 Checkout: Tax settings API response:', response);
        ...
        if (isSuccess && data) {
          ...
          console.log('✅ Checkout: Setting tax settings:', newTaxSettings);
          setTaxSettings(newTaxSettings);
        } else {
          console.log('❌ Checkout: Tax settings response not successful:', response);
        }
      })
      .catch(error => {
        console.error('❌ Checkout: Failed to fetch tax settings:', error);
      });
  }, [])
  ...
  // Function to calculate price with tax included
  const getPriceWithTax = (basePrice: number) => {
    if (taxMode && taxMenuPrice === 0) {
      // Menu prices already include tax - add tax to base price
  ...
  const subtotal = useMemo(
    () => itemsToPay.reduce((acc, inst) => acc + getPriceWithTax(inst.price), 0),
    [itemsToPay, taxMode, taxMenuPrice, taxPercentage],
  )
  ...
  console.log('🧮 Checkout: Tax calculation debug:', {
    taxSettings,
    ...
    applyInCheckout: taxMenuPrice === 1,
    pricesIncludeTax: taxMenuPrice === 0,
    // Clear explanation
    explanation: taxMenuPrice === 1 ? "Apply tax in checkout bill - SHOULD show tax" : "Menu prices include tax - SHOULD NOT show tax"
  });
        ...
        <div className="space-y-8">
          <div className="surface rounded-3xl p-6 shadow-lg">
            ...
            {allItems.map((cartItem, index) => {
              ...
              if (hasOptions) {
                // For items with options, show each individually
                return (
                  <div key={`${cartItem.item.id}-${index}-${JSON.stringify(cartItem.selectedOptions || {})}`} className="flex justify-between items-center py-2 divider last:border-0">
                    <div className="flex items-center space-x-4">
                      <span>1x</span>
                      <span>{itemName}</span>
                      {cartItem.selectedOptions && Object.keys(cartItem.selectedOptions).length > 0 && (
                        <span className="text-sm text-gray-600">
                          - {Object.values(cartItem.selectedOptions).map(optionId => {
                            const option = cartItem.item.options?.find(opt => 
                              opt.values.some(val => val.id.toString() === optionId)
                            )
                    ...
                    <span className="font-semibold">${cartItem.item.price.toFixed(2)}</span>
                  </div>
                )
              } else {
                // For items without options, show grouped
                return (
                  <div key={`${cartItem.item.id}-${index}-no-options`} className="flex justify-between items-center py-2 divider last:border-0">
                    <div className="flex items-center space-x-4">
          ...
          {/* Coupon Input Field */}
          <div className="space-y-2 p-3 surface-sub rounded-xl">
            {!appliedCoupon ? (
