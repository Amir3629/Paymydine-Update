  ...
  const getTotalPrice = () => {
    ...
    Object.values(selectedOptions).forEach(optionId => {
      ...
      itemOptions.forEach(option => {
        ...
        if (optionValue) {
          // Apply tax to option prices if "Menu prices already include tax" is selected
          const optionPriceWithTax = calculatePriceWithTax(optionValue.price)
          total += optionPriceWithTax * cartItem.quantity
        }
      })
    })
    return total
                        ...
                        <label key={value.id} className="flex items-center gap-2 text-xs cursor-pointer">
                          ...
                          {value.price > 0 && (
                            <span className="text-paydine-champagne font-medium">
                              +{formatCurrency(calculatePriceWithTax(value.price))}
                            </span>
                          )}
                        </label>
                      ))}
...
function PaymentModal({ isOpen, onClose, items: allItems, tableInfo }: PaymentModalProps) {
      ...
      .then(response => {
        ...
        if (isSuccess && data) {
          setTaxSettings({
            ...
            tax_menu_price: data.tax_menu_price || 0,
            tax_delivery_charge: data.tax_delivery_charge || 0,
          });
        }
      })
      .catch(error => {
  ...
  // Tax calculation based on tax_menu_price setting:
  // tax_menu_price: 0 = "Apply tax on top of my menu price" (add tax to subtotal)
  // tax_menu_price: 1 = "Menu prices already include tax" (no additional tax)
  const taxMode = Number(taxSettings.tax_mode) || 0
  const taxPercentage = Number(taxSettings.tax_percentage) || 0
  const taxMenuPrice = Number(taxSettings.tax_menu_price) || 0

  // Helper function to calculate price with tax
  const getPriceWithTax = (price: number) => {
    return taxMode && taxMenuPrice === 0 ? price + (price * (taxPercentage / 100)) : price;
  }
  ...
  const validateCoupon = async () => {
    ...
    try {
      const response = await apiClient.validateCoupon(couponCode, subtotal)
      ...
      if (response.success && response.data) {
        ...
        setAppliedCoupon({
          ...
          type: coupon.type === 'F' ? 'fixed' : 'percentage',
          id: coupon.id
        })
        setCouponCode("")
        setCouponError("")
        toast({
          ...
          description: `${coupon.code} - ${coupon.type === 'percentage' ? `${coupon.discount}%` : `$${coupon.discount}`} discount applied`
        })
      } else {
        setCouponError((response as any).message || "Invalid coupon code")
  ...
  const subtotal = useMemo(
    () => itemsToPay.reduce((acc, inst) => {
      ...
      // Add option prices (with tax if applicable)
        ...
        if (menuItem && menuItem.item.options) {
          Object.values(itemOptions).forEach(optionId => {
            menuItem.item.options!.forEach(option => {
              ...
              if (optionValue) {
                // Apply tax to option prices if "Menu prices already include tax" is selected
                const optionPriceWithTax = getPriceWithTax(optionValue.price)
                itemTotal += optionPriceWithTax * (inst.quantity || 1)
              }
            })
          })
        }
    ...
    [itemsToPay, selectedOptions, allItems, taxMode, taxMenuPrice, taxPercentage],
  )

  const taxAmount = (taxMode && taxMenuPrice === 1) 
  ...
  const couponDiscount = appliedCoupon ? 
    (appliedCoupon.type === 'percentage' 
      ? (subtotal * (parseFloat(String(appliedCoupon.discount)) / 100))
      : parseFloat(String(appliedCoupon.discount))
    ) : 0
  
  const finalTotal = subtotal + taxAmount + tipAmount - couponDiscount
  ...
  console.log('🧮 Tax calculation debug:', {
    ...
    tax_menu_price: taxSettings.tax_menu_price,
    parsed_taxMode: taxMode,
    parsed_taxPercentage: taxPercentage,
    parsed_taxMenuPrice: taxMenuPrice,
    subtotal,
    taxAmount,
    finalTotal,
    // Additional debug info
    shouldShowTax: taxMode && taxMenuPrice === 1,
    taxModeEnabled: taxMode === 1,
    applyInCheckout: taxMenuPrice === 0,
    pricesIncludeTax: taxMenuPrice === 1,
    // Clear explanation
    explanation: taxMenuPrice === 1 ? "Menu prices include tax - SHOULD show tax" : "Apply tax in checkout bill - SHOULD NOT show tax"
  });
        ...
        <div className="p-4 space-y-4 overflow-y-auto flex-1">
          ...
          <div className="surface-sub rounded-2xl p-3 space-y-2">
            <div className="flex items-center justify-between">
              ...
              <Button
                variant={showCouponField ? "default" : "outline"}
                size="sm"
                  ...
                  <div className="flex items-center justify-between bg-green-50 border border-green-200 rounded-lg p-2">
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-xs text-green-800">
                        {appliedCoupon.code} - {appliedCoupon.type === 'percentage' ? `${parseFloat(String(appliedCoupon.discount))}%` : `€${parseFloat(String(appliedCoupon.discount)).toFixed(2)}`} off
                      </span>
                    </div>
                    <Button
                      variant="ghost"
          ...
          <span className="font-semibold">{formatCurrency(subtotal)}</span>
            ...
            {taxMode && taxMenuPrice === 0 && (
              <div className="flex justify-between text-xs text-gray-600">
                <span>{t("all_prices_include_tax")}</span>
              </div>
            )}
            {tipAmount > 0 && (
              <div className="flex justify-between text-xs">
...
function ExpandingToolbarMenuItemCard({ 
  item, 
  onSelect, 
  onFirstAdd, 
  calculatePriceWithTax, 
  ...
  return (
    ...
    >
      ...
      <div className="flex-grow">
        ...
        <p className="text-lg font-semibold text-paydine-rose-beige">{formatCurrency(calculatePriceWithTax(item.price || 0))}</p>
...
function ExpandingBottomToolbar({
  ...
  taxMenuPrice,
  taxPercentage,
}: ExpandingBottomToolbarProps & { taxMode: number; taxMenuPrice: number; taxPercentage: number }) {
                ...
                <OptimizedImage
                          ...
                          <div>
                            ...
                            <motion.div
                              ...
                              exit={{ opacity: 0, y: -5 }}
                            >
          {formatCurrency(taxMode && taxMenuPrice === 0 ? (item.item.price || 0) + ((item.item.price || 0) * (taxPercentage / 100)) : (item.item.price || 0))} × {item.quantity}
                            </motion.div>
                          </div>
                        </div>
                        <motion.div
...
function MenuContent() {
  ...
  useEffect(() => {
    ...
    api.getSettings()
      .then(response => {
        ...
        // Check if response has success field, otherwise treat direct data as successful
        const isSuccess = response.success !== false;
        const data = response.data || response;
        ...
        if (isSuccess && data) {
          ...
          console.log('✅ Setting tax settings:', newTaxSettings);
          setTaxSettings(newTaxSettings);
        } else {
          console.log('❌ Tax settings response not successful:', response);
        }
      })
      .catch(error => {
        console.error('❌ Failed to fetch tax settings:', error);
  ...
  // Helper function for JSX to calculate price with tax
