...
export class ApiClient {
  ...
  async getAvailableCoupons(): Promise<{ success: boolean; data: any[] }> {
    try {
      const endpoint = this.envConfig.getApiEndpoint('/checkout/coupons');
      const response = await fetch(endpoint);
      const data = await response.json();
      return data;
    } catch (error) {
      console.log('Failed to fetch available coupons:', error);
      return {
        success: false,
        data: []
      };
  ...
  async validateCoupon(code: string, subtotal: number): Promise<{ success: boolean; data?: any; message?: string }> {
    try {
      const endpoint = this.envConfig.getApiEndpoint('/checkout/validate-coupon');
      const response = await fetch(endpoint, {
        method: 'POST',
        headers: {
          'Accept': 'application/json',
