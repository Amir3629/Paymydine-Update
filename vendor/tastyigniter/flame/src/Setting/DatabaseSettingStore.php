...
class DatabaseSettingStore extends SettingStore
{
    ...
    protected function write(array $data)
    {
        ...
        // FIXED: Only process keys that are in the save data
        // This prevents accidental deletion of settings when saving a subset
        foreach ($insertData as $key => $value) {
            if (isset($keys[$key])) {
                // Key exists in DB
                if (is_null($value)) {
                    // Explicitly set to null - mark for deletion
                    $deleteKeys[] = $key;
                } elseif ($keys[$key] != $value) {
                    // Value changed - mark for update
                    $updateData[$key] = $value;
                }
                // If value unchanged, do nothing
            } else {
                // Key doesn't exist in DB - will be inserted (if not null)
                if (!is_null($value)) {
                    $newInsertData[$key] = $value;
                }
            }
        }
        ...
        // Insert new settings (only non-null values)
        if ($newInsertData) {
            $this->newQuery(true)
                ->insert($this->prepareInsertData($newInsertData));
        }

        // FIXED: Only delete keys that were explicitly set to null in save data
        // This prevents accidental deletion when saving a subset of settings
        if ($deleteKeys) {
            $this->newQuery()
