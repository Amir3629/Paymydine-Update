...
export function WorldlineInlineCardForm({
  ...
  const buildPaymentRequest = (nextData: typeof formData) => {
    ...
    const maybeSet = (fieldId: string, value: string) => {
      if (!value) return
      try {
    ...
    const cardholderName = (nextData.cardholderName || "").trim()
    const cardNumber = normalizeCardNumber(nextData.cardNumber)
    const expiryDigits = (nextData.expiryDate || "").replace(/\D/g, "").slice(0, 4)
    ...
    // Worldline product schema can vary by merchant/product config
    maybeSet("expiryDate", expiryDigits)      // MMYY
    maybeSet("expiryMonth", expiryMonth)      // MM
    maybeSet("expiryYear", expiryYear4)       // YYYY

    maybeSet("cvv", cvv)
    maybeSet("securityCode", cvv)
  ...
  const validateWorldlineFields = (nextData: typeof formData) => {
    ...
    const expiryMonthField = paymentProduct.getField("expiryMonth")
    const expiryYearField = paymentProduct.getField("expiryYear")
    const cvvField = paymentProduct.getField("cvv") || paymentProduct.getField("securityCode")

    const cardNumberValidation = cardNumberField?.validate?.(cardNumberValue)

    let expiryValidation: any = null
    if (expiryDateField) {
      expiryValidation = expiryDateField.validate?.(expiryDigits)
    } else {
      const expiryMonthValidation = expiryMonthField?.validate?.(expiryMonth)
      const expiryYearValidation = expiryYearField?.validate?.(expiryYear4)
      expiryValidation = expiryMonthValidation || expiryYearValidation
    }
...
try {
  ...
  console.info("WORLDLINE PRODUCT FIELDS:", fields.map((f:any)=>({
    id: f.id,
    type: f.type
  })))
} catch (e) {
  console.error("FAILED TO READ FIELDS", e)
}
        ...
        try {
          const p: any = resolvedInit.paymentProduct
          const fields = typeof p?.getFields === "function"
            ? p.getFields()
            : (Array.isArray(p?.fields) ? p.fields : [])

          if (!fields || fields.length === 0) {
            setError(
              "WORLDLINE_PRODUCT_DEBUG:\n" +
              JSON.stringify({
                productKeys: Object.keys(p || {}),
                hasGetFields: typeof p?.getFields,
                id: p?.id,
                paymentMethod: p?.paymentMethod,
                displayHints: p?.displayHints || null,
                fieldsPropType: Array.isArray(p?.fields) ? "array" : typeof p?.fields,
                fieldsPreview: Array.isArray(fields)
                  ? fields.map((f: any) => ({
                      id: f?.id,
                      type: f?.type,
                      dataType: f?.dataType,
          ...
          setError("WORLDLINE_PRODUCT_DEBUG_FAILED: " + String(debugErr))
        }
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const availableFieldIds = typeof paymentProduct?.getFields === "function"
        ? paymentProduct.getFields().map((f: any) => f?.id).filter(Boolean)
        : []
      const debugPayload = {
        rawError: rawMsg,
        availableFieldIds,
        sentValues: {
          ...
          expiryDate: (formData.expiryDate || "").replace(/\D/g, "").slice(0, 4),
