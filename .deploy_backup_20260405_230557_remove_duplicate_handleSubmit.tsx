  ...
  const handleSubmit = async (event: React.FormEvent) => {
    try {
      console.info("🔥 SUBMIT STARTED", {
        expiryRaw: typeof getExpiryDigits === "function" ? getExpiryDigits(formData?.expiryDate) : String(formData?.expiryDate ?? ""),
        expiryDisplay: typeof normalizeExpiry === "function" ? normalizeExpiry(formData?.expiryDate) : String(formData?.expiryDate ?? ""),
        card: typeof normalizeCardNumber === "function" ? normalizeCardNumber(formData?.cardNumber) : String(formData?.cardNumber ?? ""),
        cvv: typeof normalizeCvv === "function" ? normalizeCvv(formData?.cvv) : String(formData?.cvv ?? "")
      })
    } catch (e) {
    ...
    try {
      console.info("🔥 SUBMIT STARTED", {
        expiryRaw: getExpiryDigits(formData?.expiryDate),
        expiryDisplay: normalizeExpiry(formData?.expiryDate),
        card: normalizeCardNumber(formData?.cardNumber),
...
export function WorldlineInlineCardForm({
  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      // 🚨 FORCE CORRECT VALUES INTO SDK (bypass mask issues)
      paymentRequest.setValue("cardNumber", normalizeCardNumber(formData.cardNumber))
      paymentRequest.setValue("expiryDate", getExpiryDigits(formData.expiryDate)) // MMYY ONLY
      ...
      const debugValues = {
        ...
        expiryDateDisplay: normalizeExpiry(formData.expiryDate),
        expiryDateRaw: getExpiryDigits(formData.expiryDate),
        cvv: normalizeCvv(formData.cvv),
        cardholderName: String(formData.cardholderName ?? ""),
      }
      console.info("[WorldlineInlineCardForm] WORLDLINE FINAL VALUES", debugValues)

      // 🔥 FORCE RAW VALUES INTO PAYMENT REQUEST (FINAL FIX)
      try {
        paymentRequest.setValue("cardNumber", normalizeCardNumber(formData.cardNumber))
        paymentRequest.setValue("expiryDate", getExpiryDigits(formData.expiryDate)) // MUST BE MMYY
        paymentRequest.setValue("cvv", normalizeCvv(formData.cvv))
        paymentRequest.setValue("cardholderName", String(formData.cardholderName ?? "").trim())

        console.info("[WORLDLINE FORCE OVERRIDE]", {
          expiryRaw: getExpiryDigits(formData.expiryDate)
        })
      } catch (e) {
        console.warn("FORCE SET FAILED", e)
      }
      ...
      // 🔥 DEEP DEBUG BEFORE ENCRYPT
      try {
        console.info("WORLDLINE PAYMENT REQUEST DUMP", {
          cardNumber: paymentRequest.getValue("cardNumber"),
          expiryDate: paymentRequest.getValue("expiryDate"),
          cvv: paymentRequest.getValue("cvv"),
          cardholderName: paymentRequest.getValue("cardholderName"),
          keys: Object.keys(paymentRequest || {})
        })
      } catch(e) {
        console.warn("DEBUG READ FAILED", e)
      }
      ...
      const debugPayload = {
        ...
        sentValues: {
          cardNumber: normalizeCardNumber(formData.cardNumber),
          expiryDate: normalizeExpiry(formData.expiryDate),  // MM/YY enforced
          cvv: normalizeCvv(formData.cvv),
          cardholderName: formData.cardholderName,
          email: formData.email,
        },
