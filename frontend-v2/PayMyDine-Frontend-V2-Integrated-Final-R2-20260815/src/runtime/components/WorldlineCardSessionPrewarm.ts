...
function normalizedItems(value: WorldlineCardPrewarmInput['selected_items']) {
  ...
  return value
    .map((entry) => ({ order_menu_id: Number(entry?.order_menu_id || 0), quantity: Number(entry?.quantity || 0) }))
...
function cleanup() {
  const now = Date.now()
  for (const [key, entry] of cache.entries()) if (now - entry.createdAt > TTL_MS) cache.delete(key)
...
async function createSession(input: WorldlineCardPrewarmInput): Promise<any> {
  const response = await window.fetch(ENDPOINT, {
    ...
    body: JSON.stringify({ ...input, payment_method: 'card', provider: 'worldline' }),
  })
  const data = await response.json().catch(() => ({}))
  if (!response.ok || data?.success === false) throw new Error(String(data?.error || data?.message || `HTTP ${response.status}`))
  if (String(data?.flow || '').toLowerCase() !== 'native_card') throw new Error('Worldline did not return a native card session.')
  return data
}
...
export async function consumeWorldlineCardSession(input: WorldlineCardPrewarmInput): Promise<any | null> {
  ...
  cache.delete(key)
  try { return await entry.promise } catch { return null }
}
