...
import { Bell, Car, Menu, Phone, ReceiptText, Search, ShoppingBag, StickyNote } from 'lucide-react'
...
export default function NoirEditorial() {
  const {
    ...
    visibleItems, featuredItems, search, setSearch, openItem, openService, openCart,
    openCheckout, cartCount, activeOrder, formatCurrency, direction,
  } = useMenuRuntime()
    ...
    <main className={styles.root} style={rootStyle} dir={direction} data-theme-id="noir_editorial">
      <header className={styles.header}>
        ...
        <div className={styles.headerActions}>
          ...
          <LanguageSelect />
          <button className={styles.iconButton} type="button" onClick={() => openCart()} aria-label={labels.cart}>
            <ShoppingBag /><span>{cartCount}</span>
          </button>
          <button className={styles.iconButton} type="button" onClick={() => openCheckout()} aria-label={labels.tableOrder}>
            <Menu /><span>{activeOrder?.items.length || 0}</span>
          </button>
        </div>
      </header>
        ...
        <div className={styles.heroCopy}>
          ...
          <div className={styles.heroActions}>
            {bootstrap.features.callRestaurant && bootstrap.restaurant.phone && (
              <a className={styles.callButton} href={`tel:${bootstrap.restaurant.phone}`}><Phone /> <span><small>{labels.callRestaurant}</small>{bootstrap.restaurant.phone}</span></a>
            )}
            {bootstrap.features.waiterCall && <button className={styles.ghostAction} type="button" onClick={() => openService('waiter')}><Bell /> {labels.callWaiter}</button>}
            {bootstrap.features.valet && <button className={styles.ghostAction} type="button" onClick={() => openService('valet')}><Car /> {labels.valet}</button>}
          </div>
        </div>
      ...
      <section className={styles.items}>
        {visibleItems.map((item, index) => (
          <article className={`${styles.item} ${index % 2 ? styles.itemReverse : ''}`} key={item.id}>
      ...
      <aside className={styles.serviceRail} aria-label={labels.service}>
        ...
        <button type="button" onClick={() => openCheckout()}><ReceiptText /><span>{labels.tableOrder}</span></button>
        <button type="button" onClick={() => openCart()}><ShoppingBag /><span>{labels.cart} · {cartCount}</span></button>
      </aside>
