      ...
      footer={cart.length > 0 && (
        <div className={styles.stack}>
          ...
          {bootstrap.features.tableOrdering && (bootstrap.table.id || bootstrap.table.number) ? (
            ...
            <button className={styles.primary} type="button" onClick={openCheckout}>{labels.checkout}</button>
          )}
        </div>
      )}
    >
...
function ServiceSheet() {
  const { labels, serviceMode, callWaiter, sendNote, requestValet, requestStatus } = useMenuRuntime()
  const [note, setNote] = useState('')
  const [name, setName] = useState('')
  const [licensePlate, setLicensePlate] = useState('')
  const [carMake, setCarMake] = useState('')
  const [localError, setLocalError] = useState('')
  const title = serviceMode === 'waiter' ? labels.callWaiter : serviceMode === 'note' ? labels.leaveNote : labels.valet
  ...
  const run = async () => {
    ...
    try {
      ...
      if (serviceMode === 'note') await sendNote(note)
  ...
  return (
    <PanelShell
      ...
      modal
      footer={<button className={styles.primary} type="button" onClick={() => void run()} disabled={status?.state === 'sending'}>{status?.state === 'sending' ? labels.pending : serviceMode === 'waiter' ? labels.callWaiter : serviceMode === 'note' ? labels.send : labels.requestValet}</button>}
    >
      <div className={styles.stack}>
        {serviceMode === 'waiter' && <div className={styles.orderCard}><Bell /><strong>{labels.waiterConfirm}</strong></div>}
        {serviceMode === 'note' && <label className={styles.label}>{labels.leaveNote}<textarea className={styles.textarea} value={note} onChange={(event) => setNote(event.target.value)} maxLength={1000} placeholder={labels.notePlaceholder} /></label>}
        {serviceMode === 'valet' && (
          <div className={styles.stack}>
...
function CheckoutSheet() {
  const { labels, activeOrder, cart, cartSubtotal, formatCurrency, confirmPersonalItems, orderLoading } = useMenuRuntime()
