...
function asset(value: unknown): string | null {
  ...
  if (clean.startsWith('assets/') || clean.startsWith('storage/') || clean.startsWith('api/')) return `/${clean}`
...
export function normalizeRestaurant(settingsPayload: unknown, restaurantPayload: unknown): RestaurantBrand {
  ...
  return {
    ...
    description: str(first(restaurant, ['description']), str(first(settings, ['restaurant_description','site_description']))),
...
export function normalizeFeatures(settingsPayload: unknown, themePayload: unknown): FeatureFlags {
  ...
  return {
    waiterCall: read(['waiter_call_enabled','pmd_waiter_call_enabled']),
    tableNotes: read(['table_notes_enabled','pmd_table_notes_enabled']),
    valet: read(['valet_enabled','enable_valet','pmd_valet_enabled','show_valet']),
    tableOrdering: read(['table_order_enabled','guest_order']),
    splitBill: read(['split_bill_enabled','pmd_split_bill_enabled']),
    tips: read(['tips_enabled','tip_enabled']),
    coupons: read(['coupons_enabled','coupon_enabled']),
    socialLinks: read(['pmd_homepage_social_icons_enabled','pmd_kazen_social_enabled','pmd_kazen_website_enabled','social_links_enabled']),
    callRestaurant: read(['call_restaurant_enabled'], true),
  }
}
...
export function normalizeTable(payload: unknown, query: { tableId?: string|null; tableNo?: string|null; qr?: string|null }): TableContext {
  const root = unwrap(payload)
  return {
    id: str(first(root, ['table_id','id']), query.tableId || '') || null,
    number: str(first(root, ['table_no','table_number','number']), query.tableNo || '') || null,
    name: str(first(root, ['table_name','name'])) || null,
    qr: str(first(root, ['qr_code','qr']), query.qr || '') || null,
