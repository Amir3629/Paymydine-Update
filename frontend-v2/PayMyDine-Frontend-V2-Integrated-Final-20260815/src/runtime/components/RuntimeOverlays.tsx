...
function ServiceSheet() {
  const { labels, serviceMode, callWaiter, requestValet, requestStatus } = useMenuRuntime()
  const [name, setName] = useState('')
  const [licensePlate, setLicensePlate] = useState('')
  const [carMake, setCarMake] = useState('')
  const [localError, setLocalError] = useState('')
  const title = serviceMode === 'waiter' ? labels.callWaiter : labels.valet
  ...
  return (
    <PanelShell
      ...
      modal
      footer={<button className={styles.primary} type="button" onClick={() => void run()} disabled={status?.state === 'sending'}>{status?.state === 'sending' ? labels.pending : serviceMode === 'waiter' ? labels.callWaiter : labels.requestValet}</button>}
    >
      <div className={styles.stack}>
