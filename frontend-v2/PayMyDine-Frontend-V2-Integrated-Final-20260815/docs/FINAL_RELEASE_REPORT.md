...
A V2-only backend read endpoint and 10-theme Admin field definition are included. The installer adds `/api/v2/frontend-theme` after the legacy theme routes, backs up affected live files and does not replace `/simple-theme`. This isolates staging V2 from current port-3001 theme compatibility.
