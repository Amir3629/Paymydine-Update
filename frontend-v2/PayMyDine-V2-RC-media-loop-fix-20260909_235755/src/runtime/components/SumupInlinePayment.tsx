...
function normalizeLoadedSumupMethods(value: unknown): string[] {
...
function SumupCardWidgetPayment(props: Props) {
  ...
  const verifyAndSettle = async (intent: SplitPaymentIntent | null, amount: number) => {
    ...
    for (let attempt = 0; attempt < 8; attempt += 1) {
      last = await requestJson('/api/v1/payments/sumup/widget/status', { checkout_id: checkoutId, order_id: props.orderId, amount, currency: String(props.currency || 'EUR').toUpperCase() })
  ...
  useEffect(() => {
    ...
    const setup = async () => {
      ...
      try {
        ...
        const requestedMethods = requestedSumupMethods(props.methodCode)
        ...
        const config: Record<string, unknown> = {
          ...
          showFooter: true,
          onPaymentMethodsLoad: (sdkMethods: unknown) => {
  ...
  return (
    <section className={`${styles.stripeInlineBox} ${styles.sumupInlineBox}`} data-pmd-sumup-inline-widget="r1" data-pmd-sumup-method={props.methodCode}>
      <div className={styles.stripeSecureTitle}><CreditCard aria-hidden="true" /> {requestedMethodTitle}</div>
