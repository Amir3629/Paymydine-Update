...
Admin saves a canonical Theme ID. `/api/v1/frontend-theme-v2` returns that ID plus `theme_version`. Next resolves the Theme on the server before rendering. The browser does not first render a default/old Theme and then repaint it.
