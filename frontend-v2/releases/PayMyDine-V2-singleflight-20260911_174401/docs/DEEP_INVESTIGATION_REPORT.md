...
### Historical Table Note endpoint

The legacy backend exposes `POST /api/v1/table-notes`, but the final V2 Product Contract intentionally does not render or call Table Note from the dine-in QR menu. The endpoint remains a backend compatibility detail only.
