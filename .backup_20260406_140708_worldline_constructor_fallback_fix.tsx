  ...
  const handleSubmit = async (event: React.FormEvent) => {
    ...
    try {
      ...
      const buildCandidates = () => {
        ...
        out.push({
          name: "sdk.createPaymentRequest(paymentProduct.id)",
          build: () => {
            if (typeof sdk?.createPaymentRequest === "function") {
              return sdk.createPaymentRequest(paymentProduct?.id)
            }
            return null
          }
        })
        ...
        out.push({
          name: "paymentProduct.createPaymentRequest()",
          build: () => {
            if (typeof paymentProduct?.createPaymentRequest === "function") {
              return paymentProduct.createPaymentRequest()
            }
            return null
          }
        })
        ...
        out.push({
          name: "new PaymentRequest(paymentProduct)",
          build: () => new PaymentRequest(paymentProduct)
        })
        ...
        out.push({
          ...
          build: () => {
            ...
            try {
              req.paymentProduct = paymentProduct
            } catch {}

            try {
              req.paymentProductId = paymentProduct?.id
            } catch {}
